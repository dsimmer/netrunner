...
type ChooseAccessFn = (
  accessAmount: Record<string, unknown>,
...
export function registerChooseAccess(
  serverType: string,
  fn: ChooseAccessFn,
): void {
...
export function accessHelperRemote(
  ...
  const available = rootContent(state, server[0], (c) =>
    alreadyAccessed.has(c.cid),
  );

  if (
    available.length === 0 ||
    !mustContinue(state, (c) => alreadyAccessed.has(c.cid), accessAmount, args)
  )
    return null;
      ...
      effect: req(() => {
        ...
        continue_ability(
          ...
          accessHelperRemote(
            ...
            new Set([...alreadyAccessed, available[0].cid]),
            args,
          ),
          null,
          null,
  ...
  return {
    prompt:
      "Click a card to access it. You must access all cards in this server.",
    choices: {
      card: (card: Card) => available.some((c) => utils.sameCard(c, card)),
      all: true,
    },
    async: true,
    effect: req((s, sid, e, cd, tgt) => {
      ...
      continue_ability(
        ...
        accessHelperRemote(
          ...
          e,
          {
            totalMod: accessBonusCount(s, sid, ":total"),
            chosen: accessAmount.chosen + 1,
          },
          new Set([...alreadyAccessed, (target as Card).cid]),
          args,
        ),
        target,
        null,
...
export function accessHelperRd(
  ...
  const upgradeButtons = args["no-root"]
    ? []
    : root.filter(coreCard.isRezzed).map((c) => c.title);

  const unrezzedCard = "Unrezzed upgrade";
  const unrezzedCardsButton = args["no-root"]
    ? undefined
    : root.filter((c) => !coreCard.isRezzed(c)).length > 0
      ? [unrezzedCard]
      : undefined;
  ...
  if (
    choices.length === 0 ||
    !mustContinue(state, alreadyAccessedFn, accessAmount, args)
  )
    return null;
  ...
  const cardFromDeckFn = req(() => {
    ...
    continue_ability(
      ...
      accessHelperRd(
        ...
        eid,
        {
          "random-access-limit": randomLimit - 1,
          totalMod: accessBonusCount(state, side, ":total"),
          chosen: accessAmount.chosen + 1,
        },
        new Set([...alreadyAccessed, cardToAccess!.cid]),
        args,
      ),
      null,
      null,
    ...
    if (unrezzed.length === 1) {
      ...
      continue_ability(
        ...
        accessHelperRd(
          ...
          eid,
          {
            "random-access-limit": randomLimit,
            totalMod: accessBonusCount(state, side, ":total"),
            chosen: accessAmount.chosen + 1,
          },
          new Set([...alreadyAccessed, unrezzed[0].cid]),
          args,
        ),
        null,
        null,
      ...
      continue_ability(
        ...
        side,
        {
          ...
          choices: {
            card: (card: Card) => unrezzed.some((c) => utils.sameCard(c, card)),
          },
          effect: req((s, sid, e, cd, tgt) => {
            const target = forms.context(s, cd, tgt);
            accessCard(s, sid, e, target);
      ...
      if (accessed) {
        ...
        continue_ability(
          ...
          accessHelperRd(
            ...
            new Set([...alreadyAccessed, accessed.cid]),
            args,
          ),
          accessed,
          null,
...
export function accessHelperHq(
  ...
  alreadyAccessed: Set<string>,
  args: { server: string[]; noRoot?: boolean; accessFirst?: Card[] },
): any {
  const preventHandAccess = (state.run as any)?.["prevent-hand-access"];
  const hand =
    !preventHandAccess &&
    !coreEffects.anyEffects(state, "runner", ":disable-random-accesses", true)
      ? state.corp.hand
      : [];
  const alreadyAccessedFn = (card: Card) => alreadyAccessed.has(card.cid);
  const randomLimit = accessAmount["random-access-limit"] || 1;
  ...
  const cardFromButton =
    randomLimit > 0 && hand.filter((c) => !alreadyAccessedFn(c)).length > 0
      ? [cardFrom]
      : [];

  const server = (args.server as string[]) || [];
  const root = args["no-root"]
    ? []
    : rootContent(state, server[0], alreadyAccessedFn);
  ...
  const unrezzedCardsButton = args["no-root"]
    ? undefined
    : root
          .filter((c) => !coreCard.isRezzed(c))
          .filter((c) => !alreadyAccessedFn(c)).length > 0
      ? [unrezzedCard]
      : undefined;
  ...
  const cardFromHandFn = req(() => {
    const corpChooseHq = coreEffects.anyEffects(
      state,
      side,
        ...
        "corp",
        {
          ...
          choices: {
            card: (card: Card) =>
              ...
              coreCard.isCorp(card) &&
              !alreadyAccessedFn(card),
          },
          effect: req((s, sid, e, cd, tgt) => {
            ...
            continue_ability(
              ...
              accessHelperHq(
                ...
                new Set([...alreadyAccessed, selected.cid]),
                args,
              ),
              selected,
              null,
          ...
          cancel: {
            async: true,
            effect: req((s, sid, e, cd, tgt) => {
              const accessed = accessCardsFromHq(s).find(
    ...
    const accessed = accessCardsFromHq(state).find(
      (c) => !alreadyAccessedFn(c),
    );
    if (accessed) {
      ...
      effect: req(() => {
        ...
        continue_ability(
          ...
          accessHelperHq(
            ...
            new Set([...alreadyAccessed, firstCard.cid]),
            { ...args, "access-first": rest },
          ),
          firstCard,
          null,
        );
...
registerChooseAccess("remote", (accessAmount, server, args) => {
  ...
  const totalCards = onlyCard
    ? [onlyCard]
    : getAllContent(content).filter((c) =>
        coreFlags.canAccessLoud(state, side, c),
      );
...
registerChooseAccess("rd", (accessAmount, server, args) => {
  ...
  const totalCards = onlyCard
    ...
    : [
        ...take(
          accessAmount["random-access-limit"] || 1,
          accessCardsFromRd(state),
        ),
        ...(args["no-root"]
          ? []
          : (state.corp.servers as any)?.rd?.content || []),
      ];
  const totalCardsCount = totalCards.length;
  const totalMod = accessAmount.totalMod || 0;
  const posMax = maxAccess ? totalMod + maxAccess > 0 : true;
  ...
  if (posMax && posTotal && onlyCard) {
    if (
      coreCard.inDeck(onlyCard) ||
      coreCard.inZone(onlyCard, "servers", "rd")
    ) {
      accessCard(state, side, coreEid.makeEID(state), onlyCard);
    }
  } else if (posMax && posTotal) {
...
registerChooseAccess("hq", (accessAmount, server, args) => {
  ...
  if (posMax && posTotal && onlyCard) {
    if (
      coreCard.inHand(onlyCard) ||
      coreCard.inZone(onlyCard, "servers", (server as string[])?.[0] || "")
    ) {
      accessCard(state, side, coreEid.makeEID(state), onlyCard);
    }
  } else if (posMax && posTotal) {
...
registerChooseAccess("archives", (accessAmount, server, args) => {
  ...
  const totalCards = onlyCard
    ...
    : [
        ...state.corp.discard,
        ...(args["no-root"] ? [] : rootContent(state, "archives")),
      ];
  const totalCardsCount = totalCards.length;
  const totalMod = accessAmount.totalMod || 0;
  const posMax = maxAccess ? totalMod + maxAccess > 0 : true;
  ...
  if (posMax && posTotal && onlyCard) {
    if (
      coreCard.inDiscard(onlyCard) ||
      coreCard.inZone(onlyCard, "servers", "archives")
    ) {
      accessCard(state, side, coreEid.makeEID(state), onlyCard);
    }
  } else if (posMax && posTotal) {
...
export function accessHelperArchives(
  ...
  accessAmount: { chosen: number; totalMod?: number },
  alreadyAccessed: Set<string>,
  args: { server?: string[]; noRoot?: boolean },
): any {
  const alreadyAccessedFn = (card: Card) => alreadyAccessed.has(card.cid);
  ...
  const filteredAlreadyAccessed = new Set(
    [...alreadyAccessed].filter((cid) => currentAvailable.has(cid)),
  );

  const faceupCardsButtons = faceupAccessible(state, alreadyAccessedFn).map(
    (c) => c.title,
  );
  const unrezzedCard = "Unrezzed upgrade";
  ...
  const facedownCardsButton =
    facedownCards(state, alreadyAccessedFn).length > 0
      ? [facedownCard]
      : undefined;

  const everythingElse = "Everything else";
  const everythingElseButton =
    archivesInactive(state, alreadyAccessedFn).length > 0
      ? [everythingElse]
      : undefined;
    ...
    if (unrezzed.length === 1) {
      ...
      continue_ability(
        ...
        side,
        {
          ...
          choices: {
            card: (card: Card) =>
              coreCard.getZone(card)[0] === "servers" &&
              coreCard.getZone(card)[1] === "archives" &&
              !alreadyAccessedFn(card),
          },
          effect: req((s, sid, e, cd, tgt) => {
            const target = forms.context(s, cd, tgt);
            const newAlreadyAccessed = new Set([
              ...alreadyAccessed,
              (target as Card).cid,
            ]);
            accessCard(s, sid, e, target);
  ...
  const everythingElseFn = req(() => {
    ...
    continue_ability(
      ...
      accessHelperArchives(
        ...
        {
          totalMod: accessBonusCount(state, side, ":total"),
          chosen: accessAmount.chosen + inactive.length,
        },
        new Set([...alreadyAccessed, ...inactive.map((c) => c.cid)]),
        args,
      ),
      null,
      null,
  ...
  return {
    ...
    effect: req((s, sid, e, cd, tgt) => {
      ...
      const allAvailable = [
        ...faceupAccessible(state, alreadyAccessedFn),
        ...root,
      ];
...
function accessible(state: GameState, card: Card): boolean {
  return (
    ...
    coreEngine.shouldTrigger(
      ...
      card,
      null,
      coreTypes.getCardDef(card)?.["on-access"],
    )
  );
}
...
export function numCardsToAccess(
  ...
  return fn
    ? fn(state, side, server, accessAmount)
    : { totalMod: 0, chosen: 0 };
}
...
numCardsToAccessMap["rd"] = (state, side, _server, accessAmount) =>
  numCardsCentral(state, side, 1, "rd", accessAmount);
numCardsToAccessMap["hq"] = (state, side, _server, accessAmount) =>
...
export function turnArchivesFaceup(
  ...
  if (getServerType(server) === "archives") {
    ...
    const known = discard
      .filter((c) => c.seen)
      .map((c) => ({ ...c, new: undefined }));
    const unknown = discard
      .filter((c) => !c.seen)
      .map((c) => ({ ...c, seen: true, new: true }));
...
export function breachServer(
  ...
  coreSay.systemMsg(
    state,
    side,
    `breaches ${coreServers.zoneToName(server[0])}`,
  );

  coreEngine.triggerEventSimult(state, side, null, ":breach-server", {
    server: server[0],
  });
  state.breach = {
    "breach-server": server[0],
    "from-server": server[0],
  } as any;
