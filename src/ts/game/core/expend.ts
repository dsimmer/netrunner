...
import type { GameState, Card, Ability, EID } from "./types.js";
import { cardDef } from "./card_defs.js";
import { checkpoint } from "./checkpoint.js";
import { queueEvent, resolveAbility } from "./engine.js";
import { toC, mergeCosts, canPay } from "./payment.js";
import { req, wait_for } from "../macros.js";
...
export function expend(ex: Ability | undefined): Ability {
  ...
  return {
    req: req((state: GameState, side: string, eid: EID, card: Card, targets: any[]) => {
      const eidWithSource = { ...eid, source: card, sourceType: "ability" };
      const canPayResult = canPay(state, side, eidWithSource, card, null, mergedCost);
    ...
    async: true,
    action: true,
    effect: req((state: GameState, _side: string, eid: EID, card: Card, _targets: any[]) => {
      wait_for(
        state,
        [
          () => queueEvent(state, "expend-resolved", { card }),
          () => checkpoint(state, null, eid, null),
        ],
        [resolveAbility, state, "corp", { ...ex, cost: mergedCost }, card, null],
        { eid },
      );
    }),
  };
