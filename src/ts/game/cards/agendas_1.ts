...
export function addAgendaPointCounters(
  state: State,
  side: Side,
  eid: EID,
  card: Card,
  ...
  return {
    "on-score": {
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `gain ${countIce((state as any).corp)} [Credits]`,
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreGaining.gainCredits(state, side, countIce(corp), {}),
          ],
          [],
        );
      }),
    ...
    "on-score": {
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreProps.addCounter(state, side, card, type, added, {
              placed: true,
            }),
          ],
          [],
    ...
    "on-score": {
      effect: effect(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreProps.addCounter(state, side, card, ctype, qty, null),
          ],
          [],
        );
      }),
  ...
  "on-score": {
    ...
    req: req(function* (
      ...
      return allActive.some(
        (c: Card) => coreCard.installed(c) && coreCard.resource(c),
      );
    }),
    choices: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = (targets as any[])?.[0];
        coreMoving.trash(eid, target, { causeCard: card });
      },
    ),
  },
};
          ...
          const abt = (choices: Card[]) => ({
            ...
            cancel: {
              msg: (
                ...
                `trash ${utils.quantify(choices.length, "card")} from the top of R&D`,
              async: true,
              effect: effect(
                (
                  state: State,
                  ...
                  coreMoving.trashCards(eid, choices, {
                    unpreventable: true,
                    causeCard: card,
                  });
                },
            ...
            effect: req(function* (
              ...
              yield wait_for(
                ...
                [
                  { asyncResult: "result" },
                  coreInstalling.corpInstall(s, sd, chosen, null, {
                    ignoreAllCost: true,
              ...
              const remaining = choices.filter(
                (x: Card) =>
                  !(x && "uuid" in x && x.uuid === (chosen as any).uuid),
              );
              ...
              if (shuffleOccurred) {
                ...
                yield wait_for(
                  ...
                  [
                    { asyncResult: "result" },
                    coreMoving.trashCards(s, sd, eid, remaining, {
                      unpreventable: true,
                      causeCard: c,
                    }),
                  ],
            ...
            [
              ...
              coreEngine.resolveAbility(
                ...
                side,
                {
                  async: true,
                  prompt:
                    "The top cards of R&D are (top->bottom): " +
                    utils.enumerateCards(choices),
                  choices: ["OK"],
                },
                card,
                null,
      ...
      effect: req(function* (
        ...
        if (t === "Gain 1 [Credits]") {
          coreSay.systemMsg(
            state,
            side,
            `uses ${card.title} to gain 1 [Credits]`,
          );
  ...
  events: [
    {
      ...
      optional: {
        req: req(function* (
          ...
          const validCtx = (ctxs: any[]) =>
            ctxs.some(
              (c: any) => coreCard.installed(c.card) && coreCard.corp(c.card),
            );
          return (
            validCtx(targets as any[]) &&
            (state as any)?.activePlayer === ":runner" &&
            coreEvents.firstEvent(state, side, "runner-trash", (t: any[]) => {
              const first = t[0];
              return (
                first &&
                coreCard.installed(first.card) &&
                coreCard.corp(first.card)
              );
            })
          );
        }),
        player: ":runner",
        prompt:
          "Spend [click] to prevent the corporation having +1 allotted [click] during their next turn?",
        "yes-ability": {
          cost: [corePayment.toC("click", 1)],
          "display-side": ":runner",
          msg: ":cost",
        ...
        "no-ability": {
          ...
          msg: (
            ...
            card: Card,
            targets: any[],
          ) => "gain [Click] during their next turn",
          effect: effect(
            (
              state: State,
              side: Side,
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const validCtx = (evs: any[]) =>
          evs.some((e: any) => coreCard.corp(e.card));
        return (
          validCtx(targets as any[]) &&
          coreEvents.firstEvent(
            ...
            side,
            "runner-trash",
            (t: any[]) => t[0] && coreCard.corp(t[0].card),
          )
        );
      }),
...
export const armedIntimidation: CardDef = {
  ...
  "on-score": {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const target: any = (targets as any[])?.[0];
      return `force the Runner to ${forms.target(state, card, targets)?.charAt(0).toLowerCase() || ""}${forms.target(state, card, targets)?.slice(1) || ""}`;
    },
    effect: req(function* (
      state: State,
      side: Side,
      ...
      if (t === "Take 2 tags") {
        ...
        coreDamage.damage(state, ":runner", eid, ":meat", 5, {
          card: card,
          unboostable: true,
        });
      }
        ...
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreEffects.registerLingeringEffect(card, {
            ...
            value: req(function* (
              ...
              return Array(brokenSubs).fill(
                corePayment.toC("trash-from-hand", 1),
              );
            }),
          });
          coreEffects.registerLingeringEffect(card, {
...
export const artificialCryptocrash: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreGaining.loseCredits(":runner", eid, 7);
      },
    ),
  },
};
...
export const astroScriptPilotProgram: CardDef = {
  ...
  "on-score": agendaCounters(1),
  abilities: [
    Object.assign(coreDefHelpers.placeAdvancementCounter(true, 1), {
      cost: [corePayment.toC("agenda", 1)],
    }),
  ],
};
...
export const awardBait: CardDef = {
  ...
  "on-access": {
    ...
    req: req(function* (
      ...
      const canAdvance = allInstalled.filter((c: Card) =>
        coreCard.canBeAdvanced(state, c),
      );
      ...
      continue_ability(
        ...
        side,
        {
          ...
          msg: (
            ...
            const target: any = (targets as any[])?.[0];
            return `place ${utils.quantify(c, "advancement counter")} on ${coreToString.cardStr(state, target)}`;
          },
          async: true,
          effect: effect(
            coreProps.addProp(state, side, eid, target, ":advance-counter", c, {
              placed: true,
            }),
          ),
        },
...
export const azefProtocol: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreDamage.damage(eid, ":meat", 2, { card: card });
      },
    ),
  },
};
...
function bacterialProgrammingInteract(
  ...
  const phrases: string[] = [
    toTrash.length > 0
      ? `trash ${utils.quantify(toTrash.length, "card")} from R&D`
      : null,
    toAdd.length > 0
      ? `add ${utils.quantify(toAdd.length, "card")} to HQ`
      : null,
    toTop.length > 0
      ? `rearrange the top ${utils.quantify(toTop.length, "card")} of R&D`
      : null,
  ...
  const summaryText = () => {
    if (toTrash.length > 0)
      return utils.enumerateCards(toTrash) + " will be trashed. ";
    if (toAdd.length > 0)
      return utils.enumerateCards(toAdd) + " will be added to HQ. ";
    if (toTop.length > 0)
      return (
        "the top of R&D will be (top->bottom): " +
        utils.enumerateCards(toTop.slice().reverse())
      );
    return "";
  };
  return {
    ...
    choices: [
      {
        option: "OK",
        ability: {
          msg: (_s: any, _sd: any, _e: any, _c: any, _t: any) =>
            enumerateText(phrases),
          async: true,
          effect: effect(function* (
            s: any,
            sd: any,
            ...
            for (const c2 of toAdd) {
              yield wait_for(
                state,
                [{ asyncResult: "result" }, coreMoving.move(s, sd, c2, "hand")],
                [],
              );
            }
            for (const c2 of toTrash) {
              yield wait_for(
                ...
                [
                  { asyncResult: "result" },
                  coreMoving.move(s, sd, c2, "deck", { front: true }),
                ],
                [],
              );
            }
            ...
            yield wait_for(
              ...
              [
                ...
                coreMoving.trashCards(
                  ...
                  sd,
                  eid2,
                  deckCards.slice(0, toTrash.length),
            ...
            yield wait_for(
              state,
              [{ asyncResult: "result" }, coreEngine.checkpoint(s, sd, eid2)],
              [],
            );
          }),
        },
      },
      {
        ...
        ability: bacterialProgrammingInteract(
          state,
          side,
          card,
          cards,
    ...
    optional: {
      ...
      "yes-ability": {
        ...
        prompt: (
          ...
          "The top cards of R&D are (top->bottom): " +
          utils.enumerateCards((state as any).corp?.deck?.slice(0, 7) || []),
        choices: ["OK"],
        effect: req(function* (
          state: State,
          side: Side,
          ...
          const corp = (state as any).corp;
          const setAsideCards = coreSetAside.setAsideForMe(
            state,
            side,
            eid,
            corp?.deck?.slice(0, 7) || [],
                  ...
                  ability: {
                    ...
                    effect: effect(function* (
                      ...
                      yield wait_for(
                        ...
                        [
                          { asyncResult: "result" },
                          coreEngine.checkpoint(s, sd, eid2),
                        ],
                        [],
                      );
                    }),
...
export const theBasaltSpire: CardDef = {
  ...
  stolen: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        continue_ability(coreDefHelpers.corpRecur(), card, null);
      },
    ),
  },
  flags: {
  ...
  abilities: [
    {
      ...
      cost: [
        corePayment.toC("trash-from-deck", 1),
...
export const bellona: CardDef = {
  ...
  "steal-cost-bonus": req(function* (
    ...
    return [corePayment.toC("credit", 5)];
  }),
...
export const betterCitizenProgram: CardDef = {
  ...
  events: [
    {
      ...
      optional: {
        req: req(function* (
          ...
          return (
            coreCard.hasSubtype(contextCard, "Run") &&
            coreEvents.firstEvent(
              state,
              ":runner",
              "play-event",
              (t: any[]) => t[0] && coreCard.hasSubtype(t[0].card, "Run"),
            ) &&
            !coreEvents.event(
              state,
              ":runner",
              "runner-install",
              (t: any[]) =>
                t[0] && coreCard.hasSubtype(t[0].card, "Icebreaker"),
            )
          );
        }),
        "waiting-prompt": true,
  ...
  abilities: [
    coreOptional.setAutoresolve(":auto-fire", "Better Citizen Program"),
  ],
};
  ...
  "on-score": {
    optional: {
      req: req(function* (
        ...
        return (
          scored.filter((c: Card) => c.title !== "Bifrost Array").length > 0
        );
      }),
      ...
      "yes-ability": {
        ...
        choices: {
          card: (c: Card) =>
            coreCard.agenda(c) &&
            c.title !== "Bifrost Array" &&
            coreCard.inScored(c) &&
            coreFlags.whenScored(c),
        },
        msg: (
          state: State,
          side: Side,
          ...
          return ((): string => {
            const target: any = (targets as any[])?.[0];
            return `trigger the "when scored" ability of ${target.title}`;
          })();
        },
        async: true,
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            const target: any = (targets as any[])?.[0];
            coreDefHelpers.continueAbility(
              coreCard.getCardDef(target),
              target,
              null,
            );
          },
...
export const brainRewiring: CardDef = {
  ...
  "on-score": {
    optional: {
      "waiting-prompt": true,
      prompt:
        "Pay credits to add random cards from the grip to the bottom of the stack?",
      "yes-ability": {
      ...
      "yes-ability": {
        ...
        effect: req(function* (
          ...
          yield wait_for(
            ...
            [
              { asyncResult: "result" },
              coreProps.addCounter(state, side, card, ":agenda", bp, null),
            ],
            [],
          );
        }),
...
export const cfcExcavationContract: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(function* (
      ...
      const bioroidCount = allActive.filter((c: Card) =>
...
export const characterAssassination: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = (targets as any[])?.[0];
        coreMoving.trash(eid, target, { unpreventable: true, causeCard: card });
      },
    ),
  },
};
...
export const chronosProject: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreMoving.moveZone(":runner", ":discard", ":rfg");
      },
    ),
  },
};
...
export const cityWorksProject: CardDef = {
  ...
  "on-access": {
    req: req(function* (
      ...
      return forms.installed(state, card);
    }),
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      `do ${meatDamage()} meat damage`,
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreDamage.damage(eid, ":meat", meatDamage(), { card: card });
      },
    ),
  },
};
...
export const cloneRetirement: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreBadPublicity.loseBadPublicity(1);
      },
    ),
    silent: true,
  },
