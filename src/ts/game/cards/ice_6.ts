...
function faceupArchivesTypesCount(corp: any): number {
  ...
  subroutines: [
    {
      "change-in-game-state": {
        ...
        req: req(function* (state: State): Generator<any, any, any> {
          return ((state as any).corp?.discard?.length ?? 0) > 0;
        }),
      },
        ...
        yield wait_for(
          ...
          [
            ...
            coreEngine.resolveAbility(
              state,
              side,
              coreIce.moveCardToTopOrBottom(targets[0], ":corp"),
              card,
              null,
            ),
          ],
...
export const logjam: CardDef = {
  ...
  "on-rez": {
    msg: msg(function (state: State, side: Side, eid: EID, card: Card) {
      return `place ${utils.quantify(1 + faceupArchivesTypesCount((state as any).corp), "advancement counter")} on itself`;
    }),
    async: true,
    effect: effect(function* (
      state: State,
      ...
      yield wait_for(
        ...
        [
          ...
          coreProps.addProp(
            ...
            card,
            ":advance-counter",
            1 + faceupArchivesTypesCount((state as any).corp),
            { placed: true },
          ),
        ],
        [],
  ...
  subroutines: [
    Object.assign({}, gainCreditsSub(2), endTheRun, {
      label: "Gain 2 [Credits] and end the run",
...
export const loki: CardDef = {
  ...
  "on-encounter": {
    req: req(function* (state: State): Generator<any, any, any> {
      return (
        coreBoard
          ...
          .filter((c: Card) => coreCard.ice(c)).length >= 2
      );
    }),
    choices: {
      card: (c: Card) => coreCard.ice(c) && coreCard.active(c),
      "not-self": true,
    },
    msg: msg(function (
      state: State,
    ...
    effect: req(function* (
      ...
      const additionalSubs = ((target as any).subroutines ?? []).map(
        (s: any) => s["sub-effect"],
      );
      coreEffects.registerLingeringEffect(state, ":corp", card, {
      ...
      effect: req(function* (
        ...
        if (tgts[0] === "Done") {
          ...
          coreSay.systemMsg(
            s,
            sd,
            `uses ${(c as any).title} to make itself gain ${tgts[0]}`,
          );
          ...
          yield wait_for(
            ...
            [
              ...
              coreEngine.resolveAbility(
                s,
                sd,
                iceSubtypeChoice(newChoices, s, sd, c),
                c,
                null,
              ),
            ],
  ...
  return {
    ...
    "derez-effect": {
      effect: req(function* (
        ...
        coreEffects.unregisterEffectsForCard(
          state,
          side,
          card,
          (e: any) => e.type === ":gain-subtype",
        );
      }),
    },
    events: [
    ...
    subroutines: [
      {
        ...
        "change-in-game-state": {
          ...
          req: req(function* (
            ...
            return (
              coreCard.hasSubtype(card, "Code Gate") &&
              (((state as any).runner?.credit ?? 0) > 0 ||
      ...
      },
      {
        ...
        "change-in-game-state": {
          ...
          req: req(function* (
            ...
            return (
              coreCard.hasSubtype(card, "Sentry") &&
              coreBoard
        ...
        yield wait_for(
          ...
          [
            ...
            coreEngine.resolveAbility(
              state,
              side,
              endTheRunUnlessRunnerPays(corePayment.toC("click", 1), "ability"),
              card,
              null,
            ),
          ],
...
export const macrophage: CardDef = {
  ...
  subroutines: [
    ...
    traceAbility(3, {
      ...
      choices: {
        card: (c: Card) =>
          coreCard.installed(c) && coreCard.hasSubtype(c, "Virus"),
      },
      msg: msg(function (
        state: State,
        side: Side,
...
export const mamba: CardDef = {
  ...
  abilities: [
    powerCounterAbility(
      Object.assign({}, coreDefHelpers.doNetDamage(1), {
        req: req(function* (state: State): Generator<any, any, any> {
  ...
  subroutines: [
    {
      ...
      prompt: "Choose one",
      choices: [
        "Corp gains 4 [Credits] and end the run",
        "Add Meridian to score area",
      ],
      msg: msg(function (
        state: State,
        side: Side,
...
export const mestnichestvo: CardDef = {
  ...
  "on-encounter": {
    optional: {
      prompt:
        "Remove 1 hosted advancement counter to make the Runner lose 3 [Credits]?",
      req: req(function* (
        state: State,
        side: Side,
        eid: EID,
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const context = targets[0];
        const firstPrinted = (context?.ice?.subroutines ?? []).find(
          (s: any) => s.printed,
        );
