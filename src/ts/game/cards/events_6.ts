...
export const moshing: CardDef = {
  ...
  onPlay: {
    ...
    effect: req(function* (
      ...
      yield wait_for(
        state,
        [{ asyncResult: "result" }, coreDrawing.draw(state, side, 3)],
        [],
      );
      yield wait_for(
        state,
  ...
  onPlay: {
    ...
    choices: req(function* (
      ...
      return corePrompts.cancellable(
        deck.filter((c: Card) => coreCard.hasSubtype(c, "Icebreaker")),
        "sorted",
      );
    }),
      ...
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        continue_ability(
          (() => {
            ...
            if (
              (state as any).runner?.register?.successfulRun &&
              ...
              return {
                optional: {
                  ...
                  noAbility: {
                    effect: effect(
                      coreMoving.move(state, side, icebreaker, "hand"),
                      coreShuffling.shuffle(state, side, "deck"),
                    ),
                  },
                },
...
export const networking: CardDef = {
  ...
  onPlay: {
    ...
    effect: req(function* (
      ...
      yield wait_for(
        state,
        [{ asyncResult: "result" }, coreTags.loseTags(state, side, 1)],
        [],
      );
    ...
    effect: req(function* (
      ...
      if (msg === "Gain 4 [Credits]") {
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreGaining.gainCredits(state, "runner", eid, 4),
          ],
          [],
        );
      } else {
...
export const onTheLam: CardDef = {
  ...
  prevention: [
    {
      ...
      ability: {
        ...(corePrevention.preventUpToNTags(3) || {}),
    ...
    },
    {
      ...
      ability: {
        ...(corePrevention.preventUpToNDamage(3, [
          "net",
          "meat",
          "core",
          "brain",
  ...
  onPlay: {
    ...
    choices: {
      card: (c: Card) => coreCard.resource(c) && coreCard.installed(c),
    },
    onChangeGameState: {
      req: req(function* (
        state: State,
      ...
      silent: req(function* (
        ...
        const ashes =
          (state as any).runner?.discard?.filter(
            (c: Card) => c.title === "Out of the Ashes",
          ) || [];
        return (
          card !== ashes[0] ||
          !coreEngine.notUsedOnce(
            state,
            { once: "per-turn", onceKey: "out-of-ashes" },
            card,
          )
        );
      }),
    ...
    optional: {
      ...
      yesAbility: {
        ...
        effect: req(function* (
          ...
          const next = (state as any).runner?.discard?.find(
            (c: Card) =>
              c.title === "Out of the Ashes" && !utils.sameCard(card, c),
          );
          if (next) {
            yield continue_ability(
              state,
  ...
  onPlay: {
    ...
    effect: req(function* (
      ...
      yield wait_for(
        ...
        [
          { asyncResult: "result" },
          coreGaining.gainCredits(state, "runner", 10),
        ],
        [],
      );
      yield wait_for(
      ...
      ability: {
        ...
        choices: {
          req: req(function* (
            ...
            return (
              zone &&
              (state as any).run?.server?.[0] !== zone[1] &&
          ...
          if (coreCard.agenda(msg)) {
            ...
            coreFlags.registerRunFlag(
              ...
              function* (
                ...
                return !utils.sameCard(c, protectedCard);
              },
            );
            coreFlags.registerRunFlag(
              state,
            ...
            yield wait_for(
              ...
              [
                { asyncResult: "result" },
                coreAccess.accessCard(state, side, protectedCard),
              ],
              [],
            );
            ...
            yield wait_for(
              ...
              [
                { asyncResult: "result" },
                coreAccess.accessCard(state, side, eid, msg),
              ],
              [],
            );
          }
  ...
  onPlay: {
    ...
    choices: req(function* (
      ...
      return deck
        .filter(
          (c: Card) =>
            coreCard.hasSubtype(c, "Run") &&
            corePayment.canPay(
              state,
              side,
              { ...eid, source: card, sourceType: "play" },
              c,
              null,
              [
                corePayment.toC(
                  "credit",
                  coreCostFns.playCost(state, side, c) || 0,
                ),
              ],
            ),
        )
  ...
  events: [
    {
      ...
      effect: req(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreMoving.trash(state, "runner", card, {
              cause: "purge",
              causeCard: card,
            }),
          ],
      ...
      ability: {
        ...
        choices: {
          req: req(function* (
            ...
            return coreFlags.inCorpScored(state, side, msg);
          }),
        },
        msg: msg(
          "host itself on ",
...
export const populistRally: CardDef = {
  ...
  onPlay: {
    req: req(function* (
      ...
      return (coreBoard.allActiveInstalled(state, "runner") || []).some(
        (c: Card) => coreCard.hasSubtype(c, "Seedy"),
      );
    }),
    msg: "give the Corp 1 fewer [Click] to spend on [corp-pronoun] next turn",
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreGaining.lose("corp", "click-per-turn", 1);
      },
    ),
  },
  events: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreGaining.gain("corp", "click-per-turn", 1);
        },
      ),
    },
  ],
...
export const powerNap: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg(function* (
      ...
      const count =
        (state as any).runner?.discard?.filter((c: Card) =>
          coreCard.hasSubtype(c, "Double"),
        ).length || 0;
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          coreCard.agenda(ctx.accessedCard) &&
          coreEvents.firstEvent(state, side, "access", (t: any[]) =>
            coreCard.agenda(t[0]?.accessedCard),
          )
        );
      }),
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const ctx: any =
          ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0];
        return (
          coreCard.rezzed(ctx.ice) &&
          coreEngine.notUsedOnce(state, { once: "per-run" }, card) &&
          coreIce.getStrength(ctx.ice) <=
                    ...
                    yesAbility: {
                      ...
                      msg: msg(
                        ...
                        (
                          ...
                          card: Card,
                          targets: any[],
                        ) => coreToString.cardStr(state, ice),
                      ),
                      effect: effect(
                        coreMoving.trash(eid, ice, { causeCard: card }),
                      ),
                    },
                  },
                };
      ...
      ability: {
        ...
        effect: req(function* (
          ...
          coreEngine.registerPendingEvent(
            ...
            "runner-gain-tag",
            card,
            privilegedAccessInstallResource(),
          );
          coreEngine.registerPendingEvent(
            state,
            "runner-gain-tag",
            card,
            privilegedAccessInstallProgram(),
          );
          yield wait_for(
            state,
...
export const processAutomation: CardDef = {
  ...
  onPlay: {
    ...
    effect: req(function* (
      ...
      yield wait_for(
        state,
        [{ asyncResult: "result" }, coreGaining.gainCredits(state, side, 2)],
        [],
      );
      yield wait_for(
        state,
  ...
  onPlay: {
    ...
    effect: req(function* (
      ...
      const allAmounts = Array.from(
        { length: (state as any).runner?.credit + 1 },
        (_, i) => i,
      );
      ...
      yield continue_ability(
        state,
        side,
        pushYourLuckRunnerChoice(choices),
        card,
        null,
      );
    }),
  ...
  return {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        continue_ability(
          "corp",
          pushYourLuckCorpChoice(choices, parseInt(msg, 10)),
          card,
          null,
        );
      },
...
function pushYourLuckCorpChoice(choices: string[], spent: number): any {
  return {
    ...
    effect: req(function* (
      ...
      const correctGuess = (
        msg === "Even" ? (n: number) => n % 2 === 0 : (n: number) => n % 2 !== 0
      )(spent);
      yield wait_for(
        state,
        [
      ...
      coreSay.systemMsg(
        state,
        "corp",
        `${correctGuess ? "" : "in"}correctly guesses ${msg.toLowerCase()}`,
      );
      yield wait_for(
        state,
        [
      ...
      if (correctGuess) {
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreGaining.gainCredits(state, "runner", eid, spent * 2),
          ],
          [],
        );
      }
...
export const pushingTheEnvelope: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg(function* (
      ...
      return (state as any).runner?.hand?.length <= 2
            ...
            return {
              choices: {
                card: (c: Card) =>
                  ...
                  (coreCard.getZone(c) as string[])[
                    (coreCard.getZone(c) as string[]).length - 1
                  ] === "content" &&
                  !c.rezzed,
              },
              msg: msg(
                ...
                (
                  ...
                  card: Card,
                  targets: any[],
                ) => coreUtils.quantify(c, "advancement counter"),
                " on ",
                (
                  state: State,
                  side: Side,
              ...
              effect: effect(
                ...
                coreProps.addProp(state, "corp", msg, "advance-counter", c, {
                  placed: true,
                }),
  ...
  events: [
    ...
    },
    {
      ...
      effect: req(function* (
        ...
        const cardsToDraw =
          ((coreCard.getCard(state, card) as any)?.power as number) || 0;
        yield continue_ability(
          state,
          side,
          {
            msg: msg(
              cardsToDraw > 0
                ? `draw ${cardsToDraw} card and gain 3 [Credits]`
                : "gain 3 [Credits]",
            ),
            async: true,
            effect: effect(
              (
                ...
                cardsToDraw > 0
                  ? coreDrawing.draw(state, side, cardsToDraw)
                  : null;
                coreGaining.gainCredits(state, side, eid, 3);
              },
            ),
  ...
  onPlay: {
    ...
    choices: req(function* (
      ...
      const isSwappable = (c: Card) =>
        c.type === "Identity" &&
        c.side === "Runner" &&
        runnerIdentity?.faction === c.faction &&
        !isDraftId(c) &&
    ...
    effect: req(function* (
      ...
      for (const c of oldIdentity?.hosted || []) {
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreMoving.move(state, side, c, "temp-hosted"),
          ],
          [],
        );
      }
      ...
      for (const c of (state as any).runner?.tempHosted || []) {
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreHosting.host(state, side, (state as any).runner?.identity, c, {
              facedown: true,
            }),
          ],
          [],
...
function rebootInstallCards(toInstall: Card[], titles: string[]): any {
  if (toInstall.length > 0) {
    return {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreInstalling.runnerInstall(state, "runner", toInstall[0], {
            facedown: true,
            noMsg: true,
          });
  ...
  return effect(
    (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      coreSay.systemMsg(
        state,
        "runner",
        `uses ${card.title} to install ${titles.join(", ")} facedown`,
      );
      coreEid.effectCompleted(state, side, eid);
    },
  );
  ...
  onPlay: {
    req: req(function* (
      ...
      return (coreBoard.allInstalled(state, "runner") || []).some(
        (c: Card) =>
          coreCard.runner(c) && (coreCard.program(c) || coreCard.hardware(c)),
      );
    }),
    async: true,
    effect: req(function* (
      ...
      yield wait_for(
        ...
        [
          { asyncResult: "result" },
          coreEngine.resolveAbility(state, side, rejigPickUp(), card, null),
        ],
        [],
      );
      yield continue_ability(
