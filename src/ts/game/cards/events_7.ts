...
import { runAnyServerAbility, runCentralServerAbility, runServerAbility, tutorAbi } from './events_1';
...
function cutlery(_subtype: string): any { return {}; }
...
export function rejigPickUp(): any {
  return {
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreMoving.move(state, side, msg, 'hand'); coreEid.completeWithResult(state, side, eid, msg.cost); }),
  };
}
...
export function rejigPutDown(bonus: number): any {
  return {
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreInstalling.runnerInstall({ ...eid, source: card, sourceType: 'runner-install' }, msg, { costBonus: -bonus, msgKeys: { installSource: card, displayOrigin: true } }); }),
  };
}
...
export const reprise: CardDef = {
  ...
  onPlay: {
    async: true,
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).runner?.register?.stoleAgenda; }),
    prompt: 'Choose an installed Corp card to add to HQ',
    waitingPrompt: true,
    choices: { card: (c: Card) => coreCard.installed(c) && coreCard.corp(c) },
    msg: msg('add ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, msg), ' to HQ'),
    cancel: repriseOptRun(),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreMoving.move('corp', msg, 'hand'); continue_ability(repriseOptRun(), card, null); }),
  },
};
...
function repriseOptRun(): any {
  return {
    optional: {
      ...
      noAbility: {
        effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreSay.systemMsg(`declines to use ${coreCard.getCard(state, card)?.title || 'this card'} to make a run`); }),
      },
    },
  };
}
...
export const reshape: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg('swap the positions of ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, targets?.[0]), ' and ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, targets?.[1])),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreMoving.swapIce(targets?.[0], targets?.[1]); }),
  },
};
...
export const retrievalRun: CardDef = {
  ...
  events: [{
    ...
    ability: {
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreInstalling.runnerInstall(state, side, eid, msg, { msgKeys: { installSource: card, displayOrigin: true }, ignoreAllCost: true }); }),
    },
  }],
};
...
function riggedResultsRunnerChoice(choices: string[]): any {
  return {
    ...
    choices: choices,
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { continue_ability(riggedResultsCorpChoice(choices, parseInt(msg, 10)), card, null); }),
  };
}
...
function riggedResultsChooseIce(): any {
  return {
    ...
    msg: msg('make a run and bypass ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, msg)),
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; coreEngine.registerEvents(
        card,
        [{
          event: 'encounter-ice',
          automatic: 'bypass',
          ...
          msg: msg('bypass ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => ctx.ice?.title),
  ...
  onPlay: {
    ...
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      yield continue_ability(
        ...
        {
          optional: {
            prompt: msg('Charge ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => rigTarget?.title, '?'),
            req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return coreCharge.canCharge(state, side, rigTarget); }),
            yesAbility: {
              async: true,
              effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreCharge.chargeCard(eid, rigTarget); }),
              msg: msg('charge ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => rigTarget?.title),
            },
          },
        },
        card,
...
export const ripDeal: CardDef = {
  ...
  events: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; coreEngine.registerEvents(card, [{
        event: 'candidates-determined',
        duration: 'end-of-run',
        async: true,
        req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; return ctx === 'hq'; }),
        ...
        effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          ...
          yield continue_ability(
            ...
            side,
            {
              async: true,
              showDiscard: true,
              prompt: msg('Choose ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreUtils.quantify(cardsToMove, 'card'), ' to add from the heap to the grip'),
              msg: msg('add ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => (targets || []).map((c: Card) => c.title).join(', '), ' from the heap to the grip'),
...
export const ritual: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg('draw ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreUtils.quantify((state as any).runner?.click || 0, 'card')),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDrawing.draw(state, side, eid, (state as any).runner?.click || 0); }),
  },
};
...
export const runAmok: CardDef = {
  ...
  onPlay: {
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreUpdate.updateIn(card, ['special', 'runAmok'], () => runAmokGetRezzedCids(coreBoard.allInstalled(state, 'corp'))); coreRuns.makeRun(eid, msg, coreCard.getCard(state, card)); }),
  },
  events: [{
    event: 'run-ends',
      ...
      yield continue_ability(
        ...
        diffCards.length > 0
          ? {
              ...
              effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreMoving.trash(eid, msg, { causeCard: card }); }),
            }
          : null,
        card,
        null
...
export const runningHot: CardDef = {
  ...
  onPlay: {
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainClicks(3); coreEid.effectCompleted(eid); }),
  },
};
...
export const runningInterference: CardDef = {
  ...
  staticAbilities: [{
    ...
    value: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return [corePayment.toC('credit', msg.cost)]; }),
  }],
  ...
  events: [
    {
      event: 'encounter-ice',
      automatic: 'bypass',
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return coreEvents.firstRunEvent(state, side, 'encounter-ice'); }),
      once: 'per-run',
      msg: msg('bypass ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, forms.currentIce)),
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreRuns.bypassIce(state); }),
    },
    {
      event: 'encounter-ice',
      skippable: true,
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { continue_ability(
          {
            optional: {
              prompt: msg('Spend [Click] to bypass ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, forms.currentIce), '?'),
              waitingPrompt: true,
              yesAbility: {
                msg: msg('bypass ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, forms.currentIce)),
...
export const satelliteUplink: CardDef = {
  ...
  onPlay: {
    choices: {
      max: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
    ...
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      yield continue_ability(
        ...
        side,
        {
          ...
          msg: msg('trash ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => trashed?.title, ' and install ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg, ', lowering the cost by ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => tcost, ' [Credits]'),
          effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreInstalling.runnerInstall({ ...eid, source: card, sourceType: 'runner-install' }, msg, { costBonus: -tcost }); }),
        },
        card,
        null
      );
...
export const scrounge: CardDef = {
  ...
  onPlay: {
    ...
    showDiscard: true,
    onChangeGameState: {
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).runner?.discard?.some((c: Card) => coreCard.program(c)); }),
    },
    choices: {
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
        const t = targets[0];
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreInstalling.runnerInstall(state, side, msg, { msgKeys: { installSource: card, displayOrigin: true, includeCostFromEid: eid } }); continue_ability(scroungeBottomOneProgram(), card, null); }),
...
function scroungeBottomOneProgram(): any {
  return {
    ...
    showDiscard: true,
    msg: msg('put ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg, ' on the bottom of the stack'),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreMoving.move(state, side, msg, 'deck'); }),
  };
}
...
export const scrubbed: CardDef = {
  ...
  events: [{
    event: 'encounter-ice',
    once: 'per-turn',
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; coreEffects.registerLingeringEffect(card, {
        type: 'ice-strength',
        duration: 'end-of-run',
        req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; return utils.sameCard(msg, ctx.ice); }),
        value: -2,
  ...
  staticAbilities: [{
    ...
    value: {
      ...
      ability: {
        async: true,
        condition: 'floating',
        req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; return corePrevention.preventable(ctx); }),
  ...
  events: [
    {
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreUpdate.updateIn(state, 'runner', 'rdAccessFn', 'reverse'); }),
    },
    {
      event: 'run-ends',
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreUpdate.updateIn(state, 'runner', 'rdAccessFn', 'seq'); }),
    },
  ],
};
...
export const singularity: CardDef = {
  ...
  events: [{
    ...
    ability: {
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; coreMoving.trashCards(eid, (ctx.runServer || {}).content, { causeCard: card }); }),
    },
  }],
};
...
export const socialEngineering: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg('select ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, msg)),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; coreEngine.registerEvents(
        card,
        [{
          event: 'rez',
          ...
          msg: msg('gain ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreCostFns.rezCost(state, side, coreCard.getCard(state, ctx.card)), ' [Credits]'),
          async: true,
...
export const sparkOfInspiration: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { sparkOfInspirationSearch(state, side, eid, card, (state as any).runner?.deck || [], []); }),
  },
};
...
function sparkOfInspirationSearch(state: State, side: Side, eid: EID, card: Card, remainder: Card[], revealedCards: Card[]): any {
  ...
  return effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { continue_ability(sparkOfInspirationShuffleBack(revealedCards), card, null); });
}
...
function sparkOfInspirationInstallProgram(state: State, side: Side, eid: EID, card: Card, revealedCard: Card, revealedCards: Card[]): any {
  if (coreInstalling.runnerCanPayAndInstall(state, side, { ...eid, source: card }, revealedCard, { costBonus: -10 })) {
    return continue_ability(
      ...
      {
        optional: {
          prompt: msg('Install ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => revealedCard.title, ' paying 10 [Credits] less?'),
          waitingPrompt: true,
          yesAbility: {
            async: true,
            effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreRevealing.revealLoud(state, side, card, { andThen: 'shuffle the Stack' }, revealedCards); coreInstalling.runnerInstall(coreEid.makeEid(state, { source: card, sourceType: 'runner-install' }), revealedCard, { costBonus: -10, msgKeys: { installSource: card, displayOrigin: true } }); coreShuffling.shuffle(state, side, 'deck'); coreSay.systemMsg(state, side, 'shuffles the Stack'); coreEid.effectCompleted(state, side, eid); }),
          },
...
function sparkOfInspirationShuffleBack(revealedCards: Card[]): any {
  return {
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreRevealing.revealLoud(state, side, card, { andThen: 'shuffle the Stack' }, revealedCards); coreShuffling.shuffle(state, side, 'deck'); coreEid.effectCompleted(state, side, eid); }),
  };
}
...
export const spearPhishing: CardDef = {
  ...
  events: [{
    event: 'encounter-ice',
    automatic: 'bypass',
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return forms.runPosition === 1; }),
    msg: msg('bypass ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => ctx.ice?.title),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreRuns.bypassIce(state); }),
  }],
};
    ...
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0];
      ...
      yield continue_ability(
        ...
        side,
        {
          ...
          msg: msg('host ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => trojan.title, ' on ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, msg)),
          effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreHosting.host(state, side, msg, trojan); coreIce.updateAllIce(state, side); }),
        },
        card,
        null
      );
...
export const steelskinScarring: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDrawing.draw(eid, 3); }),
  },
  onTrash: {
    whenInactive: true,
