...
export const hadesFragment: CardDef = {
  ...
  flags: { 'corp-phase-12': req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
  ...
  abilities: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreMoving.move(state, side, target, 'deck'); }),
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `add ${(target.seen ? target.title : 'a card')} to the bottom of R&D`; })(); },
  }],
  events: [{ ...(coreDefHelpers.gainCreditsAbility(0)), 'change-in-game-state': { req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).corp?.discard?.length > 0; }), silent: true } }],
};
    ...
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      continue_ability(
        state, side,
        {
          ...
          msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `place ${utils.quantify(c, 'power counter')} on ${target.title}`; })(); },
          async: true,
          effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreProps.addCounter(state, side, eid, target, ':power', c, null); }),
        },
        card,
        null
      );
...
export const highRiskInvestment: CardDef = {
  ...
  abilities: [{
    ...
    label: 'gain credits',
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `gain ${(state as any).runner?.credit} [Credits]`,
    async: true,
    'keep-menu-open': ':while-agenda-tokens-left',
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainCredits(eid, (state as any).runner?.credit || 0); }),
  }],
};
  ...
  events: [{
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      continue_ability(
        state, side,
        {
          choices: { 'not-self': true, req: req(function*(s: State, sd: Side, eid2: EID, c: Card, t: any[]): Generator<any, any, any> {
            const t2 = t[0];
            return t2 && coreCard.canBeAdvanced(s, t2);
          }) },
          msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return `place ${utils.quantify(n, 'advancement counter')} on ${coreToString.cardStr(state, target)}`; },
          async: true,
...
export const hybridRelease: CardDef = {
  ...
  'on-score': {
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreInstalling.corpInstall(eid, target, null, { msgKeys: { installSource: card, displayOrigin: true } }); }),
  },
};
...
export const ikawahProject: CardDef = {
  ...
  'steal-cost-bonus': req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
...
export const improvedProteinSource: CardDef = {
  ...
  abilities: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainCredits(':runner', eid, 4); }),
  }],
  'on-score': {
    async: true,
    interactive: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return true; }),
...
export const improvedTracers: CardDef = {
  ...
  'static-abilities': [
    { type: ':ice-strength', req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return coreCard.hasSubtype(targets[0] || {}, 'Tracer'); }), value: 1 },
    { type: ':trace-base-strength', req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
...
export const jumon: CardDef = {
  ...
  events: [{
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return `place 2 advancement counters on ${coreToString.cardStr(state, target)}`; },
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreProps.addProp(':corp', eid, target, ':advance-counter', 2, { placed: true }); }),
  }],
};
    ...
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      continue_ability(
        state, side,
        {
          ...
          async: true,
          effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreMoving.trash(eid, target); }),
        },
        card,
        null
      );
...
function addAbiFn(): any {
  return {
    ...
    'waiting-prompt': true,
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `add ${target.title} from HQ to [their] score area`; })(); },
...
export const letThemDream: CardDef = {
  ...
  'agendapoints-runner': req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return 1; }),
};
...
function findAbi(zone: string): any {
  return {
    ...
    choices: zone === ':rd'
      ? req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
...
export const licenseAcquisition: CardDef = {
  ...
  'on-score': {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `install and rez ${target.title}, ignoring all costs`; })(); },
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreInstalling.corpInstall(eid, target, null, { 'install-state': ':rezzed-no-cost', msgKeys: { installSource: card, displayOrigin: true } }); }),
  },
};
...
export const lightningLaboratory: CardDef = {
  ...
  events: [{
    ...
    async: true,
    optional: {
      prompt: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `Remove 1 hosted agenda counter to rez up to 2 pieces of ice protecting ${coreServers.zoneToName(forms.context(state, card, targets)?.server || '')}, ignoring all costs?`,
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      'yes-ability': {
        ...
        effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          ...
          continue_ability(
            state, side,
            {
              prompt: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `Choose up to 2 pieces of ice protecting ${coreServers.zoneToName(currentServer)}`,
              'waiting-prompt': true,
  ...
  return {
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      continue_ability(
        state, side,
        {
          prompt: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `Choose ${utils.quantify(derezCount, 'piece')} of ice protecting ${coreServers.zoneToName([zone])} to derez`,
          'waiting-prompt': true,
          choices: { card: (c: Card) => coreCard.ice(c) && coreCard.rezzed(c) && (coreCard.getZone(c) || [])[1] === zone, max: derezCount, min: derezCount },
          msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `derez ${utils.enumerateStr(targets.map((t: Card) => coreToString.cardStr(state, t)))}`,
          async: true,
...
export const longevitySerum: CardDef = {
  ...
  'on-score': {
    ...
    choices: { max: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).corp?.hand?.length || 0; }), card: (c: Card) => coreCard.corp(c) && coreCard.inHand(c) },
    msg: { public: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `trash ${utils.quantify(targets.length, 'card')} from HQ`, corp: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `trash ${utils.quantify(targets.length, 'card')} from HQ (${utils.enumerateCards(targets, { sorted: true })})` },
    async: true,
    cancel: { msg: 'decline trashing any cards from HQ', async: true, effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreShuffling.shuffleIntoRDEffect(state, side, eid, card, 3); }) },
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      if (legalMoves.length > 0) {
        continue_ability(
          state, side,
          {
            ...
            choices: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return legalMoves; }),
            msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `move ${toMove.title} to ${target}`; })(); },
            effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
        ...
        continue_ability(
          state, side,
          { prompt: `You have no legal moves for ${toMove.title}`, msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `reveal that they have no legal moves for ${toMove.title}`, choices: ['OK'] },
          card,
          null
        );
      }
...
export const mandatorySeedReplacement: CardDef = {
  ...
  'on-score': {
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { msr(); }),
  },
};
...
export const medicalBreakthrough: CardDef = {
  ...
  'static-abilities': [{
    ...
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (targets[0] || {}).title === 'Medical Breakthrough'; }),
    value: -1,
  }],
};
...
export const metiesCityLuxuryLine: CardDef = {
  ...
  'steal-cost-bonus': req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return [corePayment.toC('click', 1)]; }),
  'on-score': { msg: 'gain [Click]', silent: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return true; }), effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainClicks(state, ':corp', 1); }) },
};
...
export const merger: CardDef = {
  title: 'Merger',
  'agendapoints-runner': req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return 3; }),
};
...
export const midnight3Arcology: CardDef = {
  ...
  'on-score': {
    async: true,
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => 'draw 3 cards and skip [their] discard step this turn',
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreEffects.registerLingeringEffect(card, { type: ':skip-discard', duration: ':end-of-turn', value: true }); coreDrawing.draw(':corp', eid, 3); }),
  },
};
...
export const napdContract: CardDef = {
  title: 'NAPD Contract',
  'steal-cost-bonus': req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return [corePayment.toC('credit', 4)]; }),
...
export const netQuarantine: CardDef = {
  ...
  events: [
    { event: 'successful-trace', async: true, effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
    ...
    { event: 'unsuccessful-trace', async: true, effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      const extra = Math.floor((targets[0]?.runnerSpent || 0) / 2);
      if (extra > 0) {
        coreSay.systemMsg(state, ':corp', `uses ${card.title} to gain ${extra} [Credits]`);
        coreGaining.gainCredits(state, side, eid, extra);
  ...
  events: [{
    ...
    optional: {
      ...
      'yes-ability': {
        ...
        msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `install a card from HQ${(coreCard.getCounters(coreCard.getCard(state, card), ':advancement') || 0) <= 5 ? ' and rez it, ignoring all costs' : ''}`,
        async: true,
        effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const target: any = (targets as any[])?.[0];
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      continue_ability(
        state, side,
        {
          ...
          'waiting-prompt': true,
          choices: { max: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).corp?.hand?.length || 0; }), card: (c: Card) => coreCard.corp(c) && coreCard.inHand(c) },
          msg: { public: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `shuffle ${utils.quantify(targets.length, 'card')} from HQ into R&D`, corp: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `shuffle ${utils.enumerateCards(targets, { sorted: true })} from HQ into R&D` },
