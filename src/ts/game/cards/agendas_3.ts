...
export const hadesFragment: CardDef = {
  ...
  flags: {
    "corp-phase-12": req(function* (
      ...
      return (
        ...
        coreFlags.isScored(state, ":corp", card)
      );
    }),
  },
  abilities: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          ...
          coreMoving.move(state, side, target, "deck");
        },
      ),
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        return ((): string => {
          const target: any = (targets as any[])?.[0];
          return `add ${target.seen ? target.title : "a card"} to the bottom of R&D`;
        })();
      },
    },
  ],
  events: [
    {
      ...coreDefHelpers.gainCreditsAbility(0),
  ...
  "on-score": {
    ...
    effect: req(function* (
      ...
      continue_ability(
        ...
        {
          choices: {
            card: (c: Card) => (coreCard.getCounters(c, ":power") || 0) > 0,
          },
          msg: (
            state: State,
            side: Side,
            ...
            return ((): string => {
              const target: any = (targets as any[])?.[0];
              return `place ${utils.quantify(c, "power counter")} on ${target.title}`;
            })();
          },
          async: true,
          effect: effect(
            (
              ...
              const target: any = (targets as any[])?.[0];
              coreProps.addCounter(state, side, eid, target, ":power", c, null);
            },
          ),
        },
        card,
  ...
  abilities: [
    {
      ...
      label: "gain credits",
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `gain ${(state as any).runner?.credit} [Credits]`,
      async: true,
      "keep-menu-open": ":while-agenda-tokens-left",
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreGaining.gainCredits(eid, (state as any).runner?.credit || 0);
        },
      ),
    },
  ],
  ...
  events: [
    {
      ...
      effect: effect(function* (
        ...
        const n =
          (coreCard.getCounters(cardObj, ":advancement") || 0) >= 6 ? 2 : 1;
        continue_ability(
          state,
          side,
          {
            ...
            msg: (
              ...
              const target: any = (targets as any[])?.[0];
              return `place ${utils.quantify(n, "advancement counter")} on ${coreToString.cardStr(state, target)}`;
            },
            async: true,
            effect: effect(
              coreProps.addProp(":corp", eid, target, ":advance-counter", n, {
                placed: true,
              }),
            ),
          },
  ...
  abilities: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreDamage.damage(eid, ":net", 1, { card: card });
        },
      ),
    },
  ],
...
export const hybridRelease: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = (targets as any[])?.[0];
        coreInstalling.corpInstall(eid, target, null, {
  ...
  "on-score": {
    ...
    effect: effect(function* (
            ...
            side,
            {
              optional: {
                ...
                "yes-ability": {
                  ...
                  async: true,
                  effect: effect(
                    coreBadPublicity.gainBadPublicity(":corp", eid, 1),
                  ),
                },
              },
            },
      ...
      coreSay.systemMsg(
        state,
        side,
        `uses ${card.title} to gain ${n} [Credits]`,
      );
  ...
  abilities: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreGaining.gainCredits(":runner", eid, 4);
        },
      ),
    },
  ],
  ...
  "move-zone": req(function* (
    ...
    if (coreCard.inScored(card) && (card as any)["scored-side"] === ":corp") {
      coreSay.systemMsg(
        state,
        side,
        `uses ${card.title} to increase the strength of Tracer ice by 1`,
      );
      coreSay.systemMsg(
        state,
        side,
        `uses ${card.title} to increase the base strength of all trace subroutines by 1`,
      );
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return coreCard.hasSubtype(targets[0] || {}, "Tracer");
      }),
      value: 1,
    },
    {
      ...
      req: req(function* (
        ...
        return (
          secondTargets &&
          (secondTargets as any)["source-type"] === ":subroutine"
        );
      }),
      value: 1,
    },
  ...
  events: [
    {
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = (targets as any[])?.[0];
        return `place 2 advancement counters on ${coreToString.cardStr(state, target)}`;
      },
      async: true,
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const target: any = (targets as any[])?.[0];
          coreProps.addProp(":corp", eid, target, ":advance-counter", 2, {
            placed: true,
          });
        },
      ),
...
export const kimberliteField: CardDef = {
  ...
  "on-score": {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const target: any = (targets as any[])?.[0];
      return `trash ${coreToString.cardStr(state, target)}`;
    },
    req: req(function* (
      state: State,
      side: Side,
...
export const kingmaking: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(function* (
      ...
      yield wait_for(
        state,
        [{ asyncResult: "result" }, coreDrawing.drawUpTo(state, side, card, 3)],
        [],
      );
...
function addAbiFn(): any {
  return {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      return ((): string => {
        const target: any = (targets as any[])?.[0];
        return `add ${target.title} from HQ to [their] score area`;
      })();
    },
    effect: effect(function* (
      state: State,
      ...
      coreInitializing.cardInit(state, ":corp", c, {
  ...
  prevention: [
    {
      ...
      ability: {
        ...
        req: req(function* (
          ...
          return corePrevention.preventable(
            forms.context(state, card, targets),
          );
        }),
        effect: effect(function* (
          state: State,
          ...
          yield wait_for(
            ...
            [
              { asyncResult: "result" },
              corePrevention.preventJackOut(state, side),
            ],
            [],
          );
      ...
      zone === ":rd"
        ? req(function* (
            ...
            return corePrompts.cancellable(
              (corp?.deck || [])
                .filter((c: Card) => coreCard.agenda(c))
                .sort((a: Card, b: Card) =>
        ...
        : {
            card: (c: Card) =>
              coreCard.agenda(c) &&
              (zone === ":hq" ? coreCard.inHand(c) : coreCard.inDiscard(c)),
          },
    effect: effect(function* (
      state: State,
      side: Side,
...
function moveToOne(c: Card, from: string): any {
  const andThen = (s: string) =>
    `${from === ":rd" ? ", shuffle R&D, and then " : " and "}${s}`;
      ...
      ability: {
        ...
        effect: effect(function* (
          ...
          coreRevealing.revealLoud(
            ...
            eid,
            card,
            { "and-then": andThen("add it to the bottom of R&D") },
            c,
          );
        }),
      },
...
export const licenseAcquisition: CardDef = {
  ...
  "on-score": {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.corp(c) &&
        (coreCard.asset(c) || coreCard.upgrade(c)) &&
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      return ((): string => {
        const target: any = (targets as any[])?.[0];
        return `install and rez ${target.title}, ignoring all costs`;
      })();
    },
    async: true,
    effect: effect(
  ...
  events: [
    {
      ...
      optional: {
        prompt: (
          ...
          `Remove 1 hosted agenda counter to rez up to 2 pieces of ice protecting ${coreServers.zoneToName(forms.context(state, card, targets)?.server || "")}, ignoring all costs?`,
        req: req(function* (
          state: State,
          side: Side,
          eid: EID,
            ...
            continue_ability(
              ...
              side,
              {
                prompt: (
                  ...
                  `Choose up to 2 pieces of ice protecting ${coreServers.zoneToName(currentServer)}`,
                "waiting-prompt": true,
                choices: {
                  card: (c: Card) =>
                    coreCard.ice(c) &&
                    !coreCard.rezzed(c) &&
                    (coreCard.getZone(c) || [])[1] === currentServer,
                  max: 2,
                },
                async: true,
                cancel: {
...
function iceDerez(zone: string): any {
  return {
    ...
    req: req(function* (
      ...
      const zoneMatches = (c: Card) => {
        const z = (c as any).zone as string[] | undefined;
        return z?.[0] === "servers" && z?.[1] === zone && z?.[2] === "ices";
      };
      return allActive.some(zoneMatches);
    }),
    ...
    effect: effect(function* (
      ...
      const allActive = coreBoard.allActiveInstalled(state, ":corp");
      const matchingIces = allActive.filter((c: Card) => {
        const z = (c as any).zone as string[] | undefined;
        return z?.[0] === "servers" && z?.[1] === zone && z?.[2] === "ices";
      });
      ...
      continue_ability(
        ...
        side,
        {
          prompt: (
            ...
            `Choose ${utils.quantify(derezCount, "piece")} of ice protecting ${coreServers.zoneToName([zone])} to derez`,
          "waiting-prompt": true,
          choices: {
            card: (c: Card) =>
              coreCard.ice(c) &&
              coreCard.rezzed(c) &&
              (coreCard.getZone(c) || [])[1] === zone,
            max: derezCount,
            min: derezCount,
          },
          msg: (
            ...
            `derez ${utils.enumerateStr(targets.map((t: Card) => coreToString.cardStr(state, t)))}`,
          async: true,
  ...
  "on-score": {
    ...
    msg: {
      public: (
        ...
        card: Card,
        targets: any[],
      ) => `trash ${utils.quantify(targets.length, "card")} from HQ`,
      corp: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `trash ${utils.quantify(targets.length, "card")} from HQ (${utils.enumerateCards(targets, { sorted: true })})`,
    },
    async: true,
    cancel: {
  ...
  abilities: [
    {
      ...
      effect: effect(function* (
        ...
        const notSameZone = (zones: string[]) =>
          zones.filter((z: string) => z !== coreServers.zoneToName(zone));
        const legalZonesFn =
          toMove && (coreCard.getCardDef(toMove) as any)?.legalZones
            ? (zones: string[]) =>
                (coreCard.getCardDef(toMove) as any).legalZones(
                  state,
                  side,
                  eid,
                  card,
        ...
        const regionRestriction = (zones: string[]) => {
          if (coreCard.hasSubtype(toMove, "Region")) {
            return zones.filter((z: string) => {
              ...
              const content = (state as any).corp?.servers?.[serverZ[1] as string]?.content;
              return !content?.some((c: Card) =>
                coreCard.hasSubtype(c, "Region"),
              );
            });
          }
          return zones;
        ...
        const legalMoves = regionRestriction(notSameZone(serverList)).filter(
          Boolean,
        );
        if (legalMoves.length > 0) {
          continue_ability(
            ...
            side,
            {
              ...
              msg: (
                ...
                return ((): string => {
                  const target: any = (targets as any[])?.[0];
                  return `move ${toMove.title} to ${target}`;
                })();
              },
              effect: effect(function* (
                state: State,
                ...
                const c = coreMoving.move(state, side, toMove, [
                  ...coreServers.serverToZone(state, target),
                  ":content",
                ]);
          ...
          continue_ability(
            ...
            side,
            {
              prompt: `You have no legal moves for ${toMove.title}`,
              msg: (
                state: State,
                side: Side,
                eid: EID,
                card: Card,
                targets: any[],
              ) => `reveal that they have no legal moves for ${toMove.title}`,
              choices: ["OK"],
            },
            card,
            null,
        ...
        function (state: State, side: Side, card: Card) {
          ((): boolean => {
            coreToasts.toast(
              state,
              ":corp",
              "Cannot score cards this turn due to Luminal Transubstantiation.",
              "warning",
            );
            return false;
          })();
    ...
    effect: effect(function* (
      ...
      if (targets.length === 2) {
        ...
        coreSay.systemMsg(
          state,
          side,
          `swaps the position of ${coreToString.cardStr(state, targets[0])} and ${coreToString.cardStr(state, targets[1])}`,
        );
  ...
  "move-zone": req(function* (
    ...
    if (coreCard.inScored(card) && (card as any)["scored-side"] === ":corp") {
      coreSay.systemMsg(
        state,
        side,
        `uses ${card.title} to gain 1 addition [Click] per turn`,
      );
...
export const medicalBreakthrough: CardDef = {
  ...
  flags: {
    "has-events-when-stolen": req(function* (
      state: State,
      side: Side,
      eid: EID,
      card: Card,
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return (targets[0] || {}).title === "Medical Breakthrough";
      }),
      value: -1,
    },
  ],
...
export const metiesCityLuxuryLine: CardDef = {
  ...
  "steal-cost-bonus": req(function* (
    ...
    return [corePayment.toC("click", 1)];
  }),
  "on-score": {
    msg: "gain [Click]",
    silent: req(function* (
...
export const megaprixQualifier: CardDef = {
  ...
  "on-score": {
    ...
    req: req(function* (
      ...
      const scored = [
        ...((state as any).corp?.scored || []),
        ...((state as any).runner?.scored || []),
      ];
      return (
        scored.filter((c: Card) => c.title === "Megaprix Qualifier").length > 1
      );
    }),
    async: true,
    effect: effect(function* (
    ...
    effect: req(function* (
      ...
      if (t === "Gain 7 [Credits]") {
        coreSay.systemMsg(
          state,
          side,
          `uses ${card.title} to gain 7 [Credits]`,
        );
        ...
        coreSay.systemMsg(
          state,
          side,
          `uses ${card.title} to do 7 meat damage`,
        );
...
export const midnight3Arcology: CardDef = {
  ...
  "on-score": {
    async: true,
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      "draw 3 cards and skip [their] discard step this turn",
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
...
export const napdContract: CardDef = {
  ...
  "steal-cost-bonus": req(function* (
    ...
    return [corePayment.toC("credit", 4)];
  }),
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return (
          coreEvents.turnEvents(state, side, ":initialize-trace").length === 1
        );
      }),
      value: 0,
    },
      ...
      effect: effect(function* (
        ...
        if (extra > 0) {
          coreSay.systemMsg(
            state,
            ":corp",
            `uses ${card.title} to gain ${extra} [Credits]`,
          );
      ...
      optional: {
        ...
        "yes-ability": {
          ...
          msg: (
            ...
            `install a card from HQ${(coreCard.getCounters(coreCard.getCard(state, card), ":advancement") || 0) <= 5 ? " and rez it, ignoring all costs" : ""}`,
          async: true,
          effect: effect(function* (
            state: State,
            side: Side,
            ...
            yield wait_for(
              ...
              [
                { asyncResult: "result" },
                coreInstalling.corpInstall(eid, target, "New remote", {
...
export const nextBigThing: CardDef = {
  ...
  flags: {
    "has-abilities-when-stolen": req(function* (
      state: State,
      side: Side,
      eid: EID,
      card: Card,
  ...
  abilities: [
    {
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          state,
          [{ asyncResult: "result" }, coreDrawing.draw(state, side, eid, 4)],
          [],
        );
        continue_ability(
          state,
          side,
          {
            ...
            msg: {
              public: (
                ...
                `shuffle ${utils.quantify(targets.length, "card")} from HQ into R&D`,
              corp: (
                state: State,
                side: Side,
                eid: EID,
                ...
                `shuffle ${utils.enumerateCards(targets, { sorted: true })} from HQ into R&D`,
            },
            ...
            effect: effect(function* (
              ...
              for (const t of targets) {
                yield wait_for(
                  ...
                  [
                    { asyncResult: "result" },
                    coreMoving.move(state, side, t, "deck"),
                  ],
                  [],
                );
              }
...
export const nextWave2: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(function* (
      ...
      const allInstalled = coreBoard.allInstalled(state, ":corp");
      const nextIces = allInstalled.filter(
        (c: Card) =>
          coreCard.rezzed(c) &&
          coreCard.ice(c) &&
          coreCard.hasSubtype(c, "NEXT"),
      );
