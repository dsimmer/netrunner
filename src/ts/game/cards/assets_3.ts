  ...
  abilities: [
    {
      action: true,
      label:
        "Add this asset to your score area as an agenda worth 1 agenda point",
  ...
  abilities: [
    {
      ...
      effect: req(function* (
        ...
        const hasLocation = (
          coreBoard.allInstalled(state, ":runner") || []
        ).some((c: Card) => coreCard.hasSubtype(c, "Location"));
        if (hasLocation) {
          yield wait_for(
            state,
            [
              ...
              coreEngine.resolveAbility(
                ...
                side,
                {
                  ...
                  choices: {
                    min: 1,
                    card: (c: Card) => coreCard.hasSubtype(c, "Location"),
                  },
                  async: true,
                  effect: req(function* (
                    s: State,
  ...
  abilities: [
    {
      ...
      effect: effect(function* (
        ...
        coreBadPublicity.loseBadPublicity(
          state,
          side,
...
export const falseFlag: CardDef = {
  ...
  "on-access": {
    ...
    effect: effect(function* (
      ...
      const n = Math.floor(
        coreCard.getCounters(coreCard.getCard(state, card), ":advancement") / 2,
      );
      yield wait_for(
        state,
  ...
  abilities: [
    {
      ...
      label:
        "Add this asset to your score area as an agenda worth 3 agenda points",
...
export const federalFundraising: CardDef = (() => {
      ...
      "no-ability": {
        effect: effect(function* (
          ...
          coreSay.systemMsg(
            state,
            side,
            `declines to use ${(card as any)?.title} to draw 1 card`,
          );
        }),
      },
    },
  ...
  const ability: any = {
    ...
    req: req(function* (state: State): Generator<any, any, any> {
      return (
        !!(state as any).corpPhase12 && (state as any).corp?.deck?.length > 0
      );
    }),
    skippable: true,
    interactive: req(function* (
  ...
  "static-abilities": [
    {
      ...
      value: req(function* (state: State): Generator<any, any, any> {
        return Object.keys((state as any).corp?.servers?.remote || {}).map(
          (k: string) => k,
        );
      }),
    },
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const ctx = (targets as any)[0] || {};
        return (
          ctx.targetServer === ":archives" &&
          coreEvents.firstEvent(
            state,
            ":runner",
            ":run",
            (t: any[]) => (t[0] || {}).targetServer === ":archives",
          ) &&
...
export const fullImmersionRecStudio: CardDef = {
  ...
  "can-host": req(function* (
    ...
    return (
      (coreCard.asset(targets[0]) || coreCard.agenda(targets[0])) &&
      ((card as any).hosted?.length || 0) < 2
    );
  }),
  "trash-cost-bonus": req(function* (
    state: State,
    side: Side,
    eid: EID,
    card: Card,
  ...
  abilities: [
    ...
    {
      label:
        "Install a previously-installed asset or agenda on this asset (fixes only)",
      req: req(function* (
        state: State,
        side: Side,
        eid: EID,
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          ctx.runnerCredits != null &&
          ctx.corpCredits !== ctx.runnerCredits
        );
      }),
  ...
  const searchForOperation: any = {
    ...
    msg: msg(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        return t === "Done"
          ? "shuffle R&D"
          : `add ${coreCard.getTitle(t as Card)} from R&D to HQ`;
      },
    ),
    choices: req(function* (state: State): Generator<any, any, any> {
...
export const heartsAndMinds: CardDef = (() => {
  ...
  const ability: any = {
    ...
    label:
      "Move 1 hosted advancement counter to another card you can advance (start of turn)",
    skippable: true,
    once: ":per-turn",
    "waiting-prompt": true,
    prompt:
      "Choose an installed card to move 1 hosted advancement counter from",
    choices: {
      card: (c: Card) =>
        coreCard.installed(c) && coreCard.getCounters(c, ":advancement") > 0,
    },
    async: true,
    effect: req(function* (
      state: State,
              ...
              choices: {
                req: req(function* (
                  ...
                  return (
                    ...
                    coreProps.canBeAdvanced(state, t) &&
...
export const clydeVanRite: CardDef = (() => {
  const ability: any = {
    ...
    label:
      "make the Runner pay 1 [Credits] or trash the top card of the stack (start of turn)",
    msg: msg(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const valid = (evs: any[]) =>
          evs.some(
            (e: any) => coreCard.corp(e.card) && coreCard.installed(e.card),
          );
        return (
          valid(targets as any[]) &&
