...
export const corporateWar: CardDef = {
  title: 'Corporate War',
  'on-score': {
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => ((state as any).corp?.credit > 6 ? 'gain 7 [Credits]' : 'lose all credits'),
    interactive: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return true; }),
    async: true,
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      const corp = (state as any).corp;
...
export const cyberdexSandbox: CardDef = {
  ...
  'on-score': {
    optional: {
      ...
      'yes-ability': { msg: 'purge virus counters', async: true, effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { corePurging.purge(eid); }) },
    },
  },
  events: [{
    event: 'purge',
...
export const elivagarBifurcation: CardDef = {
  ...
  'on-score': {
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreRezzing.derez(state, side, eid, target); }),
  },
};
...
export const efficiencyCommittee: CardDef = {
  ...
  abilities: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainClicks(2); coreFlags.registerTurnFlag!(card, ':can-advance', () => false); }),
...
export const electiveUpgrade: CardDef = {
  ...
  abilities: [{
    ...
    cost: [corePayment.toC('click', 1), corePayment.toC('agenda', 1)],
    'once': ':per-turn',
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainClicks(2); }),
    msg: 'gain [Click][Click]',
  }],
};
  ...
  events: [{
    ...
    optional: {
      ...
      'yes-ability': {
        ...
        async: true,
        msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `reveal ${target.title} from R&D, shuffle R&D, and place it ontop`; })(); },
        ...
        cancel: {
          msg: 'shuffle R&D',
          cost: [corePayment.toC('agenda', 1)],
          effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreShuffling.shuffle(state, side, ':deck'); }),
        },
      },
    },
  }],
...
export const escalateVitriol: CardDef = {
  ...
  abilities: [{
    ...
    'change-in-game-state': { req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return forms.tagged(state); }) },
    cost: [corePayment.toC('click', 1)],
    'once': ':per-turn',
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `gain ${utils.countTags(state)} [Credits]`,
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainCredits(eid, utils.countTags(state)); }),
  }],
};
...
export const falseLead: CardDef = {
  ...
  events: [{
    ...
    optional: {
      ...
      'waiting-prompt': true,
      'yes-ability': {
        'change-in-game-state': { req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).runner?.click >= 2; }) },
        label: 'runner loses [Click][Click]',
        msg: 'force the Runner to lose [Click][Click]',
        cost: [corePayment.toC('forfeit-self')],
        effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.loseClicks(':runner', 2); }),
      },
    },
  }],
  abilities: [{
...
export const fetalAI: CardDef = {
  title: 'Fetal AI',
  flags: { 'rd-reveal': req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return true; }) },
  'on-access': { ...(coreDefHelpers.doNetDamage(2)), req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return !coreCard.inDiscard(card); }) },
  'steal-cost-bonus': req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return [corePayment.toC('credit', 2)]; }),
};
...
export const firmwareUpdates: CardDef = {
  ...
  abilities: [{
    ...
    'once': ':per-turn',
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreProps.addProp(eid, target, ':advance-counter', 1, { placed: true }); }),
  }],
};
...
export const flowerSermon: CardDef = {
  ...
  abilities: [{
    ...
    'once': ':per-turn',
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `reveal ${(state as any).corp?.deck?.[0]?.title} and draw 2 cards`,
    async: true,
    'waiting-prompt': true,
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const target: any = (targets as any[])?.[0];
      const corp = (state as any).corp;
      ...
      continue_ability(
        state, side,
        {
          ...
          msg: { public: 'add 1 card in HQ to the top of R&D', corp: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return `add facedown ${target.title} to the top of R&D`; }},
...
export const fujiiAssetRetrieval: CardDef = {
  ...
  stolen: {
    ...
    interactive: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return true; }),
    msg: 'do 2 net damage',
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDamage.damage(eid, ':net', 2, { card: card }); }),
  },
  'on-score': {
    async: true,
    interactive: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return true; }),
...
export const geneticResequencing: CardDef = {
  ...
  'on-score': {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `place 1 agenda counter on ${target.title}`; })(); },
    async: true,
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const target: any = (targets as any[])?.[0];
  ...
  abilities: [
    {
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreHosting.host(card, target, { facedown: true }); }),
    },
    {
      action: true,
      ...
      choices: { all: true, req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const target: any = (targets as any[])?.[0];
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreMoving.move(state, side, target, 'hand'); }),
    },
  ],
};
