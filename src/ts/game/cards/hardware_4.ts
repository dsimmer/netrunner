  ...
  prevention: [
    {
      ...
      ability: {
        ...
        req: req(function* (
          ...
          return (
            (ctx.type === "net" || ctx.type === ":net") && preventableFn(ctx)
          );
        }),
        effect: req(function* (
          state: State,
  ...
  abilities: [
    {
      ...
      effect: req(function* (
        ...
        const cardObj = getCardFn(state, card);
        coreUpdate.update(state, ":runner", {
          ...cardObj,
          special: { ...cardObj.special, "flame-out-trigger": true },
        });
        (corePayment as any).spendCredits?.(
          state,
          side,
          eid,
          card,
    ...
    },
    {
      ...
      effect: req(function* (
        ...
        systemMsg(
          state,
          ":runner",
          `takes ${credits} hosted [Credits] from Flame-out`,
        );
        const cardObj = getCardFn(state, card);
        coreUpdate.update(state, ":runner", {
          ...cardObj,
  ...
  events: [
    {
      ...
      effect: req(function* (
        ...
        updateFn(state, side, {
          ...cardObj,
          special: { ...cardObj.special, "flame-out-trigger": false },
        });
    ...
    "pay-credits": {
      ...
      custom: req(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            addCounterFn(state, side, cardObj, "credit", -1, {
  ...
  abilities: [
    {
      ...
      "change-in-game-state": {
        req: req(function* (
          ...
          return !!(runFn(state) || getCurrentEncounterFn(state));
        }),
      },
      req: req(function* (
        state: State,
    ...
    },
    {
      ...
      "change-in-game-state": {
        req: req(function* (
          ...
          return countRealTagsFn(state) > 0;
        }),
      },
      req: req(function* (
        state: State,
...
export const forger: CardDef = {
  title: "Forger",
  events: [
    (coreChooseOneMod as any).chooseOneHelper(
      {
        event: "tag-interrupt",
        req: req(function* (
          state: State,
          ...
          return !!(
            getPreventFn(state)?.tag?.remaining > 0 &&
  ...
  abilities: [
    {
      ...setAutoresolveFn(
        "auto-fire",
        "Friday Chip placing virus counters on itself",
      ),
    },
  ],
  ...
  events: [
    {
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        return ((): string => {
          const target: any = (targets as any[])?.[0];
          return `move 1 virus counter to ${target.title}`;
        })();
      },
      skippable: true,
      req: req(function* (
        ...
        return (
          getCounters(card, "virus") > 0 && countVirusProgramsFn(state) > 0
        );
      }),
      ...
      effect: req(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            addCounterFn(state, ":runner", card, "virus", -1, {
              "suppress-checkpoint": true,
            }),
          ],
          [],
            ...
            "yes-ability": {
              ...
              effect: effect(
                (
                  ...
                  systemMsg(
                    ":runner",
                    `uses ${card.title} to place 1 virus counter on itself`,
                  );
        ...
        const multAb = {
          ...
          effect: effect(
            (
              ...
              systemMsg(
                ":runner",
                `uses ${card.title} to place ${quantify(target, "virus counter")} on itself`,
              );
  ...
  abilities: [
    {
      ...
      effect: req(function* (
        ...
        const setAsideCards = getSetAsideFn(state, side, eid).sort(
          (a: any, b: any) => (a.title || "").localeCompare(b.title || ""),
        );
        systemMsg(
          state,
          side,
          `${(eid as any).latestPaymentStr || "The player"} to use ${card.title} to set aside ${enumerateCards(setAsideCards)} from the top of the stack`,
        );
        yield wait_for(
          state,
          [
        ...
        const installFn = (setAsideCards: Card[]) => ({
          ...
          choices: req(function* (
            ...
            const validCards = setAsideCards.filter(
              (c: Card) =>
                (programFn(c) ||
                  (resourceFn(c) && hasSubtypeFn(c, "Virtual"))) &&
                runnerCanPayAndInstallFn(
                  state,
                  side,
          ...
          effect: req(function* (
            ...
            if (target === "Done") {
              continue_ability(
                state,
                side,
                shuffleNextFn(setAsideCards, null, null),
                card,
                null,
              );
              return;
            ...
            continue_ability(
              state,
              side,
              shuffleNextFn(setAsideCards2, null, null),
              card,
              null,
            );
          }),
        ...
        const shuffleNextFn = (
          ...
          prompt: (
            ...
            const finished =
              (toShuffle?.length ?? 0) >= 3 || setAsideCards.length === 0;
            if (finished) {
            ...
            if (finished) {
              if (target === "Done") {
                continue_ability(
                  state,
                  side,
                  shuffleEndFn(setAsideCards, toShuffle || []),
                  card,
                  null,
                );
                ...
                continue_ability(
                  state,
                  side,
                  shuffleNextFn([...(setAsideCards || [])], null, [
                    ...(toShuffle || []),
                  ]),
                  card,
                  null,
              ...
              continue_ability(
                state,
                side,
                shuffleNextFn(newSetAside, target, newToShuffle),
                card,
                null,
              );
            }
        ...
        const shuffleEndFn = (removeFromGame: Card[], shuffleBack: Card[]) => ({
          msg: (
            ...
            `shuffle ${enumerateCards(shuffleBack, ":sorted")} into the stack and remove ${enumerateCards(removeFromGame, ":sorted")} from the game`,
          effect: req(function* (
            state: State,
            side: Side,
            eid: EID,
...
export const gamedragonPro: CardDef = {
  ...
  "on-install": {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      return ((): string => {
        const target: any = (targets as any[])?.[0];
        return `host itself on ${target.title}`;
      })();
    },
  },
  events: [
  ...
  abilities: [
    {
      ...
      effect: req(function* (
        ...
        const newEffects = effects.reduce((acc: any[], e: any) => {
          if (
            sameCard(host, e.card) &&
            e.type === ":breaker-strength" &&
            e["original-duration"]
          ) {
            acc.push({
              ...e,
              duration: e["original-duration"],
  ...
  events: [
    {
      ...
      effect: req(function* (
        ...
        const lastPump = {
          ...
          "original-duration": effects[effects.length - 1]?.duration,
        };
  ...
  abilities: [
    {
      ...
      effect: req(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            exposeFn(state, side, coreEid.makeEID(state, eid), [
              forms.currentIce?.(state),
            ]),
          ],
          [],
        );
...
export const hermes: CardDef = {
  ...
  let: {
    ability: {
      ...
      "waiting-prompt": true,
      choices: {
        card: (c: Card) => !faceupFn(c) && installedFn(c) && corpFn(c),
        all: true,
      },
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = (targets as any[])?.[0];
        return `add ${cardStr(state, target)} to HQ`;
      },
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const target: any = (targets as any[])?.[0];
          moveFn(":corp", target, ":hand");
        },
      ),
    },
  },
    ...
    },
    {
      ...
      optional: {
        req: req(function* (
          ...
          return (
            targetServerFn(forms.context(state, card, targets)) === ":archives"
          );
        }),
        prompt: (_state: State, _side: Side, _eid: EID, card: Card) =>
          `Trash ${card.title} to force the Corp to lose 3 [Credits]?`,
        "yes-ability": {
          async: true,
      ...
      optional: {
        req: req(function* (
          ...
          const pred = (c: any) =>
            ...
            c.outermost &&
            c["during-run"] &&
            c["on-attacked-server"];
          return (
            pred(ctx) &&
            getCardFn(state, ctx.ice) &&
            firstEventFn(state, side, "subroutines-broken", (t: any[]) => {
              const c = t[0];
              return c && pred(c);
            })
          );
        ...
        prompt: (
          ...
          `Remove this hardware from the game to trash ${(forms.context(state, card, targets) as any)?.ice?.title || "the ice"}?`,
        "yes-ability": {
          async: true,
          ...
          msg: (
            ...
            `trash ${cardStr(state, (forms.context(state, card, targets) as any)?.ice)}`,
          effect: effect(
            (
              state: State,
              side: Side,
              ...
              trashFn(eid, (forms.context(state, card, targets) as any)?.ice, {
                causeCard: card,
              });
            },
          ),
  ...
  events: [
            ...
            optional: {
              ...
              "yes-ability": {
                ...
                effect: req(function* (
                  ...
                  const targetCard = discard.find(
                    (c: Card) => c["printed-title"] === card["printed-title"],
                  );
                  if (targetCard) {
                    runnerInstallFn(
                      state,
    ...
    },
    {
      ...
      effect: req(function* (
        ...
        const matchingCount = scored.filter(
          (c: Card) => c["printed-title"] === card["printed-title"],
        ).length;
        if (matchingCount === 3) {
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const companionPred = (c: Card) =>
          !facedownFn(c) && hasSubtypeFn(c, "Companion");
        const validCtx = (targets: any[]) =>
          targets.some((t: any) => {
            const c = t.card;
  ...
  events: [
    {
      ...
      optional: {
        req: req(function* (
          ...
          return (
            firstEventFn(state, ":runner", "successful-run") &&
        ...
        "yes-ability": {
          prompt:
            "Choose an installed virus program to place 1 virus counter on",
          choices: {
            card: (c: Card) =>
              installedFn(c) && hasSubtypeFn(c, "Virus") && programFn(c),
          },
          msg: (
            state: State,
            side: Side,
            ...
            return ((): string => {
              const target: any = (targets as any[])?.[0];
              return `place 1 virus counter on ${target.title}`;
            })();
          },
          async: true,
          effect: effect(
            (
              ...
              const target: any = (targets as any[])?.[0];
              addCounterFn(eid, target, "virus", 1);
            },
          ),
        },
      },
