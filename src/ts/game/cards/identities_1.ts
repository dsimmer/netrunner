  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          coreEvents.firstEvent(state, "corp", "corp-install") &&
                ...
                effect: effect(function* (
                  ...
                  if (
                    ...
                    coreToasts.toast(
                      state,
                      "corp",
                      "Cannot afford to pay 1 [Credit] to block card exposure",
                      "info",
                    );
                      ...
                      optional: {
                        ...
                        noAbility: {
                          ...
                          effect: effect(function* (
                            ...
                            coreExpose.expose(state, "runner", eid, [
                              installedCard,
                            ]);
                          }),
                        },
                        yesAbility: {
                          ...
                          effect: effect(function* (
                            ...
                            const result: any = (state as any)
                              .__lastAsyncResult;
                            coreSay.systemMsg(
                              state,
                              "corp",
                              `${result?.msg || ""} to prevent exposing ${coreToString.cardStr(state, installedCard) || ""}`,
                            );
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          isRem &&
          coreEvents.firstEvent(state, side, "corp-install", (entry: any) => {
      ...
      effect: effect(function* (
        ...
        const context = (targets as any)?.[0]?.context || {};
        const originalServer = coreServers.zoneToName(
          (coreCard.getZone(context.card) as any[])?.[1],
        );
        continue_ability(
          state,
          side,
          {
            prompt:
              "Choose a card to install in or protecting another remote server",
            waitingPrompt: true,
            choices: {
              card: (c: Card) =>
                coreCard.corp(c) &&
                ...
                coreCard.inHand(c) &&
                !(
                  (coreServers as any).installableServers?.(state, c) || []
                ).every?.((s: string) => ["HQ", "R&D", "Archives"].includes(s)),
            },
            async: true,
            effect: effect(function* (
              state: State,
              ...
              continue_ability(
                ...
                side,
                {
                  ...
                  waitingPrompt: true,
                  choices: (
                    ((coreBoard as any).getRemoteNames?.(state) ||
                      []) as string[]
                  )
                    .filter((s: string) => s !== originalServer)
                    .concat(["New remote"]),
                  async: true,
                  effect: effect(function* (
                    state: State,
  ...
  staticAbilities: [
    {
      ...
      req: req(function* (
        ...
        if (!encounter || !currentIce || !coreCard.rezzed(currentIce))
          return false;
        const server = coreBoard.cardToServer(state, currentIce);
        const serverIce = (server as any)?.ices || [];
        return (
          serverIce.length > 0 &&
          coreCard.sameCard(currentIce, serverIce[serverIce.length - 1])
        );
      }),
      value: 1,
    },
        ...
        continue_ability(
          ...
          side,
          {
            ...
            choices: {
              ...
              all: true,
              card: (c: Card) =>
                coreCard.runner(c) && (c.zone || []).includes?.("play-area"),
            },
            async: true,
            effect: effect(function* (
              state: State,
...
export const card_AgInfusion_NewMiraclesForANewWorld: CardDef = {
  ...
  abilities: [
    {
      label:
        "Trash a piece of ice to choose another server- the runner is now running that server",
      once: "per-turn",
      async: true,
      req: req(function* (
        state: State,
        ...
        const currentIce = coreIce.getCurrentIce?.(state);
        return (
          run && run.phase === "approach-ice" && !coreCard.rezzed(currentIce)
        );
      }),
      prompt:
        "Choose another server and redirect the run to its outermost position",
      choices: req(function* (
        ...
        const run = (state as any).run;
        const currentServer =
          (coreServers as any).centralToName?.(run?.server) || run?.server?.[0];
        const allServers: string[] =
          (coreServers as any).servers?.(state) || [];
      ...
      effect: effect(function* (
        ...
        const iceCount = (
          ((state as any).corp as any)?.servers?.[target]?.ices || []
        ).length;
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          ts === "archives" &&
          coreEvents.firstSuccessfulRunOnServer(state, ["archives"])
        );
      }),
      changeInGameState: {
        silent: true,
        ...
        continue_ability(
          ...
          side,
          {
            ...
            msg: msg(function* (
              ...
              const idx = ((state as any).corp?.deck || [])
                .slice(0, 3)
                .findIndex((c: Card) => coreCard.sameCard(c, targets[0]));
              const pos =
                idx === 0
                  ? "top "
                  : idx === 1
              ...
              yield wait_for(
                ...
                [
                  ...
                  coreMoving.trash(
                    ...
                    coreEid.makeEid(state, eid),
                    targets[0],
                    { causeCard: card, suppressCheckpoint: toDraw > 0 },
                  ),
                ],
                [],
              );
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          (targets as any)?.[0]?.["from-side"] === "corp" ||
          (targets as any)?.[0]?.fromSide === "corp"
        );
      }),
    },
    {
      ...
      req: req(function* (
        ...
        return (targets || []).some(
          (ctx: any) =>
            ...
            coreCard.inHand(ctx?.card) &&
            (ctx?.cause || ctx?.["cause-card"] || ctx?.causeCard),
        );
      }),
    },
...
const card_Apex_InvasivePredatorAbility: any = {
  ...
  choices: {
    ...
    req: req(function* (
      ...
      return (
        coreCard.runner(targets[0]) && coreCard.inHandStar?.(state, targets[0])
      );
    }),
  },
  req: req(function* (
...
export const card_Apex_InvasivePredator: CardDef = {
  ...
  events: [
    { ...card_Apex_InvasivePredatorAbility, event: "runner-turn-begins" },
  ],
  flags: {
    "runner-phase-12": req(function* (): Generator<any, any, any> {
      return true;
    }),
  },
  ...
  events: [
    {
      ...
      msg: msg(function* (
        ...
        const t = targets?.[0] || "";
        const decap =
          typeof t === "string" ? t.charAt(0).toLowerCase() + t.slice(1) : "";
      ...
      effect: effect(function* (
        ...
        if (targets[0] === "Take 1 tag") {
          ...
          coreDamage.damage(state, "runner", eid, "meat", 2, {
            unboostable: true,
            card,
          });
        }
  ...
  abilities: [
    {
      req: req(function* (
        ...
        return (
          run &&
          !(card as any)?.special?.["per-turn-used"] &&
          !coreInstalling.installLocked?.(state, side)
        );
      }),
      async: true,
      ...
      choices: req(function* (
        ...
        const inHand =
          (coreDefHelpers as any).allCardsInHandStar?.(state, "runner") || [];
        return inHand.filter(
          (c: Card) =>
            coreCard.program(c) &&
            (coreInstalling as any).runnerCanPayAndInstall?.(
              state,
              side,
          ...
          coreEngine.registerEvents(state, side, card, [
            {
              ...
              async: true,
              msg: msg(function* (): Generator<any, any, any> {
                return `trash ${installedCard?.title || ""}`;
              }),
              effect: effect(function* (
                state: State,
                side: Side,
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          context.side === "runner" &&
          (context.payment || []).some(
            (p: any) =>
              p?.["paid/type"] === "trash-can" ||
              p?.["paid/type"] === ":trash-can",
          )
        );
      }),
          ...
          side,
          {
            ...
            choices: {
              card: (c: Card) =>
                ...
                coreCard.corp(c) &&
                coreCard.corpInstallableType(c) &&
                (isRemoteZ || !coreCard.isType(c, "Asset")) &&
            ...
            effect: effect(function* (
              ...
              coreInstalling.corpInstall(
                ...
                eid,
                targets[0],
                coreServers.zoneToName(parentZone),
                {
                  msgKeys: { installSource: card, displayOrigin: true },
                },
              );
...
function azType(card: Card): boolean {
  return (
    coreCard.hardware(card) ||
    (coreCard.resource(card) &&
      coreCard.hasAnySubtype(card, ["Job", "Connection"]))
  );
}
  ...
  events: [
    {
      ...
      silent: req(function* (): Generator<any, any, any> {
        return true;
      }),
      msg: msg(function* (
        state: State,
  ...
  events: [
    ...
    },
    {
      ...
      req: req(function* (
        ...
        return !cardsInSlot.some(
          (c: any) =>
            (coreCard.asset(c) || coreCard.agenda(c)) &&
            (coreCard.rezzed(c) || c.faceup),
        );
      }),
      async: true,
      effect: effect(function* (
        ...
        const tcard: Card =
          (targets as any)?.[0]?.card || (targets as any)?.[0]?.context?.card;
  ...
  events: [
    ...
    },
    {
      ...
      req: req(function* (
        ...
        if (
          !target ||
          !coreCard.isType(context.card, target) ||
          context.facedown
        )
          return false;
        return coreEvents.firstEvent(
          ...
          "runner",
          "runner-install",
          (entry: any) => coreCard.isType(entry?.[0]?.card, target),
        );
      }),
      async: true,
      effect: effect(function* (
    ...
    },
    {
      ...
      req: req(function* (
        ...
        return (
          coreEvents.firstEvent(state, "runner", "play-event") &&
          coreCard.isType(context.card, target)
        );
      }),
      async: true,
      ...
      choices: {
        req: req(function* (
          ...
          return (
            ...
            (coreCard.resource(t) || coreCard.hardware(t)) &&
            (coreInstalling as any).runnerCanPayAndInstall?.(
              state,
              side,
              eid,
...
export const card_BlueSun_PoweringTheFuture: CardDef = {
  ...
  events: [
    {
      ...card_BlueSun_PoweringTheFutureAbility,
      event: "corp-turn-begins",
      automatic: "last",
      skippable: true,
    },
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const isRd = Array.isArray(server)
          ? server.length === 1 && server[0] === "rd"
          : server === "rd";
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const context = (targets as any)?.[0]?.context || {};
        return (
          context["damage-type"] === "brain" || context.damageType === "brain"
        );
      }),
      msg: msg(function* (
        state: State,
      ...
      effect: effect(function* (
        ...
        for (const c of trashed) {
          const candidates = allCards.filter(
            (x: Card) =>
              !coreCard.inRfg(x) && coreCard.runner(x) && x.title === c.title,
          );
...
export const card_ChronosProtocol_SelectiveMindMapping: CardDef = {
  ...
  req: req(function* (
    ...
    return !evs.some(
      (e: any) => (e?.[0]?.["damage-type"] || e?.[0]?.damageType) === "net",
    );
  }),
  effect: effect(function* (
    state: State,
      ...
      optional: {
        req: req(function* (
          ...
          return (
            ...
            amount > 0 &&
            !coreEvents
              .turnEvents(state, "runner", "damage")
              ?.some(
                (e: any) =>
                  (e?.[0]?.["damage-type"] || e?.[0]?.damageType) === "net",
              ) &&
            hand.length > 0
          );
        }),
        ...
        yesAbility: {
          ...
          choices: {
            card: (c: Card) => coreCard.runner(c) && coreCard.inHand(c),
          },
          msg: msg(function* (
            state: State,
            side: Side,
...
function earthStationFlip(state: State, side: Side, card: Card): void {
  if ((card as any).flipped) {
    coreSay.systemMsg(
      state,
      "corp",
      "flipped [pronoun] identity to Earth Station: SEA Headquarters",
    );
    coreUpdate.update!(state, side, {
      ...card,
      flipped: false,
  ...
  events: [
    ...
    },
    {
      ...
      req: req(function* (
        ...
        const context = (targets as any)?.[0]?.context || {};
        return (
          coreServers.targetServer({ server: context.server }) === "hq" &&
          (card as any).flipped
        );
      }),
      effect: effect(function* (
  ...
  staticAbilities: [
    {
      ...
      req: req(function* (
        ...
        return (
          (!flipped && server === "hq") || (flipped && remotes.includes(server))
        );
      }),
      value: req(function* (
        state: State,
  ...
  events: [
    {
      ...
      optional: {
        req: req(function* (
          ...
          const ctx = (targets as any)?.[0]?.context || {};
          const validCtx = (entry: any) =>
            (entry?.[0]?.amount ?? entry?.amount ?? 0) > 0;
          return (
            (ctx.amount || 0) > 0 &&
        ...
        yesAbility: {
          ...
          effect: effect(function* (
            ...
            (coreRevealing as any).revealAndQueueEvent?.(
              state,
              side,
              targets[0],
            );
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          coreCard.operation(accessed) &&
          coreEvents.firstEvent(state, side, "access", (entry: any) =>
            coreCard.operation(
              entry?.[0]?.["accessed-card"] || entry?.[0]?.accessedCard,
            ),
          )
        );
      }),
  ...
  interactions: {
    "pay-credits": {
      req: req(function* (
        ...
        return (
          coreEid.sourceType(eid) === "ability" &&
          coreCard.hasSubtype(abTarget as Card, "Icebreaker")
        );
      }),
      type: "recurring",
    },
...
export const card_EpiphanyAnalytica_NationsUndivided: CardDef = {
  ...
  events: [
    {
      ...card_EpiphanyAnalytica_Ability,
      event: "runner-trash",
      req: req(function* (
        state: State,
        side: Side,
    ...
    },
    {
      ...card_EpiphanyAnalytica_Ability,
      event: "agenda-stolen",
      req: req(function* (): Generator<any, any, any> {
        return true;
      }),
    },
  ],
  abilities: [
    {
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            (coreDefHelpers as any).scry?.(state, side, card, side, 3),
          ],
          [],
        );
        continue_ability(
          ...
          "corp",
          {
            ...
            effect: effect(function* (
              ...
              const idx = top.findIndex((c: Card) =>
                coreCard.sameCard(targets[0], c),
              );
...
function esaCheckBrain(targets: any[]): boolean {
  ...
  return (
    (ctx.amount || 0) > 0 &&
    (ctx["damage-type"] === "brain" || ctx.damageType === "brain")
  );
}
export const card_EsaAfontov_EcoInsurrectionist: CardDef = {
  ...
  events: [
    {
      ...
      optional: {
        req: req(function* (
          ...
          return (
            esaCheckBrain(targets) &&
            coreEvents.firstEvent(state, "runner", "damage", (entry: any) =>
              esaCheckBrain(entry),
            )
          );
        }),
        ...
        yesAbility: {
          ...
          effect: effect(function* (
            ...
            continue_ability(
              state,
              side,
              (coreSabotage as any).sabotageAbility?.(2),
              card,
              null,
            );
          }),
  ...
  abilities: [
    coreOptional.setAutoresolve(
      "auto-fire",
      "Esâ Afontov: Eco-Insurrectionist drawing cards",
    ),
  ],
};
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const installedCard = context.card;
        const previousZone =
          context["previous-zone"] || context.previousZone || [];
        return (
          coreCard.program(installedCard) &&
          previousZone.some((z: string) => z === "discard")
        );
      }),
      msg: "draw 1 card",
      effect: effect(function* (
  ...
  interactions: {
    "access-ability": {
      ...
      req: req(function* (
        ...
        const virusCount =
          (coreMemory as any).numberOfRunnerVirusCounters?.(state) || 0;
        ...
        if (playOrRez === 0) {
          continue_ability(
            ...
            side,
            {
              async: true,
              msg: msg(function* (): Generator<any, any, any> {
                return `trash ${accessed.title} at no cost`;
              }),
              effect: effect(function* (
                state: State,
                side: Side,
          ...
          yield wait_for(
            ...
            [
              ...
              coreEngine.resolveAbility?.(
                state,
                side,
                (coreCostFns as any).pickVirusCountersToSpend?.(playOrRez),
                card,
                null,
              ),
            ],
          ...
          if (result?.msg) {
            coreSay.systemMsg(
              state,
              "runner",
              `uses ${card.title} to trash ${accessed.title} at no cost, spending ${result.msg}`,
            );
            coreMoving.trash(
              state,
              side,
            ...
            (coreAccess as any).accessNonAgenda?.(state, side, eid, accessed, {
              "skip-trigger-event": true,
            });
          }
        }
      }),
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          ctx.targetServer === "hq" &&
          coreEvents.firstSuccessfulRunOnServer(state, "hq")
        );
      }),
      msg: "gain 2 [Credits]",
      async: true,
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const context = (targets as any)[0]?.context || {};
        const accessedCard = context.accessedCard
          ? coreCard.getCard(state, context.accessedCard)
          : null;
  ...
  events: [
    {
      req: req(function* (
        ...
        if (
          ...
          eid.sourceType === "runner-steal"
        ) {
          const addCosts = (eid as any).additionalCosts || [];
          for (const ac of addCosts) {
            if (
              (ac.cost?.type === "credit" || ac.cost?.type === "x-credit") &&
              coreEid.source(ac.source)?.side === "Corp"
            )
              return true;
          }
        }
  ...
  events: [
    {
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreGaining.gainCredits(state, "corp", 5),
          ],
          [],
        );
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          accessedCard &&
          coreCard.agenda(accessedCard) &&
          (coreEvents.eventCount(state, side, "agenda-stolen") || 0) > 0
        );
      }),
      effect: effect(function* (
        state: State,
        ...
        coreToasts.toast(
          state,
          "runner",
          "Cannot steal due to Haarpsichord Studios.",
          "warning",
        );
      }),
    },
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        if (
          !ice ||
          !coreCard.rezzed(ice) ||
          !coreCard.hasSubtype(ice, "Bioroid")
        )
          return false;
        return coreEvents.firstEvent(
          state,
      ...
      choices: {
        req: req(function* (
          ...
          return (
            ...
            coreRezzing.canPayToRez(state, side, eid, targets[0], {
              "cost-bonus": -4,
            })
          );
        }),
  ...
  staticAbilities: [
    {
      ...
      req: req(function* (
        ...
        return coreCard.hasSubtype(targets[0], "Bioroid");
      }),
      value: 1,
    },
  ],
        ...
        if (tagged) {
          if (!gripRevealed) {
            coreSay.systemMsg(
              state,
              "corp",
              `uses ${coreCard.getTitle(card)} make the Runner play with [runner-pronoun] grip revealed`,
            );
            const grip = (state as any).runner?.hand || [];
            const gripStr =
              grip.length > 0 ? utils.enumerateCards(grip) : "no cards";
            coreSay.systemMsg(
              state,
              "corp",
              `uses ${coreCard.getTitle(card)} to see that the Runner currently has ${gripStr} in [runner-pronoun] grip`,
            );
          ...
          if (gripRevealed) {
            ...
            coreSay.systemMsg(
              state,
              "corp",
              `uses ${coreCard.getTitle(card)} to note that the Runner had ${gripStr} in [runner-pronoun] grip before it was concealed`,
            );
      ...
      effect: effect(function* (
        ...
        if (sameType) {
              ...
              yesAbility: {
                ...
                choices: {
                  req: req(function* (
                    ...
                    return (
                      coreCard.isType(targets[0], cardType) &&
                      coreDefHelpers.inHandStar(state, targets[0])
                    );
                  }),
                },
                async: true,
          ...
          continue_ability(
            ...
            side,
            {
              prompt: `You have no ${cardType} to install`,
              choices: ["Carry on!"],
              promptType: "bogus",
            },
            card,
  ...
  events: [
      ...
      effect: effect(function* (
        ...
        if (card.flipped && !accessedCards) {
          coreSay.systemMsg(
            state,
            "runner",
            "flips [their] identity to Hoshiko Shiro: Untold Protagonist",
          );
          coreUpdate.update!(state, side, {
            ...card,
            flipped: false,
          ...
          coreSay.systemMsg(
            state,
            "runner",
            "gains 2 [Credits] and flips [their] identity to Hoshiko Shiro: Mahou Shoujo",
          );
          coreUpdate.update!(state, side, {
            ...card,
            flipped: true,
    ...
    },
    {
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          state,
          [{ asyncResult: "result" }, coreDrawing.draw(state, "runner", 1)],
          [],
        );
        yield wait_for(
          state,
        ...
        coreSay.systemMsg(
          state,
          "runner",
          `uses ${card.title} to draw 1 card and lose 1 [Credits]`,
        );
        return coreEid.effectCompleted(state, side, eid);
      }),
    },
      ...
      effect: effect(function* (
        ...
        if (revealed) {
          coreSay.systemMsg(
            state,
            side,
            `uses ${card.title} to reveal ${revealed.title} from the top of the Stack`,
          );
      ...
      effect: effect(function* (
        ...
        if (revealed) {
          coreSay.systemMsg(
            state,
            side,
            `uses ${card.title} to reveal ${revealed.title} from the Grip`,
          );
          coreRevealing.reveal(state, side, eid, revealed);
        }
      }),
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          ((state as any).corp?.agendaPoint || 0) >
          ((state as any).runner?.agendaPoint || 0)
        );
      }),
      automatic: "gain-credits",
      msg: "gain 2 [Credits]",
  ...
  staticAbilities: [
    {
      ...
      value: req(function* (
        ...
        return ((state as any).corp?.discard || []).filter((c: Card) => !c.seen)
          .length;
      }),
    },
  ],
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const corpInstallEvents =
          coreEvents.turnEvents(state, side, "corp-install") || [];
        const advanceEvents =
          coreEvents.turnEvents(state, side, "advance") || [];
        const context = (targets as any)[0]?.context || {};
        const contextCard = context.card
          ? coreCard.getCard(state, context.card)
          : null;
        if (!contextCard) return true;
        const matchesInstall = corpInstallEvents.every(
          (e: any) => !coreCard.sameCard(contextCard, e[0].card),
        );
        const matchesAdvance = advanceEvents.every(
          (e: any) => !coreCard.sameCard(contextCard, e[0].card),
        );
  ...
  events: [
    ...
    },
    {
      ...
      req: req(function* (
        ...
        return (
          hasMostFaction(state, "runner", "Shaper") &&
      ...
      effect: effect(function* (
        ...
        continue_ability(
          ...
          side,
          {
            ...
            choices: {
              card: (c: Card) => coreCard.installed(c) && coreCard.corp(c),
            },
            msg: msg(
              "place ",
              (n: number) => utils.quantify(n, "advancement counter"),
              " on ",
              (c: Card) => coreToString.cardStr(state, c) || "",
            ),
            async: true,
            effect: effect(function* (
              ...
              coreProps.addProp("corp", eid, targets[0], "advance-counter", p, {
                placed: true,
              });
            }),
          },
  ...
  staticAbilities: [
    {
      ...
      req: req(function* (
        ...
        if (!run) return false;
        return (
          (coreEvents.runEventCount(state, side, "tag-interrupt") || 0) === 0
        );
      }),
      value: true,
    },
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        if (!run) return false;
        return (
          (coreEvents.runEventCount(state, side, "tag-interrupt") || 0) <= 1
        );
      }),
      msg: "avoid 1 tag",
      effect: effect(function* (
  ...
  abilities: [
    {
      ...
      effect: effect(function* (
        ...
        if (target === "The Brewery")
          coreToasts.toast(
            state,
            "corp",
            "Flip to: The Brewery (Do 2 net damage)",
            "info",
          );
        else if (target === "The Tank")
          coreToasts.toast(
            state,
            "corp",
            "Flip to: The Tank (Shuffle Archives into R&D)",
            "info",
          );
        else if (target === "The Greenhouse")
          coreToasts.toast(
            state,
            "corp",
            "Flip to: The Greenhouse (Place 4 advancement counters on a card)",
            "info",
          );
        else
      ...
      effect: effect(function* (
        ...
        if (flip === "The Brewery") {
          ...
          coreSay.systemMsg(
            state,
            side,
            `uses The Tank to shuffle Archives into R&D`,
          );
          ...
          coreSay.systemMsg(
            state,
            side,
            `uses The Greenhouse to place 4 advancement counters on a card that can be advanced`,
          );
          coreUpdate.update!(state, side, {
            ...card,
            code: "greenhouse",
            ...
            side,
            {
              ...
              choices: {
                req: req(function* (
                  ...
                  return coreCard.canBeAdvanced(state, targets[0]);
                }),
              },
              async: true,
              effect: effect(function* (
                ...
                coreProps.addProp(eid, targets[0], "advance-counter", 4, {
                  placed: true,
                });
              }),
            },
          ...
          coreToasts.toast(
            state,
            "corp",
            `Unknown Jinteki Biotech: Life Imagined card: ${flip}`,
            "error",
          );
          return coreEid.effectCompleted(state, side, eid);
        }
  ...
  events: [
    {
      ...
      effect: effect(function* (
        ...
        coreDamage.damage(eid, "net", 1, { card });
      }),
    },
    {
      event: "agenda-stolen",
  ...
  events: [
    {
      ...
      msg: msg(
        "trash ",
        (r: any) => (r.hand || [])[0]?.title || "the top card",
      ...
      effect: effect(function* (
        ...
        coreMoving.mill("corp", eid, "runner", 1);
      }),
    },
  ],
};
  ...
  staticAbilities: [
    {
      ...
      req: req(function* (
        ...
        return (
          coreEvents.eventCount(state, side, "run", (ctx: any) =>
            coreServers.isCentral(ctx[0]?.server || ""),
          ) === 0
        );
      }),
      value: req(function* (
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          ((state as any).corp?.discard || []).filter((c: Card) => !c.seen)
            .length > 0
        );
      }),
      msg: "gain 1 [Credits]",
  ...
  abilities: [
    {
      ...
      cost: [corePayment.toC("click", 1)],
      label:
        "Install a non-virus program from the stack, lowering the cost by 1 [Credit]",
            ...
            (c: Card) =>
              ...
              corePayment.canPay(
                ...
                [
                  corePayment.toC(
                    "credit",
                    coreCostFns.installCost(state, side, c, {
                      "cost-bonus": -1,
                    }) || 0,
                  ),
                ],
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreInstalling.runnerInstall(state, side, target, {
              "cost-bonus": -1,
              msgKeys: { "display-origin": true, "install-source": card },
            }),
          ],
          [],
        );
        ...
        coreEngine.registerEvents(state, side, card, [
          {
            ...
            effect: effect(function* (
              ...
              coreMoving.move(
                state,
                side,
                coreCard.getCard(state, installedCard),
                "rfg",
              );
            }),
          },
  ...
  staticAbilities: [
    {
      ...
      req: req(function* (
        ...
        const isKateType =
          coreCard.hardware(targets[0]) || coreCard.program(targets[0]);
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const contextCard = context.card
          ? coreCard.getCard(state, context.card)
          : null;
        if (!contextCard || !coreCard.hasSubtype(contextCard, "Run"))
          return false;
        return coreEvents.firstEvent(
          state,
          "runner",
  ...
  events: [
    {
      ...
      effect: effect(function* (
        ...
        const canInstall = hand.some((c: Card) => {
          ...
          const cost = coreCostFns.installCost(state, side, c, {
            "cost-bonus": -1,
          });
          return corePayment.canPay(
            state,
            side,
            { ...eid, source: card, sourceType: "runner-install" },
            c,
            null,
            [corePayment.toC("credit", cost || 0)],
          );
        });
        if (canInstall) {
          continue_ability(
            ...
            side,
            {
                  ...
                  return (
                    ...
                    corePayment.canPay(
                      ...
                      [
                        corePayment.toC(
                          "credit",
                          coreCostFns.installCost(state, side, targets[0], {
                            "cost-bonus": -1,
                          }) || 0,
                        ),
                      ],
              ...
              effect: effect(function* (
                ...
                coreInstalling.runnerInstall(state, side, eid, targets[0], {
                  "cost-bonus": -1,
                  msgKeys: { "display-origin": true, "install-source": card },
                });
              }),
...
export const card_LaramyFisk_SavvyInvestor: CardDef = {
  ...
  events: [
    {
      ...
      skippable: true,
      async: true,
      interactive: coreOptional.getAutoresolve(
        "auto-fire",
        (v: any) => !coreOptional.never(v),
      ),
      silent: coreOptional.getAutoresolve("auto-fire", (v: any) =>
        coreOptional.never(v) ? true : false,
      ),
      optional: {
        req: req(function* (
          state: State,
          ...
          return coreEvents.firstEvent(
            ...
            side,
            "successful-run",
            (ctx: any) => coreServers.isCentral(ctx[0]?.server || ""),
          );
        }),
        autoResolve: coreOptional.getAutoresolve("auto-fire"),
        ...
        noAbility: {
          effect: effect(function* (
            ...
            coreSay.systemMsg(`declines to use ${card.title}`);
          }),
        },
      },
    },
  ...
  abilities: [
    {
      effect: effect(function* (state: State): Generator<any, any, any> {
        coreOptional.setAutoresolve("auto-fire", "Laramy Fisk: Savvy Investor");
      }),
    },
  ],
};
...
export const card_Lat_EthicalFreelancer: CardDef = {
          ...
          side,
          {
            optional: {
              req: req(function* (
                ...
                return runHand.length === corpHand.length;
              }),
              autoResolve: coreOptional.getAutoresolve("auto-fire"),
              waitingPrompt: true,
              prompt: "Draw 1 card?",
  ...
  abilities: [
    {
      effect: effect(function* (state: State): Generator<any, any, any> {
        coreOptional.setAutoresolve("auto-fire", "Lat: Ethical Freelancer");
      }),
    },
  ],
};
  ...
  events: [
    {
      ...
      choices: {
        card: (c: Card) =>
          !coreCard.faceup(c) && coreCard.installed(c) && coreCard.corp(c),
        all: true,
      },
      changeInGameState: {
        silent: true,
        req: req(function* (state: State): Generator<any, any, any> {
          return (coreBoard.allInstalled(state, "corp") || []).some(
            (c: Card) => !coreCard.faceup(c) && coreCard.installed(c),
          );
        }),
      },
      msg: msg(
      ...
      effect: effect(function* (
        ...
        coreMoving.move("corp", targets[0], "hand");
      }),
    },
    {
      event: "agenda-stolen",
...
export const card_LEOConstruction_LaborSolutions: CardDef = {
  ...
  abilities: [
    {
      cost: [corePayment.toC("bioroid-run-server", 1)],
      once: "per-turn",
      label: "end the run",
      msg: "end the run",
      async: true,
  ...
  events: [
    {
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreDrawing.draw(state, "runner", 2, { suppressCheckpoint: true }),
          ],
          [],
        );
        coreTags.gainTags(state, "runner", eid, 1);
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          contextCard &&
          coreCard.ice(contextCard) &&
          coreEvents.firstEvent(state, side, "rez", (ctx: any) => {
            const c = ctx.card ? coreCard.getCard(state, ctx.card) : null;
      ...
      effect: effect(function* (
        ...
        coreGaining.gainCredits("runner", eid, 2);
      }),
    },
  ],
};
  ...
  events: [
    {
      ...
      effect: effect(function* (
        ...
        const context = (targets as any)[0]?.context || {};
        const cards = context.cards || [];
        const installable = cards.filter(
          (c: Card) =>
            (coreCard.hardware(c) || coreCard.program(c)) &&
            coreInstalling.runnerCanPayAndInstall(state, "runner", eid, c, {
              noToast: true,
            }),
        );
  ...
  events: [
    {
      ...
      prompt: msg(function* (
        ...
        const deck = (state as any).runner?.deck || [];
        if (deck.length > 0)
          return `trash ${utils.enumerateCards(deck.slice(0, 2))} from the stack and draw 1 card`;
