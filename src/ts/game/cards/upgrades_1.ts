    ...
    optional: {
      prompt: msg(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          `Move ${card.title} to another server?`,
      ),
      "waiting-prompt": true,
      "yes-ability": {
        ...
        effect: effect(function* (
          ...
          yield wait_for(
            state,
            [{ asyncResult: "result" }, coreEngine.unregisterEvents(card)],
            [],
          );
          yield wait_for(
            state,
...
export function canSmartPurge(state: State): boolean {
  ...
  const titles = allInstalled
    .map((c: Card) => c.title)
    .filter(Boolean) as string[];
  return (
    !titles.includes("Acacia") &&
    !titles.includes("Fester") &&
    !titles.includes("Heliamphora")
  );
}
    ...
    mobileSysopEventFn(),
    {
      ...
      psi: {
        ...
        "not-equal": {
          msg: msg(
            (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
              `prevent the Runner from accessing cards other than ${card.title}`,
          ),
          async: true,
          effect: effect(
            (
        ...
        equal: {
          msg: msg(
            (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
              `prevent the Runner from accessing ${card.title}`,
          ),
          async: true,
          effect: effect(
            (
              ...
              coreFlags.registerRunFlag(
                card,
                ":can-access",
                (_state: State, _side: Side, target: Card) =>
                  !coreCard.sameCard(target, card),
              );
              coreEffects.effectCompleted(eid);
            },
          ),
  ...
  "static-abilities": [
    {
      ...
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const t = targets[0];
          return (
            t && coreCard.ice(t) && coreServers.protectingSameServer(card, t)
          );
        },
      ),
      value: -2,
      ...
      effect: effect(function* (
        ...
        if (ctx["did-steal"]) {
          yield wait_for(
            ...
            [
              { asyncResult: "result" },
              coreTags.gainTags(state, ":corp", eid, 2),
            ],
            [],
          );
          yield wait_for(
            ...
            [
              ...
              coreSay.systemMsg(
                state,
                ":corp",
                `${card.title} gives the Runner 2 tags`,
              ),
            ],
            [],
          );
        ...
        "yes-ability": {
          ...
          effect: effect(function* (
            ...
            yield wait_for(
              ...
              [
                { asyncResult: "result" },
                coreRezzing.rez({ ...eid, source: card }, cardCtx, {
                  "cost-bonus": -3,
                }),
              ],
              [],
...
export const angeliqueGarzaCorrea: CardDef = {
  ...
  "on-access": {
    optional: {
      ...
      prompt: msg(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          `Pay 2 [Credits] to use ${card.title} ability?`,
      ),
      "no-ability": {
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
  ...
  events: [
    {
      ...
      interactive: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          !!(state as any).corp?.hand?.some((c: Card) =>
            coreCard.corpInstallableType(c),
          ),
      ),
      silent: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          !!(state as any).corp?.hand?.some(
            (c: Card) => !coreCard.corpInstallableType(c),
          ),
      ),
      async: true,
      effect: effect(function* (
  ...
  events: [
    {
      ...
      trace: {
        ...
        req: req(forms.thisServer),
        successful: {
          msg: (_state: State, _side: Side, _eid: EID, card: Card) =>
            `prevent the Runner from accessing cards other than ${card.title}`,
          effect: effect(
            (
              state: State,
                ...
                coreEngine.registerEvents(state, side, card, [
                  {
                    ...
                    effect: effect(
                      coreMoving.trash(
                        eid,
                        coreFinding.getCard(state, iceCard)!,
                        { causeCard: card },
                      ),
                    ),
                  },
            ...
            yield wait_for(
              ...
              [
                ...
                coreSay.systemMsg(
                  state,
                  side,
                  `uses ${card.title} to force the Runner to encounter ${coreToString.cardStr(state, iceCard)}`,
                ),
              ],
              [],
            );
  ...
  abilities: [
    {
      ...
      msg: msg(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          `reveal ${utils.enumerateCards((state as any).corp?.deck?.slice?.(0, 3) || [])} from the top of R&D`,
      ),
      "waiting-prompt": true,
      effect: effect(function* (
        state: State,
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreRevealing.reveal(state, side, deckCards),
          ],
          [],
        );
        yield wait_for(
              ...
              side,
              {
                ...
                msg: msg(
                  (
                    ...
                    card: Card,
                    targets: any[],
                  ) => `add 1 of the revealed cards to HQ`,
                ),
                effect: effect(function* (
                  s: State,
                  sd: Side,
                  ...
                  yield wait_for(
                    ...
                    [
                      { asyncResult: "result" },
                      coreMoving.move(s, sd, target, "hand"),
                    ],
                    [],
                  );
                }),
  ...
  events: [
    {
      ...
      msg: msg(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const target: any = (targets as any[])?.[0];
          return `force the Runner to ${target.toLowerCase()}`;
        },
      ),
      prompt: "Choose one",
      "waiting-prompt": true,
                  ...
                  "yes-ability": {
                    choices: {
                      card: (c: Card) =>
                        coreCard.ice(c) &&
                        coreCard.rezzed(c) &&
                        !coreCard.sameCard(c, rezzedCard),
                    },
                    async: true,
                    effect: effect(function* (
                      s: State,
                      ...
                      yield wait_for(
                        ...
                        [
                          ...
                          coreRezzing.derez(
                            s,
                            sd,
                            coreFinding.getCard(s, target)!,
                            {
                              "msg-keys": {
                                "and-then": `to give ${coreToString.cardStr(s, rezzedCard)} +3 strength for the remainder of the run`,
                              },
                            },
                          ),
                        ],
  ...
  abilities: [
    {
      ...
      label:
        "Play a transaction operation from Archives, ignoring all costs, and remove it from the game",
      prompt: "Choose a transaction operation to play",
      msg: msg(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const target: any = (targets as any[])?.[0];
          return `play ${target.title} from Archives, ignoring all costs, and removes it from the game`;
        },
      ),
      choices: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const corp = (state as any).corp;
          const ops = corp?.discard?.filter(
            (c: Card) =>
              coreCard.operation(c) && coreCard.hasSubtype(c, "Transaction"),
          );
          ...
          coreEngine.continueAbility(
            {
              ...
              msg: msg(
                (
                  ...
                  const target: any = (targets as any[])?.[0];
                  return `place an advancement counter on ${coreToString.cardStr(state, target)}`;
                },
              ),
