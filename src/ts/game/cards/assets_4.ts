      ...
      const playable = hand.filter((c: Card) => {
        ...
        return (
          ...
          corePayment.canPay(
            ...
            c,
            null,
            [corePayment.toC("credit", playCost)],
          )
        );
      });
    ...
    abilities: [
      {
        ...
        effect: req(function* (
          ...
          yield wait_for(
            ...
            [
              { asyncResult: "result" },
              coreGaining.gainCredits(state, side, eid, 4, {
          ...
          yield wait_for(
            ...
            [
              ...
              coreEngine.resolveAbility(
                state,
                side,
                coreDefHelpers.corpInstallUpToNCards(2),
                card,
                null,
              ),
            ],
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          ctx.corpCredits != null &&
          coreEvents.firstEvent(state, side, ":reveal-spent-credits")
        );
      }),
      msg: msg(
...
export const ibrahimSalem: CardDef = (() => {
  const trashAbility = (cardType: string): any =>
    coreDefHelpers.withRevealedHand(
      ":runner",
      { eventSide: ":corp" },
      {
        req: req(function* (state: State): Generator<any, any, any> {
          return ((state as any).runner?.hand || []).some((c: Card) =>
            coreCard.isType(c, cardType),
          );
        }),
...
export const idiosyncresis: CardDef = (() => {
  ...
  const loseAmt = (card: Card, runner: any) =>
    Math.min(2 * adv(card), runner?.credit || 0);
  ...
  events: [
    {
      ...
      req: req(function* (state: State): Generator<any, any, any> {
        return !!(state as any).runner?.scored?.length;
      }),
      async: true,
      effect: req(function* (
        state: State,
            ...
            coreEngine.resolveAbility(
              ...
              side,
              {
                optional: {
                  prompt: msg(
                    (s: State) =>
                      `Swap ${(scored as any)?.title} for an agenda in the Runner's score area?`,
                  ),
                  "waiting-prompt": true,
                  req: req(function* (s: State): Generator<any, any, any> {
                    return !!(s as any).runner?.scored?.length;
                  }),
                  "yes-ability": {
                    ...
                    choices: {
                      req: req(function* (
                        ...
                        return (
                          coreFlags.inRunnerScored(s, sd, t[0]) &&
                          !!(t[0] as any)?.agendapoints &&
                          (t[0] as any)?.agendapoints > 0
                        );
                      }),
                    },
                    msg: msg(
                    ...
                    effect: req(function* (
                      ...
                      const [, newScored] = coreMoving.swapAgendas(
                        s,
                        sd,
                        scored,
                        t[0],
...
export const investigatorInezDelgadoA2: CardDef = (() => {
  const swapAbi = (stolen: Card): any => ({
    ...
    req: req(function* (state: State): Generator<any, any, any> {
      return !!(state as any).corp?.scored?.length;
    }),
    choices: {
      req: req(function* (
        s: State,
                  ...
                  side,
                  {
                    optional: {
                      ...
                      "yes-ability": {
                        ...
                        msg: msg(
                          (s: State) =>
                            `resolve the when-scored ability on ${(stolen as any)?.title}`,
                        ),
                        effect: req(function* (
                          s: State,
                          sd: Side,
  ...
  const ability: any = {
    ...
    effect: effect(function* (
      ...
      yield wait_for(
        ...
        [
          { asyncResult: "result" },
          coreProps.addCounter(state, side, eid, card, ":credit", 3, {
            placed: true,
          }),
        ],
        [],
  ...
  return {
    ...
    abilities: [
      ability,
      {
        ...
        cost: [corePayment.toC("click", 1)],
        label:
          "Take all hosted credits and add this asset to HQ. Install 1 card from HQ",
        async: true,
        msg: msg(
            ...
            [
              ...
              coreEngine.resolveAbility(
                ...
                side,
                {
                  ...
                  choices: {
                    card: (c: Card) =>
                      coreCard.corpInstallableType(c) && coreCard.inHand(c),
                  },
                  effect: effect(function* (
                    s: State,
                    sd: Side,
...
export const jeevesModelBioroids: CardDef = (() => {
  ...
  const cleanup = effect(function* (
    ...
    coreUpdate.update(state, side, {
      ...(coreCard.getCard(state, card) || card),
      seenThisTurn: undefined,
    });
  });
  return {
  ...
  abilities: [
    {
      ...
      "change-in-game-state": {
        req: req(function* (state: State): Generator<any, any, any> {
          return !!(state as any).runner?.deck?.length;
        }),
      },
      async: true,
      effect: req(function* (
  ...
  abilities: [
    {
      ...
      req: req(function* (
        ...
        const counters = coreCard.getCounters(
          coreCard.getCard(state, card),
          ":power",
        );
        return ((state as any).corp?.hand || []).some(
          (c: Card) =>
      ...
      choices: {
        req: req(function* (
          ...
          return (
            ...
            (targets[0] as any).agendapoints ===
              coreCard.getCounters(coreCard.getCard(state, card), ":power") &&
      ...
      effect: req(function* (
        ...
        coreInitializing.cardInit(state, ":corp", c as Card, {
          resolveEffect: false,
          initData: true,
        });
  ...
  abilities: [
    {
      req: req(function* (
        ...
        return ((state as any).corp?.hand || []).some(
          (c: Card) => coreCard.agenda(c) && power >= (c as any).agendapoints,
        );
      }),
              ...
              side,
              {
                ...
                choices: {
                  req: req(function* (
                    ...
                    return (
                      coreCard.agenda(t[0]) &&
                      (t[0] as any).agendapoints <= paidAmt
                    );
                  }),
                },
                msg: msg(
                ...
                effect: req(function* (
                  ...
                  coreFlags.registerTurnFlag(
                    ...
                    c,
                    ":can-steal",
                    (ss: State, _: Side, fc: Card) => {
                      if ((fc as any)?.title === title) {
                        coreToasts.toast(
                          ss,
                          ":runner",
                          "Cannot steal due to Lakshmi Smartfabrics.",
                          "warning",
                        );
                        return false;
                      }
  ...
  abilities: [
    {
      ...
      cancel: {
        ...coreShuffling.shuffleMyDeck,
        cost: [corePayment.toC("credit", 1), corePayment.toC("click", 1)],
        action: true,
      },
      "keep-menu-open": ":while-clicks-left",
      effect: effect(function* (
  ...
  abilities: [
    {
      ...
      choices: req(function* (state: State): Generator<any, any, any> {
        const ops = ((state as any).corp?.deck || [])
        ...
        yield wait_for(
          ...
          [
            ...
            coreEngine.resolveAbility(
              ...
              {
                req: req(function* (s: State): Generator<any, any, any> {
                  const numAccess = coreAccess.numCardsToAccess(
                    s,
                    ":runner",
                    ctx.server,
                    null,
