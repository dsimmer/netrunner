...
export const stimhack: CardDef = {
  ...
  onPlay: {
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreRuns.gainNextRunCredits(9); coreRuns.makeRun(eid, msg, card); }),
  },
  events: [{
    event: 'run-ends',
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDamage.damage(eid, 'brain', 1, { unpreventable: true, card: card }); }),
  }],
};
...
export const strikeFund: CardDef = {
  ...
  onPlay: {
    async: true,
    msg: 'gain 4 [Credits]',
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainCredits(state, 'runner', null, 4); coreEid.effectCompleted(state, side, eid); }),
  },
  onTrash: {
    whenInactive: true,
...
export const sureGamble: CardDef = {
  ...
  onPlay: {
    msg: 'gain 9 [Credits]',
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainCredits(eid, 9); }),
  },
};
...
export const surge: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg('place 2 virus counters on ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg),
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreProps.addCounter('runner', eid, msg, 'virus', 2, null); }),
  },
};
...
export const synAttack: CardDef = {
  ...
  onPlay: {
    ...
    async: true,
    msg: msg('force the Corp to ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg),
      ...
      if (msg === 'Draw 4 cards') {
        ...
        yield continue_ability(
          ...
          'corp',
          {
            ...
            async: true,
            effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreMoving.trashCards('corp', eid, targets, { unpreventable: true, causeCard: card, cause: 'forced-to-trash' }); }),
          },
          card,
          null
        );
...
export const systemOutage: CardDef = {
  ...
  events: [{
    event: 'corp-draw',
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return !coreEvents.firstEvent(state, side, 'corp-draw'); }),
    msg: 'force the Corp to lose 1 [Credits]',
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.loseCredits('corp', eid, 1); }),
  }],
};
  ...
  events: [
    {
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0];
    ...
    { event: 'corp-turn-ends', effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreUpdate.updateIn(coreCard.getCard(state, card), ['special', 'ssTarget'], () => undefined); }) },
    { event: 'runner-turn-ends', effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreUpdate.updateIn(coreCard.getCard(state, card), ['special', 'ssTarget'], () => undefined); }) },
  ],
};
...
export const tailgate: CardDef = {
  title: 'Tailgate',
  makesRun: true,
  onPlay: runServerAbility('hq', { playCostBonus: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return -(state as any).corp?.servers?.hq?.ices?.length || 0; }) }),
  events: [{
    event: 'successful-run',
    silent: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return true; }),
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; return ctx.server === 'hq' && forms.thisCardRun; }),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreEngine.registerEvents(card, [coreDefHelpers.breachAccessBonus('hq', 2, { duration: 'end-of-run' })]); }),
  }],
};
...
export const takeADive: CardDef = {
  ...
  events: [{
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreBadPublicity.gainBadPublicity(state, 'corp', eid, 1, { card: card }); }),
  }],
};
...
export const testRun: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg('install a program from the ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg),
    waitingPrompt: true,
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { continue_ability(
        (() => {
              ...
              asyncResult
                ? (() => {
                    ...
                    coreEngine.registerEvents(state, side, installedCard, [{
                      ...
                      msg: msg('move ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => installedCard.title, ' to the top of the stack'),
...
export const theMakersEye: CardDef = {
  ...
  events: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreEngine.registerEvents(card, [coreDefHelpers.breachAccessBonus('rd', 2, { duration: 'end-of-run' })]); }),
  }],
};
...
export const theNoblePath: CardDef = {
  ...
  makesRun: true,
  staticAbilities: [
    { type: 'cannot-pay-net', req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return forms.run(state); }), value: true },
    { type: 'cannot-pay-brain', req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return forms.run(state); }), value: true },
    { type: 'cannot-pay-meat', req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return forms.run(state); }), value: true },
  ],
  prevention: [{
    prevents: 'damage',
    type: 'event',
    ...
    ability: {
      ...
      condition: 'active',
      msg: msg('prevent ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => ctx.remaining, ' ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreDamage.damageName(state), ' damage'),
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { corePrevention.preventDamage(state, side, eid, 'all'); }),
    },
  }],
  onPlay: {
    async: true,
    ...
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      yield continue_ability(
        ...
        side,
        {
          ...
          msg: msg('trash [their] grip and make a run on ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg, ', preventing all damage'),
...
export const thePriceOfFreedom: CardDef = {
  ...
  events: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreFlags.registerTurnFlag(card, 'can-advance', () => false); }),
  }],
};
  ...
  onPlay: {
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreEngine.registerEvents(
        ...
        [{
          ...
          msg: msg('gain ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => (coreEvents.runEvents(state, 'runner', 'successful-run') || []).length * 2, ' [Credits]'),
          async: true,
...
export const tinkering: CardDef = {
  ...
  onPlay: {
    ...
    onChangeGameState: {
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (coreBoard.allInstalled(state, 'corp') || []).some((c: Card) => coreCard.ice(c)); }),
    },
    msg: msg('make ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, msg), ' gain Sentry, Code Gate, and Barrier until the end of the turn'),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreEffects.registerLingeringEffect(state, side, card, {
        type: 'gain-subtype',
        duration: 'end-of-turn',
        req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return utils.sameCard(msg, msg); }),
    ...
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      yield continue_ability(
        ...
        'runner',
        {
          ...
          effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreEngine.triggerEvent('searched-stack'); coreShuffling.shuffle(state, side, 'deck'); coreMoving.move(state, side, msg, 'hand'); }),
        },
        card,
        null
      );
...
export const treadLightly: CardDef = {
  ...
  staticAbilities: [{
    type: 'rez-cost',
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return forms.run(state) && coreCard.ice(msg); }),
    value: 3,
  }],
};
...
export const trickShot: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreUpdate.updateIn(state, side, 'runEid', eid); coreRuns.makeRun(state, side, eid, 'rd', card); }),
  },
  events: [
    {
      event: 'successful-run',
      ...
      async: true,
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreEngine.registerEvents(card, [coreDefHelpers.breachAccessBonus('rd', 1, { duration: 'end-of-run' })]); coreProps.addCounter(eid, card, 'credit', 2, { placed: true }); }),
    },
    {
      event: 'run-ends',
      unregisterOnceResolved: true,
      ...
      msg: msg('make a run on ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg),
      async: true,
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreRuns.makeRun(eid, msg, card); }),
    },
  ],
...
export const uninstall: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg('move ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg, ' to [their] Grip'),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreMoving.move(state, side, msg, 'hand'); }),
  },
};
...
export const unscheduledMaintenance: CardDef = {
  ...
  events: [{
    event: 'corp-install',
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; return coreCard.ice(ctx.card); }),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreFlags.registerTurnFlag(
        card,
        'can-install-ice',
        function*(state: State, side: Side, card: Card): Generator<any, any, any> {
...
export const vamp: CardDef = {
  ...
  events: [{
    ...
    ability: {
      ...
      async: true,
      onChangeGameState: { req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return corePayment.costValue(eid, 'x-credits') > 0; }) },
      msg: msg('make the corp lose ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => corePayment.costValue(eid, 'x-credits'), ' [Credits]'),
      effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
...
export const wantonDestruction: CardDef = {
  ...
  events: [{
    ...
    thisCardRun: true,
    ability: {
      msg: msg('force the Corp to discard ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg, ' card from HQ at random'),
...
export const watchTheWorldBurn: CardDef = {
  ...
  events: [{
    ...
    once: 'per-run',
    msg: msg('remove ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => ctx.accessedCard?.title, ' from the game, and watch for other copies of ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => ctx.accessedCard?.title, ' to burn'),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; coreMoving.move('corp', ctx.accessedCard, 'rfg'); coreEngine.registerEvents(card, watchTheWorldBurnRfgCardEvent(ctx.accessedCard)); }),
  }],
};
...
function watchTheWorldBurnRfgCardEvent(burnedCard: Card): any[] {
  return [{
    ...
    msg: msg('remove ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => burnedCard.title, ' from the game'),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0]; coreMoving.move('corp', ctx.accessedCard, 'rfg'); }),
  }];
}
  ...
  onPlay: {
    trace: {
      ...
      unsuccessful: coreDefHelpers.withRevealedHand('corp', { eventSide: 'corp', forced: true }, {
        ...
        player: 'runner',
        choices: { req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return targets?.some((c: Card) => coreCard.corp(c) && coreCard.inHand(c)); }), max: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return Math.min(2, (state as any).corp?.hand?.length || 0); }) },
        msg: msg('shuffle ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => (targets || []).map((c: Card) => c.title).join(', '), ' into R&D'),
        effect: effect(
          function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
    ...
    [
      {
        ...
        ability: { msg: 'force the Runner to gain 6 [Credits]', displaySide: 'corp', async: true, effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainCredits(state, 'runner', eid, 6); }) },
      },
      {
        option: 'Runner draws 4 cards',
        ability: { msg: 'force the Runner to draw 4 cards', displaySide: 'corp', async: true, effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDrawing.draw(state, 'runner', eid, 4); }) },
      },
    ]
  ),
};
...
export const windowOfOpportunity: CardDef = {
  ...
  events: [{
    ...
    async: true,
    unregisterOnceResolved: true,
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0];
      ...
      if (rezzedTargets.length > 0) {
        yield continue_ability(
          ...
          side,
          {
            ...
            waitingPrompt: true,
            choices: { req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return rezzedTargets.some((c: Card) => utils.sameCard(msg, c)); }) },
            async: true,
            effect: effect(
              (() => {
                const chosenIce = msg;
                return effect(
                  coreEngine.registerEvents(state, side, card, [{
                    ...
                    optional: {
                      ...
                      prompt: msg('Rez ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreToString.cardStr(state, chosenIce), ', ignoring all costs?'),
...
function windowOfOpportunityInstallAbi(): any {
  return {
    ...
    waitingPrompt: true,
    onChangeGameState: { silent: true, req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (coreDefHelpers.allCardsInHandStar(state, 'runner') || []).length > 0; }) },
    choices: {
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
        const t = targets[0];
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreInstalling.runnerInstall({ ...eid, source: card, sourceType: 'runner-install' }, msg, { msgKeys: { installSource: card, displayOrigin: true } }); }),
  };
}
