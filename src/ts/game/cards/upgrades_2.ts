        ...
        if (count > 0) {
          yield wait_for(state, [{ asyncResult: 'result' },
            continue_ability(state, side, {
              ...
              msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return `place 1 advancement counter on ${coreToString.cardStr(state, target)}`; }),
              async: true,
...
export const coldSiteServer: CardDef = {
  ...
  events: [{
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreProps.addCounter(eid, card, ':power', -coreCard.getCounters(card, ':power'), null); }),
  }],
  abilities: [{
    action: true,
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreProps.addCounter(eid, card, ':power', 1, null); }),
  }],
};
...
export const corporateTroubleshooter: CardDef = {
  ...
  abilities: [{
    ...
    msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return `add ${corePayment.costValue(eid, ':x-credits')} strength to ${target.title}`; }),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; coreIce.pumpIce(target, corePayment.costValue(eid, ':x-credits'), ':end-of-turn'); }),
  }],
};
  ...
  events: [{
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const target: any = (targets as any[])?.[0];
      const chooseSwap = (toSwap: Card) => ({
        ...
        cost: [corePayment.toC('trash-can')],
        msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return `swap ${coreToString.cardStr(state, toSwap)} with ${coreToString.cardStr(state, target)}`; }),
        async: true,
      ...
      const ability = {
        optional: {
          'waiting-prompt': true,
          prompt: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => `Trash ${card.title} to swap a card in this server?`),
          'yes-ability': {
            async: true,
...
export const defenseConstruct: CardDef = {
  ...
  abilities: [{
    ...
    msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => `add ${utils.quantify(targets?.length || 0, 'facedown card')} in Archives to HQ`),
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
...
export const djupstadGrid: CardDef = {
  ...
  events: [{
    ...
    interactive: req(() => true),
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDamage.damage(eid, ':brain', 1, { card }); }),
  }],
};
  ...
  events: [{
    ...
    trace: {
      ...
      successful: {
        ...
        async: true,
        effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDamage.damage(eid, ':meat', 1, {
          card,
          unpreventable: true,
        }); }),
      },
  ...
  events: [
    {
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { continue_ability(
        {
          optional: {
            prompt: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => `Pay 1 [Credit] to place 1 power counter on ${card.title}?`),
            'yes-ability': {
...
export const expoGrid: CardDef = {
  ...
  events: [{
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainCredits(eid, 1); }),
  }],
  abilities: [{
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const corp = (state as any).corp;
  ...
  interactions: {
    'pay-credits': {
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        if (!abTarget || !coreServers.sameServer(card, abTarget)) return false;
...
export const fractalThreatMatrix: CardDef = {
  ...
  events: [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const ctx = targets[0];
      return !!ctx?.['all-subs-broken'] &&
        ctx?.ice && coreServers.protectingSameServer(card, ctx.ice);
    }),
    msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const deck = (state as any).runner?.deck || [];
      if (deck.length > 0) {
        const top = deck.slice(0, 2).map((c: Card) => c.title).join(', ');
...
export const georgiaEmelyov: CardDef = {
  ...
  abilities: [{
    ...
    async: true,
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
        prompt: 'Choose a server',
        choices: coreBoard.serverList(state),
        msg: msg((s: State, sd: Side, e: EID, c: Card, ts: any[]) => `move to ${ts?.[0]}`),
        effect: effect(function*(s: State, sd: Side, e: EID, c: Card, ts: any[]): Generator<any, any, any> {
          const target: any = ts?.[0];
          const newZone = [...(coreServers.serverToZone(s, target) || []), 'content'];
          const moved = yield wait_for(s, [{ asyncResult: 'result' }, (coreMoving as any).move(s, sd, c, newZone)], []);
          yield wait_for(s, [{ asyncResult: 'result' }, coreEngine.unregisterEvents(c)], []);
          yield wait_for(s, [{ asyncResult: 'result' }, coreEngine.registerDefaultEvents(s, sd, moved)], []);
        }),
      }, card, null)], []);
    }),
  }],
...
export const kpLynn: CardDef = {
  ...
  events: [{
    ...
    msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      return target === 'End the run' ? 'end the run' : `force the Runner to ${String(target).toLowerCase()}`;
    }),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const target: any = (targets as any[])?.[0];
...
export const keeganLane: CardDef = {
  ...
  abilities: [{
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      if (!forms.thisServer(state, card)) return false;
      const installed = coreBoard.allActiveInstalled(state, ':runner') || [];
...
export const khondiPlaza: CardDef = {
  title: 'Khondi Plaza',
  'x-fn': req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
    coreBoard.getRemotes(state).length),
  recurring: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
    coreBoard.getRemotes(state).length),
  interactions: {
    'pay-credits': {
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const sourceType = (eid as any)['source-type'];
        const t = targets[0];
        return sourceType === ':rez' && t && coreCard.ice(t) && coreServers.sameServer(card, t);
      }),
      type: ':recurring',
    },
  },
...
export const midwayStationGrid: CardDef = {
  ...
  'static-abilities': [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      return c && coreCard.hasSubtype(c, 'Icebreaker') &&
        ability && ('break' in ability) &&
...
export const neoTokyoGrid: CardDef = (() => {
  const onlyEv = (state: State, side: Side, ev: string, noEv: string, card: Card): boolean => {
    const inSame = (ts: any[]) => ts?.[0]?.card && coreServers.inSameServer(card, ts[0].card);
    return coreEvents.firstEvent(state, side, ev, inSame) &&
      !coreEvents.firstEvent(state, side, noEv, inSame);
  };
  const ngReq = req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
    const ctx = targets[0];
    return ctx?.card && coreServers.inSameServer(card, ctx.card) &&
      (onlyEv(state, side, ':advance', ':advancement-placed', card) ||
        onlyEv(state, side, ':advancement-placed', ':advance', card));
  });
  const ngEffect = effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
    coreGaining.gainCredits(state, side, eid, 1);
  });
  const ng = { req: ngReq, msg: 'gain 1 [Credits]', async: true, effect: ngEffect };
  return {
    title: 'NeoTokyo Grid',
    events: [
      { ...ng, event: ':advance' },
...
export const panicButton: CardDef = {
  ...
  abilities: [
    coreDefHelpers.drawAbility(1, null, {
      ...
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const run = forms.run(state) as any;
        return !!run && coreServers.targetServer({ server: run.server }) === ':hq';
      }),
    }),
  ],
};
...
export const ryonKnight: CardDef = {
  ...
  abilities: [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      forms.thisServer(state, card) && ((state as any).runner?.click ?? 0) === 0),
    cost: [corePayment.toC('trash-can')],
...
export const tyrsHand: CardDef = {
  ...
  abilities: [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const currentIce = (state as any).encounters?.[0]?.ice || (state as any).run?.['current-ice'];
      const cardZone = coreCard.getZone(card) as string[] | undefined;
      const iceZone = currentIce && coreCard.getZone(currentIce) as string[] | undefined;
      if (!cardZone || !iceZone) return false;
      const sameServer = JSON.stringify(cardZone.slice(0, -1)) === JSON.stringify(iceZone.slice(0, -1));
...
export const trafficAnalyzer: CardDef = {
  ...
  events: [{
    ...
    trace: {
      ...
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const ctx = targets[0];
        return ctx?.card && coreServers.protectingSameServer(card, ctx.card) &&
...
export const yakovErikovichAvdakov: CardDef = (() => {
  const validTarget = (ctx: any, card: Card): boolean =>
    !!ctx?.card && coreServers.sameServer(card, ctx.card) &&
    coreCard.corp(ctx.card) && coreCard.installed(ctx.card);
  return {
    ...
    events: [
      ...
      },
      {
        ...
        req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          ...
          return sourceType !== ':corp-install' &&
            (coreCard.corp(sourceCard) ||
              cause === ':ability-cost' ||
              cause === ':subroutine' ||
              (causeCard && coreCard.corp(causeCard) && cause !== ':opponent-trashes') ||
              (causeCard && coreCard.runner(causeCard) && cause === ':forced-to-trash')) &&
  ...
  effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
    yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
      ...
      choices: req((s: State, sd: Side, e: EID, c: Card, ts: any[]) => {
        ...
        return ['Suffer 1 core damage', ...(clicks > 0 ? ['Lose all remaining [Click]'] : [])];
      }),
      async: true,
      ...
      effect: effect((s: State, sd: Side, e: EID, c: Card, ts: any[]) => {
        ...
        if (target === 'Suffer 1 core damage') {
          ...
          coreGaining.loseClicks(s, ':runner', (s as any).runner?.click ?? 0);
...
export const mumbadVirtualTour: CardDef = {
  ...
  flags: {
    'must-trash': req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
...
export const oberthProtocol: CardDef = {
  ...
  events: [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const ctx = targets[0];
      if (!ctx?.card || !coreServers.sameServer(card, ctx.card)) return false;
      const cardZone = (coreCard.getZone(card) as string[] | undefined)?.[1];
      const turnEvents = (coreEvents as any).turnEvents?.(state, side, ':advance') || [];
      const matching = turnEvents.filter((ev: any) => {
        const evCard = ev?.[0]?.card;
        return evCard && (coreCard.getZone(evCard) as string[] | undefined)?.[1] === cardZone;
      });
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      coreProps.addProp(state, ':corp', eid, targets[0]?.card, ':advance-counter', 1, { placed: true });
    }),
  }],
};
...
export const offTheGrid: CardDef = {
  title: 'Off the Grid',
  'install-req': req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
    (targets as any[]).filter((t: any) => t !== 'HQ' && t !== 'R&D' && t !== 'Archives')),
  'static-abilities': [{
...
export const oldHollywoodGrid: CardDef = (() => {
  const fromOrInSameServerReq = req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
    const t = targets[0];
    if (!t) return false;
    const scored = (state as any).runner?.scored || [];
    const notInScored = !scored.some((s: Card) => s.title === t.title);
    return notInScored && (coreServers.inSameServer(card, t) || coreServers.fromSameServer(card, t));
  });
  const fromPrevServerReq = req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
    const t = targets[0];
    if (!t) return false;
    const scored = (state as any).runner?.scored || [];
...
export const satelliteGrid: CardDef = {
  ...
  events: [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const ctx = targets[0];
      return ctx?.card && coreCard.ice(ctx.card) &&
        coreServers.protectingSameServer(card, ctx.card);
    }),
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      coreProps.setProp(state, side, targets[0].card, ':extra-advance-counter', 1);
    }),
  }],
  'leave-play': effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
    const server = coreBoard.cardToServer(state, card);
    const ices = (server as any)?.ices || [];
    for (const c of ices) {
      const updated: any = { ...c };
...
export const shackletonGrid: CardDef = (() => {
  const ev = {
    optional: {
      ...
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const t = targets[0];
        return !!forms.run(state) && forms.thisServer(state, card) &&
...
export const signalJamming: CardDef = {
  ...
  abilities: [{
    ...
    cost: [corePayment.toC('trash-can')],
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      coreFlags.registerRunFlag(state, side, card, ':corp-lock-install', () => true);
      coreFlags.registerRunFlag(state, side, card, ':runner-lock-install', () => true);
      coreToasts.toast(state, ':runner', 'Cannot install until the end of the run');
      coreToasts.toast(state, ':corp', 'Cannot install until the end of the run');
    }),
  }],
};
...
export const hypeMachine: CardDef = {
  title: 'Hype Machine',
  'rez-cost-bonus': req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
  ...
  abilities: [{
    ...
    choices: { req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const t = targets[0];
      return t && coreServers.inSameServer(card, t);
    }) },
    msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
...
export const reducedService: CardDef = {
  ...
  'static-abilities': [{
    ...
    value: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      return Array(n).fill(null).map(() => [corePayment.toC('credit', 2)]);
    }),
  }],
  events: [{
    event: ':successful-run',
  ...
  'on-rez': {
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      yield wait_for(state, [{ asyncResult: 'result' },
        coreProps.addCounter(state, ':corp', eid, card, ':power', spent, null)], []);
      yield wait_for(state, [{ asyncResult: 'result' },
    ...
    trace: {
      ...
      successful: {
        ...
        choices: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          ...
          return [...(clicks >= 2 ? ['Lose [Click][Click]'] : []), 'Suffer 1 core damage'];
        }),
        async: true,
        msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const target: any = (targets as any[])?.[0];
...
export const theRedRoom: CardDef = {
  title: 'The Red Room',
  'legal-zones': req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
    (targets as any[]).filter((t: any) => t === 'R&D' || t === 'HQ' || t === 'Archives')),
  events: [
    {
      event: ':agenda-stolen',
      async: true,
  ...
  events: [{
    ...
    optional: {
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const ctx = targets[0];
        const ice = ctx?.ice;
        if (!ice || !coreCard.rezzed(ice) || !forms.thisServer(state, card)) return false;
        const hand = (state as any).corp?.hand || [];
        return hand.some((c: Card) => c.title === ice.title);
      }),
      prompt: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      'yes-ability': {
        ...
        choices: { req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const t = targets[0];
          const current = coreIce.getCurrentIce(state);
          return t && coreCard.inHand(t) && coreCard.ice(t) && current && t.title === current.title;
        }) },
        msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const t = targets[0];
        ...
        effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          ...
          yield wait_for(state, [{ asyncResult: 'result' },
            coreMoving.trash(state, side, coreEid.makeEid(state, eid),
              { ...target, seen: true }, { causeCard: card })], []);
          const current = coreIce.getCurrentIce(state);
          yield wait_for(state, [{ asyncResult: 'result' },
    ...
    ability: {
      ...
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        return ctx?.type === ':net' &&
          ...
          coreEvents.firstRunEvent(state, side, ':pre-damage-flag',
...
export const vladisibirskCityGrid: CardDef = {
  ...
  abilities: [{
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      coreProps.addProp(state, side, eid, targets[0], ':advance-counter', 2, { placed: true });
    }),
  }],
};
...
export const vovôOzetti: CardDef = {
  ...
  'static-abilities': [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const t = targets[0];
      if (!t) return false;
      const isIceOrThreat = coreCard.ice(t) || coreThreat.threatLevel(4, state);
      const sameSrv = JSON.stringify(coreBoard.cardToServer(state, card)) ===
  ...
  events: [{
    ...
    optional: {
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        if (!forms.thisServer(state, card)) return false;
        const installed = coreBoard.allActiveInstalled(state, ':runner') || [];
        return installed.some((c: Card) => coreCard.hasSubtype(c, 'Icebreaker'));
      }),
      'waiting-prompt': true,
      prompt: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      ...
      'yes-ability': {
        ...
        effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          const target: any = targets[0];
          yield wait_for(state, [{ asyncResult: 'result' },
            coreMoving.trash(state, side, coreEid.makeEid(state, eid), card, { causeCard: card })], []);
          yield wait_for(state, [{ asyncResult: 'result' },
...
export const giordanoMemorialField: CardDef = {
  ...
  events: [{
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      const canPay = corePayment.canPay(state, ':runner', eid, card, null, [corePayment.toC('credit', creditCost)]);
      yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
        player: ':runner',
        async: true,
        'waiting-prompt': true,
        prompt: 'Choose one',
        choices: [...(canPay ? [`Pay ${creditCost} [Credits]`] : []), 'End the run'],
        msg: msg((s: State, sd: Side, e: EID, c: Card, ts: any[]) => {
          const target: any = ts?.[0];
        ...
        effect: effect(function*(s: State, sd: Side, e: EID, c: Card, ts: any[]): Generator<any, any, any> {
          const target: any = ts?.[0];
          if (target === 'End the run') {
            yield wait_for(s, [{ asyncResult: 'result' }, coreRuns.endRun(s, ':corp', e, c)], []);
          } else {
            yield wait_for(s, [{ asyncResult: 'result' },
              corePayment.pay(s, ':runner', coreEid.makeEid(s, e), c, [corePayment.toC('credit', creditCost)])], []);
            yield wait_for(s, [{ asyncResult: 'result' }, coreEffects.effectCompleted(s, sd, e)], []);
          }
        }),
      }, card, null)], []);
    }),
...
export const jinjaCityGrid: CardDef = (() => {
  const installIce = (ice: Card, ices: Card[], grids: Card[], server: string): any => {
    ...
    return {
      ...
      effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
        if (server === 'None') {
          yield wait_for(state, [{ asyncResult: 'result' },
            continue_ability(state, side, chooseIce(remaining, grids), card, null)], []);
        } else {
          yield wait_for(state, [{ asyncResult: 'result' },
            coreRevealing.reveal(state, side, ice)], []);
          coreSay.systemMsg(state, side, `reveals that they drew ${ice.title}`);
          yield wait_for(state, [{ asyncResult: 'result' },
            (coreInstalling as any).corpInstall?.(state, side, coreEid.makeEid(state, eid),
              ice, server, { 'cost-bonus': -4, 'msg-keys': { 'install-source': card, known: true, 'display-origin': true } })], []);
  ...
  const chooseGrid = (ice: Card, ices: Card[], grids: Card[]): any => {
    if (grids.length === 1) {
      ...
      return installIce(ice, ices, grids, coreServers.zoneToName(zone?.[1]) ?? '');
    }
    return {
      async: true,
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = targets[0];
        continue_ability(state, side, installIce(ice, ices, grids, target), card, null);
      }),
    };
  };
    ...
    return {
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        if (target !== 'None') {
          ...
          if (picked) {
            continue_ability(state, side, chooseGrid(picked, ices, grids), card, null);
          }
        }
      }),
    };
    ...
    events: [
      {
        ...
        req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          ...
          const cids = Object.values(trashList).flat().map((c: any) => c?.cid);
          return !cids.includes(card.cid);
        }),
        effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          const drawing: any[] = (state as any).corp?.['currently-drawing'] || [];
          const someIce = drawing.some((c: Card) => coreCard.ice(c));
          if (someIce) {
            const ices = drawing.filter((c: Card) => coreCard.ice(c) && (coreEngine as any).getCard?.(state, c));
            const installed = coreBoard.allActiveInstalled(state, ':corp') || [];
            const grids = installed.filter((c: Card) => c.title === card.title);
            if (ices.length > 0) {
              yield wait_for(state, [{ asyncResult: 'result' },
                continue_ability(state, side, chooseIce(ices, grids), card, null)], []);
            }
          } else {
            yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, ':corp', {
...
export const tucana: CardDef = (() => {
  const ability: any = {
    optional: {
      ...
      'waiting-prompt': true,
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        JSON.stringify(targets[0]?.card?.previousZone) === JSON.stringify(coreCard.getZone(card))),
      'yes-ability': {
        async: true,
        ...
        effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          ...
          yield wait_for(state, [{ asyncResult: 'result' },
            (coreInstalling as any).corpInstall?.(state, side, coreEid.makeEid(state, eid),
              target, null, { 'install-state': ':rezzed', 'combined-credit-discount': 3,
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const lingering: any = {
          ...
          optional: {
            ...ability.optional,
            req: req((s: State, sd: Side, e: EID, c: Card, ts: any[]) =>
              JSON.stringify((c as any).previousZone) === JSON.stringify(ts[0]?.card?.previousZone)),
          },
        };
...
export const warroidTracker: CardDef = (() => {
  const wt = (n: number): any => ({
    ...
    interactive: req(() => true),
    player: ':runner',
    choices: { all: true, max: n, card: (c: Card) => coreCard.runner(c) && coreCard.installed(c) },
    msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      `force the Runner to trash ${(coreCard as any).enumerateCards?.(targets) ??
  ...
  const ability = (): any => ({
    trace: {
      ...
      successful: {
        async: true,
        msg: msg((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const n = Math.min(2, (coreBoard.allInstalled(state, ':runner') || []).length);
    ...
    events: [{
      ...
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        (targets as any[]).some((t: any) => {
          ...
          const warroidZone = coreCard.getZone(card) as string[] | undefined;
...
export const zatoCityGrid: CardDef = {
  ...
  'static-abilities': [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const t = targets[0];
      return t && coreServers.protectingSameServer(card, t) && !(t as any).disabled;
    }),
    value: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => ({
      async: true,
      ...
      optional: {
        ...
        'yes-ability': {
          ...
          effect: effect(function*(s: State, sd: Side, e: EID, c: Card, ts: any[]): Generator<any, any, any> {
            ...
            const printedSubs = (targetIce?.subroutines || []).filter((sb: any) => sb?.printed);
            yield wait_for(s, [{ asyncResult: 'result' }, continue_ability(s, sd,
              printedSubs.length > 0
                ? {
                    prompt: 'Choose a subroutine to resolve',
                    choices: req((s2: State) =>
                      (coreIce as any).unbrokenSubroutinesChoice?.(targetIce) || []),
                    cost: [corePayment.toC('trash-can')],
                    msg: msg((s2: State, sd2: Side, e2: EID, c2: Card, ts2: any[]) =>
                      `resolve ("[Subroutine] ${ts2[0]}")`),
                    async: true,
                    effect: effect(function*(s2: State, sd2: Side, e2: EID, c2: Card, ts2: any[]): Generator<any, any, any> {
                      const sub = (targetIce?.subroutines || []).find((sb: any) =>
                        ts2[0] === ((coreIce as any).makeLabel?.(sb?.['sub-effect']) ?? ''));
                      if (sub) {
                        yield wait_for(s2, [{ asyncResult: 'result' },
                          (coreIce as any).resolveSubroutine?.(s2, sd2, e2, targetIce,
...
export const tranquilityHomeGrid: CardDef = {
  ...
  events: [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      const c = ctx?.card;
      if (!c) return false;
      const isAAU = coreCard.asset(c) || coreCard.agenda(c) || coreCard.upgrade(c);
      if (!isAAU || !coreServers.inSameServer(card, c)) return false;
      return coreEvents.firstEvent(state, ':corp', ':corp-install',
        (ts: any[]) => ts?.[0]?.card && coreServers.inSameServer(card, ts[0].card));
    }),
    prompt: 'Choose one',
    'waiting-prompt': true,
...
export const laCostaGrid: CardDef = (() => {
  const ability: any = {
    ...
    choices: { req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const t = targets[0];
      return t && coreCard.installed(t) && coreServers.inSameServer(card, t);
    }) },
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      coreProps.addProp(state, side, eid, targets[0], ':advance-counter', 1, { placed: true });
      ...
      'not-equal': {
        optional: {
          ...
          'yes-ability': {
            ...
            effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
              yield wait_for(state, [{ asyncResult: 'result' },
                coreMoving.trash(state, side, coreEid.makeEid(state, eid), card, { unpreventable: true, causeCard: card })], []);
              const zone = coreCard.getZone(card) as string[] | undefined;
              coreRuns.redirectRun(state, side, coreServers.zoneToName(zone?.[1]) ?? '', ':approach-ice');
              yield wait_for(state, [{ asyncResult: 'result' },
                continue_ability(state, ':runner', coreDefHelpers.offerJackOut(), card, null)], []);
            }),
          },
...
export const mahkotaLangitGrid: CardDef = {
  ...
  'static-abilities': [{
    ...
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const t = targets[0];
      return t && coreCard.installed(t) && coreCard.asset(t) &&
        coreServers.sameServer(card, t);
    }),
    value: 2,
  }],
...
export const malapertDataVault: CardDef = {
  ...
  events: [{
    ...
    optional: {
      ...
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const ctx = targets[0];
        return JSON.stringify(ctx?.card?.previousZone) === JSON.stringify(coreCard.getZone(card));
      }),
      'yes-ability': {
...
export const manegarmSkunkworks: CardDef = {
  ...
  events: [{
    ...
    choices: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const opts: string[] = [];
      if (corePayment.canPay(state, ':runner', eid, card, null, [corePayment.toC('click', 2)])) {
        opts.push('Spend [Click][Click]');
      }
      if (corePayment.canPay(state, ':runner', eid, card, null, [corePayment.toC('credit', 5)])) {
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      const target: any = (targets as any[])?.[0];
      if (target === 'Spend [Click][Click]' &&
          corePayment.canPay(state, ':runner', eid, card, null, [corePayment.toC('click', 2)])) {
        yield wait_for(state, [{ asyncResult: 'result' },
          corePayment.pay(state, side, coreEid.makeEid(state, eid), card, [corePayment.toC('click', 2)])], []);
        coreEffects.effectCompleted(state, ':runner', eid);
      } else if (target === 'Pay 5 [Credits]' &&
          corePayment.canPay(state, ':runner', eid, card, null, [corePayment.toC('credit', 5)])) {
        yield wait_for(state, [{ asyncResult: 'result' },
          corePayment.pay(state, side, coreEid.makeEid(state, eid), card, [corePayment.toC('credit', 5)])], []);
        coreEffects.effectCompleted(state, ':runner', eid);
      } else {
        coreSay.systemMsg(state, ':corp', `uses ${card.title} to end the run`);
...
export const mavirus: CardDef = (() => {
  const resolvePurge = {
    ...
    async: true,
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      yield wait_for(state, [{ asyncResult: 'result' }, corePurging.purge(state, side, eid)], []);
            ...
            continue_ability(state, side, {
              optional: {
                ...
                'no-ability': {
                  async: true,
                  effect: effect(function*(s: State, sd: Side, e: EID, c: Card, ts: any[]): Generator<any, any, any> {
                    coreSay.systemMsg(s, ':corp', `declines to use ${c.title}`);
                    if (coreCard.rezzed(c)) {
                      coreSay.systemMsg(s, sd, `uses ${c.title} to do 1 net damage`);
...
export const midori: CardDef = {
  ...
  events: [{
    event: ':approach-ice',
    'change-in-game-state': { req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      ((state as any).corp?.hand?.length ?? 0) > 0) },
    optional: {
      req: req(forms.thisServer),
      once: ':per-run',
      ...
      'yes-ability': {
        ...
        effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          ...
          yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
            ...
            effect: effect((s: State, sd: Side, e: EID, c: Card, ts: any[]) => {
              const cur = coreIce.getCurrentIce(s);
              (coreCard as any).swapCardsAsync?.(s, sd, e, cur, ts[0]);
            }),
          }, card, null)], []);
        }),
      },
...
export const mumbadCityGrid: CardDef = {
  ...
  events: [{
    event: ':pass-ice',
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      forms.thisServer(state, card) && (coreIce.getRunIces(state)?.length ?? 0) >= 2),
    async: true,
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      const ctx = targets[0];
      const passedIce = ctx?.ice;
      yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
        prompt: msg((s: State, sd: Side, e: EID, c: Card, ts: any[]) =>
          `Choose a piece of ice to swap with ${passedIce?.title}`),
        choices: { req: req((s: State, sd: Side, e: EID, c: Card, ts: any[]) => {
          const t = ts[0];
          const run = forms.run(s) as any;
          const tZone = coreCard.getZone(t) as string[] | undefined;
          return t && coreCard.installed(t) && coreCard.ice(t) &&
            coreServers.targetServer({ server: run?.server }) === tZone?.[1] &&
  ...
  'on-access': {
    ...
    trace: {
      ...
      successful: {
        ...
        choices: { card: (c: Card) =>
          coreCard.installed(c) && (coreCard.program(c) ||
...
export const selfDestruct: CardDef = {
  ...
  abilities: [{
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      yield wait_for(state, [{ asyncResult: 'result' },
        coreMoving.trashCards(state, side, coreEid.makeEid(state, eid), cards, { causeCard: card })], []);
      yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
        trace: {
          base: cards.length,
          successful: {
...
export const hiredHelp: CardDef = (() => {
  const promptToTrashOrEtr: any = {
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      const target: any = targets[0];
      if (target === 'End the run') {
        coreSay.systemMsg(state, ':runner', `declines to pay the additional cost from ${card.title}`);
        yield wait_for(state, [{ asyncResult: 'result' }, coreRuns.endRun(state, side, eid, card)], []);
      } else {
        const scored = (state as any).runner?.scored || [];
        if (scored.length > 0) {
          yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, ':runner', {
            prompt: 'Choose an Agenda to trash',
            async: true,
            choices: { max: 1, card: (c: Card) => coreCard.inScored?.(c) },
            effect: effect(function*(s: State, sd: Side, e: EID, c: Card, ts: any[]): Generator<any, any, any> {
              const t: any = ts[0];
              yield wait_for(s, [{ asyncResult: 'result' },
                coreMoving.trash(s, sd, e, t, { unpreventable: true, causeCard: c, cause: ':forced-to-trash' })], []);
              coreSay.systemMsg(s, ':runner', `trashes ${t?.title} as an additional cost to initiate a run`);
              coreEffects.effectCompleted(s, sd, e);
            }),
          }, card, null)], []);
        } else {
          coreSay.systemMsg(state, ':runner', `cannot pay the additional cost from ${card.title}`);
          yield wait_for(state, [{ asyncResult: 'result' }, coreRuns.endRun(state, side, eid, card)], []);
        }
      }
    }),
  ...
  return {
    ...
    events: [{
      ...
      async: true,
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const reg = (state as any).runner?.['runner-reg']?.['successful-run'] || [];
...
export const marcusBatty: CardDef = {
  ...
  abilities: [{
    ...
    psi: {
      ...
      'not-equal': {
        prompt: 'Choose a piece of ice',
        choices: { card: (c: Card) => coreCard.ice(c) && coreCard.rezzed(c), all: true },
        async: true,
        effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          ...
          yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
            ...
            effect: effect(function*(s: State, sd: Side, e: EID, c: Card, ts: any[]): Generator<any, any, any> {
              ...
              const sub = subs.find((sb: any) =>
                ts[0] === ((coreIce as any).makeLabel?.(sb?.['sub-effect']) ?? ''));
              if (sub) {
                yield wait_for(s, [{ asyncResult: 'result' },
                  (coreIce as any).resolveSubroutine?.(s, sd, e, ice,
                    { ...sub, 'external-trigger': true })], []);
              }
            }),
          }, card, null)], []);
...
export const isaacLiberdade: CardDef = (() => {
  const ability: any = {
    ...
    choices: { req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const t = targets[0];
      return t && coreCard.ice(t) && coreCard.getCounters(t, ':advancement') === 0 &&
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      ...
      yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
        ...
        effect: effect(function*(s: State, sd: Side, e: EID, c: Card, ts: any[]): Generator<any, any, any> {
          ...
          if (sub?.['sub-effect']) {
            yield wait_for(s, [{ asyncResult: 'result' },
              continue_ability(s, sd, sub['sub-effect'], target, null)], []);
          }
        }),
      }, card, null)], []);
    }),
...
export const theHoloMan: CardDef = (() => {
  const isBoosted = (state: State, side: Side): boolean =>
    coreEvents.noEvent(state, side, ':corp-install',
      (ts: any[]) => JSON.stringify(ts?.[0]?.card?.previousZone) === JSON.stringify(['hand']));
  const abi: any = {
    action: true,
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const n = isBoosted(state, side) ? 3 : 2;
      coreProps.addProp(state, side, eid, targets[0], ':advance-counter', n, { placed: true });
    }),
  };
  return {
    title: 'The Holo Man',
...
export const flagship: CardDef = (() => {
  const otherCardsAccessed = (state: State, card: Card): string[] => {
    const runEvents: any[] = (coreEvents as any).runEvents?.(state, ':runner', ':access') || [];
    return runEvents.flat()
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        coreEffects.registerLingeringEffect(c, {
          ...
          req: req((s: State) => {
            ...
            return !!run && JSON.stringify(run?.server) === JSON.stringify([zone?.[1]]) &&
  ...
  'on-access': {
    optional: {
      ...
      'yes-ability': {
        ...
        effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          const installed = coreBoard.allActiveInstalled(state, ':corp') || [];
          const hasIce = installed.some((c: Card) =>
            coreCard.ice(c) && coreCard.rezzed(c) && coreServers.protectingSameServer(card, c));
          if (hasIce) {
            yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
              async: true,
              choices: { req: req((s: State, sd: Side, e: EID, c: Card, ts: any[]) => {
                const t = ts[0];
                return t && coreCard.ice(t) && coreCard.installed(t) && coreCard.rezzed(t) &&
              ...
              effect: effect(function*(s: State, sd: Side, e: EID, c: Card, ts: any[]): Generator<any, any, any> {
                ...
                coreEngine.registerEvents(c, [{
                  ...
                  async: true,
                  effect: effect((s2: State, sd2: Side, e2: EID, c2: Card, ts2: any[]) => {
                    coreRuns.forceIceEncounter(s2, sd2, e2, targetCard);
                  }),
                }]);
                yield wait_for(s, [{ asyncResult: 'result' },
                  coreMoving.trash(s, sd, e, { ...c, seen: true }, { unpreventable: true, causeCard: c })], []);
              }),
            }, card, null)], []);
          } else {
            yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
...
export const merciaB4ll4Rd: CardDef = {
  ...
  events: [{
    ...
    effect: effect(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      const target: any = targets[0];
      const movedCard: any = yield wait_for(state, [{ asyncResult: 'result' },
        (coreInstalling as any).corpInstall?.(state, side, coreEid.makeEid(state, eid),
          target, null, { 'cost-bonus': -1, 'msg-keys': { 'install-source': card } })], []);
        ...
        if (!coreServers.sameServer(movedCard, card)) {
          yield wait_for(state, [{ asyncResult: 'result' }, continue_ability(state, side, {
            ...
            effect: effect((s: State, sd: Side, e: EID, c: Card, ts: any[]) => {
              ...
              const moved = (coreMoving as any).move(s, sd, c, targetZone);
...
export const mwanzaCityGrid: CardDef = (() => {
  ...
  const boostAccessWhenTrashed = (bonusServer: string): any => ({
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      (coreAccess as any).accessBonus?.(state, ':runner', bonusServer, 3);
      coreEngine.registerEvents(card, [mwanzaGainCreds, unboostAccess(bonusServer)]);
    }),
  });
  ...
  return {
    ...
    'on-trash': {
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        if (bonusServer) {
          coreEngine.registerEvents(card, [boostAccessWhenTrashed(bonusServer)]);
        }
      }),
    },
  };
...
export const nihongaiGrid: CardDef = {
  ...
  events: [{
    ...
    optional: {
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        if (!forms.thisServer(state, card)) return false;
        const runnerCreds = (corePayment as any).totalAvailableCredits?.(state, ':runner', eid, card) ?? 0;
        ...
        return (runnerCreds < 6 || runnerHand < 2) && corpHand.length > 0 && top5.length > 0;
      }),
            ...
            effect: effect((s: State, sd: Side, e: EID, c: Card, ts: any[]) => {
              ...
              if (rdc) {
                continue_ability(s, sd, {
                  ...
                  choices: { card: (cc: Card) => coreCard.inHand(cc) },
                  msg: 'swap a card from the top 5 of R&D with a card in HQ',
                  effect: effect((s2: State, sd2: Side, e2: EID, c2: Card, ts2: any[]) => {
                    const hq: any = ts2[0];
                    (coreMoving as any).move(s2, sd2, rdc, 'hand');
                    (coreMoving as any).move(s2, sd2, hq, 'deck', { index: rdc?.index });
                  }),
                }, c, null);
              }
            }),
...
export const overseerMatrix: CardDef = (() => {
  const ability: any = {
    ...
    interactive: req(() => true),
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      (targets as any[]).some((t: any) => coreCard.corp(t?.card) &&
        (coreServers.inSameServer(card, t.card) || coreServers.fromSameServer(card, t.card)))),
    'waiting-prompt': true,
    prompt: 'How many credits do you want to pay?',
    choices: { number: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const prev = (card as any).previousZone;
      const matches = (targets as any[]).filter((t: any) =>
      ...
      const totalCreds = (corePayment as any).totalAvailableCredits?.(state, ':corp', eid, card) ?? 0;
  ...
  return {
    ...
    'on-trash': {
      ...
      effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        if (forms.run(state)) {
          coreEngine.registerEvents(card, [{ ...ability, duration: ':end-of-run' }]);
        }
      }),
    },
    events: [ability],
  ...
  events: [{
    ...
    optional: {
      req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const ctx = targets[0];
        const t = ctx?.card;
        if (!t || !coreServers.sameServer(card, t) || coreCard.sameCard(t, card)) return false;
        const installed = coreBoard.allInstalled(state, ':corp') || [];
        return installed.some((c: Card) =>
          !coreCard.rezzed(c) && !coreCard.agenda(c) && coreCard.corp(c) &&
          coreRezzing.canPayToRez(state, side, { ...eid, source: card }, c, { 'cost-bonus': -2 }));
      }),
      ...
      'yes-ability': {
        ...
        choices: { req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const t = targets[0];
          return t && !coreCard.rezzed(t) && !coreCard.agenda(t) && coreCard.corp(t) &&
            coreCard.installed(t) &&
            coreRezzing.canPayToRez(state, side, { ...eid, source: card }, t, { 'cost-bonus': -2 });
        }) },
        async: true,
        effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreRezzing.rez(state, side, { ...eid, source: card }, targets[0], { 'cost-bonus': -2 });
        }),
      },
    },
  }],
