    ...
    "static-abilities": [
      ...(cdef["static-abilities"] || []),
      {
        ...
        req: req(function* (
          ...
          return getLink(forms.state) <= -2;
        }), // <= 2 link means -2 + link <= 0
        value: req(function* (
          state: State,
          side: Side,
          eid: EID,
      ...
      "static-abilities": [
        {
          ...
          req: req(function* (
            ...
            return allActiveInstalled(state, ":runner").filter((c: Card) =>
...
function unregisterEffect(uuid: string): void {
  coreEffects.unregisterEffectByUuid?.(
    undefined as any,
    undefined as any,
    uuid,
  );
...
export const aumakua: CardDef = {
  ...
  ...autoIcebreaker({
    implementation:
      "[Erratum] Whenever you finish breaching a server, if you did not steal or trash any accessed cards, place 1 virus counter on this program.",
