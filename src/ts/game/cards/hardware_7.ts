  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        const pred = (event: any[]) =>
          event.some(
            (t: any) =>
              runnerFn(t.card) && installedFn(t.card) && programFn(t.card),
          );
        return (
          sameCard(card, (forms.context(state, card, targets) as any)?.card) &&
          eventCountFn(state, null, "runner-trash", pred) +
            eventCountFn(state, null, "corp-trash", pred) +
            eventCountFn(state, null, "game-trash", pred) ===
            0
        );
      }),
      value: [toC(":program", 1)],
  ...
  events: [
    ...
    },
    {
      ...
      req: req(function* (
        ...
        return (
          targets.some((t: any) => corpFn(t.card)) &&
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          addCounterFn(":runner", eid, card, "power", 1);
        },
      ),
    },
  ],
  ...
  interactions: {
    "pay-credits": {
      req: req(function* (
        ...
        const t = target;
        return (
          eid["source-type"] === ":ability" && hasSubtypeFn(t, "Icebreaker")
        );
      }),
      type: ":recurring",
    },
  ...
  abilities: [
    {
      ...
      effect: req(function* (
        ...
        const n = allActiveInstalledFn(state, ":runner").filter(
...
export const supercorridor: CardDef = {
  title: "Supercorridor",
  "static-abilities": [muPlusFn(2), runnerHandSizePlusFn(1)],
  events: [
    {
      ...
      effect: req(function* (
        ...
        const relevant = evs.filter((ev: any) => {
          ...
          return (
            ...
            (coreBoard.getZone?.(t)?.[0] === ":hq" || ctx.ice === ":hq")
          );
        });
...
export const thePersonalTouch: CardDef = {
  ...
  hosting: {
    card: (c: Card) => hasSubtypeFn(c, "Icebreaker") && installedFn(c),
  },
  "on-install": {
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        updateBreakerStrengthFn(getCardFn(state, card));
      },
    ),
  },
  "static-abilities": [
...
export const theToolbox: CardDef = {
  title: "The Toolbox",
  "static-abilities": [muPlusFn(2), linkPlusFn(2)],
  recurring: 2,
  interactions: {
    "pay-credits": {
      req: req(function* (
  ...
  events: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          gainCreditsFn(state, side, eid, 1);
        },
      ),
    },
  ],
  ...
  let: {
    searchFn: (
      ...
      if (remainder.length > 0) {
        ...
        const newRevStr = revStr
          ? `${revStr}, ${revealedCard.title}`
          : revealedCard.title;
        ...
        if (isType) {
          if (!firstCard) {
            return theWizardsChest.let.searchFn(
              state,
              side,
              eid,
              card,
          ...
          return continue_ability(
            ...
            side,
            {
              msg: (
                ...
                card: Card,
                targets: any[],
              ) => `reveal ${revStr} from the top of the stack`,
              effect: effect(
                (
                  state: State,
                  side: Side,
              ...
              yield wait_for(
                ...
                [
                  ...
                  runnerInstallFn(
                    ...
                    target === `Install ${firstCard.title}`
                      ? firstCard
                      : secondCard,
                    {
  ...
  abilities: [
    {
      ...
      req: req(function* (
        ...
        return (
          (reg.successfulRun || []).some((s: any) => s === ":hq") &&
...
export const timeBomb: CardDef = {
  ...
  req: req(function* (
    ...
    const runner = runnerFn(state);
    const reg = runner?.reg || {};
    return (reg.successfulRun || []).some((s: any) =>
      ...
      effect: req(function* (
        ...
        if (getCounters(getCardFn(state, card), "power") >= 3) {
          ...
          systemMsg(
            state,
            side,
            `uses ${card.title} to place 1 power counter on itself`,
          );
...
export const titaniumRibs: CardDef = {
  ...
  "leave-play": req(function* (
    ...
    coreUpdate.updateIn(state, ["damage"], (d: any) => {
      if (d) delete d["damage-choose-runner"];
      return d;
    });
  }),
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          !(getDamageFn(state) as Record<string, unknown> | undefined)?.["damage-replace"]
        );
      }),
      effect: req(function* (
        state: State,
                ...
                hand.length < dmg
                  ...
                  : {
                      ...
                      choices: {
                        max: dmg,
                        all: true,
                        card: (c: Card) => inHandFn(c) && runnerFn(c),
                      },
                      msg: (
                        state: State,
                        side: Side,
                        ...
                        card: Card,
                        targets: any[],
                      ) => `trash ${enumerateCards(targets, ":sorted")}`,
  ...
  events: [
    successfulRunReplaceBreach({
      ...
      ability: {
        ...
        prompt:
          "Which card from the top of R&D would you like to access? (Card 1 is on top)",
        choices: req(function* (
          state: State,
          side: Side,
          eid: EID,
          ...
          const corp = corpFn(state);
          return Array.from(
            { length: Math.min(corp?.deck?.length || 0, 5) },
            (_, i) => String(i + 1),
          );
        }),
        msg: (
          ...
          return ((): string => {
            const target: any = (targets as any[])?.[0];
            return `only access the card at position ${target} of R&D`;
          })();
        },
        async: true,
        effect: req(function* (
  ...
  events: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          addCounterFn(state, side, eid, card, "credit", 1);
        },
      ),
    },
  ],
          ...
          continue_ability(
            {
              ...
              prompt: (
                ...
                `Swap ${(forms.context(state, card, targets) as any)?.card?.title || "stolen agenda"} for an agenda in the Corp's score area?`,
              "yes-ability": {
                ...
                msg: (
                  ...
                  return ((): string => {
                    const target: any = (targets as any[])?.[0];
                    return `swap ${(forms.context(state, card, targets) as any)?.card?.title || "stolen"} for ${target.title}`;
                  })();
                },
                effect: effect(
                  swapAgendasFn(
...
export const ubax: CardDef = {
  ...
  flags: {
    ...
    "runner-phase-12": req(function* (
      ...
      return (
        cards.filter((c: Card) => cardFlagFn(c, ":runner-turn-draw", true))
          .length > 1
      );
    }),
  },
  ...
  abilities: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const target: any = (targets as any[])?.[0];
          trashFn(eid, target, { causeCard: card });
        },
      ),
    },
  ],
      ...
      effect: req(function* (
        ...
        if (ctx.server?.[0] === ":hq") {
          systemMsg(
            state,
            side,
            `uses ${card.title} to access 1 additional card from HQ this run`,
          );
          ...
          systemMsg(
            state,
            side,
            `will use ${card.title} to breach HQ when this run ends`,
          );
          registerEventsFn(state, side, card, [
            {
              event: "run-ends",
              ...
              effect: effect(
                (
                  ...
                  breachServerFn(state, ":runner", eid, [":hq"], null);
                },
              ),
            },
          ]);
  ...
  events: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          addCounterFn(state, ":runner", eid, card, "power", 1, {
            placed: true,
          });
        },
      ),
        ...
        continue_ability(
          ...
          side,
          {
            ...
            msg: (
              ...
              const target: any = (targets as any[])?.[0];
              return `access ${quantify(target, "additional card")} from R&D`;
            },
            "waiting-prompt": true,
            async: true,
            effect: req(function* (
              ...
              addCounterFn(state, ":runner", eid, card, "power", -target, {
                placed: true,
              });
            }),
          },
  ...
  abilities: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          moveFn((runnerFn(state)?.deck || []).slice(-1)[0], ":hand");
        },
      ),
    },
  ],
  ...
  abilities: [
    {
      ...setAutoresolveFn(
        "auto-gain-credits",
        "Zamba gaining credits on expose",
      ),
    },
  ],
  events: [
    {
      event: "expose",
      interactive: getAutoresolveFn(
        "auto-gain-credits",
        complementFn(neverFn) as any,
      ),
      ...
      optional: {
        ...
        prompt: (
          ...
          `Gain ${(forms.context(state, card, targets) as any)?.cards?.length || 0} [Credits]?`,
        ...
        "yes-ability": {
          ...
          effect: effect(
            (
              ...
              gainCreditsFn(
                eid,
                (forms.context(state, card, targets) as any)?.cards?.length ||
                  0,
              );
            },
          ),
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const ctx = forms.context(state, card, targets) || {};
        return (
          isCentralFn(ctx.server) &&
          firstEventFn(state, side, "successful-run", (t: any[]) => {
            const c = t[0];
            return c && isCentralFn(c.server);
          })
        );
      }),
      msg: "draw 1 card",
