...
export const keyPerformanceIndicators: CardDef = {
  ...
  onPlay: coreChooseOne.chooseOneHelper({ count: 2, optional: true }, [
    {
      option: "Gain 2 [Credit]",
      ability: coreDefHelpers.gainCreditsAbility(2),
    },
    {
      option: "Install 1 piece of ice from HQ, ignoring all costs",
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          (state as any).corp?.hand?.some(coreCard.ice),
      ),
      ability: {
        choices: {
          card: (c: Card) =>
    ...
    },
    {
      ...
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreBoard.allInstalled(state, "corp").some(coreCard.canBeAdvanced),
      ),
      ability: {
        choices: {
          req: req(
    ...
    },
    {
      ...
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          (state as any).corp?.hand?.length >= 1,
      ),
      ability: {
        msg: "draw 1 card",
        async: true,
...
export const killSwitch: CardDef = {
  ...
  events: [
    {
      msg: msg(
        "reveal that they accessed ",
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          (state as any)?.context?.card?.title,
      ),
      trace: {
        base: 3,
        req: req(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            coreCard.agenda(targets[0]?.card) ||
            coreCard.agenda(targets[0]?.accessedCard),
        ),
        successful: {
          ...
          effect: effect(
            (
              ...
              coreDamage.damage("runner", eid, "brain", 1, { card });
            },
          ),
        },
      },
  ...
  onPlay: {
    ...
    choices: {
      max: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreBoard
            .allActiveInstalled(state, "corp")
            .filter((c: Card) => !coreCard.agenda(c)).length,
      ),
    ...
    onChangeGameState: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreBoard.allInstalled(state, "corp").some(coreCard.rezzed),
      ),
    },
    msg: msg(
      "trash ",
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        coreGaining.gainCredits(state, side, eid, targets.length * 3);
      },
    ),
  },
};
...
export const loadTesting: CardDef = {
  title: "Load Testing",
  onPlay: {
    msg: "make the Runner lose [Click] when [runner-pronoun] next turn begins",
  },
  events: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreGaining.loseClicks("runner", 1);
        },
      ),
    },
  ],
...
export const localizedProductLine: CardDef = {
  ...
  onPlay: {
    ...
    choices: req(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        corePrompts.cancellable((state as any).corp?.deck || [], {
          sorted: true,
        }),
    ),
    onChangeGameState: {
  ...
  events: [
    {
      ...
      trace: {
        req: req(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            coreEvents.firstEvent(state, side, "successful-run"),
        ),
        base: 2,
  ...
  onPlay: {
    ...
    onChangeGameState: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreBoard
            .getAllInstalled(state)
            .filter((c: Card) => (c.counters?.advancement || 0) > 0).length > 0,
      ),
    },
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const cards = coreBoard.getAllInstalled(state);
        return coreGaining.gainCredits(
          eid,
          cards.filter((c: Card) => (c.counters?.advancement || 0) > 0).length *
            2,
        );
      },
    ),
...
export const mcaInformant: CardDef = {
  ...
  onPlay: {
      ...
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreBoard
            ...
            .some((c: Card) => coreCard.hasSubtype(c, "Connection")),
      ),
    },
    choices: {
      card: (c: Card) =>
    ...
    msg: msg(
      ...
      ". The Runner has an additional tag",
    ),
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        ability: {
          ...
          effect: effect(
            (
              ...
              coreDamage.damage("corp", eid, "meat", 4);
            },
          ),
        },
      },
...
export const mediaBlitz: CardDef = {
  ...
  onPlay: {
    ...
    choices: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreCard.agenda(targets[0]) &&
          coreFlags.isScored(state, "runner", targets[0]),
      ),
    },
    onChangeGameState: {
      req: req(
...
export const medicalResearchFundraiser: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreGaining.gainCredits(state, side, 8);
...
export const midseasonReplacements: CardDef = {
  ...
  onPlay: {
    trace: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          (state as any).runner?.register?.lastTurn?.stoleAgenda,
      ),
      base: 6,
      ...
      successful: {
        msg: msg(
          "give the Runner ",
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            utils.quantify(targets[0] - targets[1], "tag"),
        ),
        async: true,
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            coreTags.gainTags(eid, targets[0] - targets[1]);
          },
        ),
      },
    },
    ...
    },
    {
      ...
      ability: {
        ...
        msg: msg(
          "do ",
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            Math.min(3, utils.countTags(state)),
          " net damage",
        ),
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
...
export const mushinNoShin: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const installedCard = coreInstalling.corpInstall(
          state,
          side,
          eid,
          targets[0],
        ...
        if (installedCard) {
          coreFlags.registerPersistentFlag(
            ...
            installedCard,
            "can-rez",
            (s: State, _sd: Side, c: Card) => {
              if (utils.sameCard(c, installedCard)) {
                coreToasts.toast(
                  s,
                  "corp",
                  "Cannot rez due to Mushin No Shin.",
                );
                return false;
              }
              return true;
            ...
            (s: State, _sd: Side, c: Card) => {
              if (utils.sameCard(c, installedCard)) {
                coreToasts.toast(
                  s,
                  "corp",
                  "Cannot score due to Mushin No Shin.",
                );
                return false;
              }
              return true;
          ...
          coreEvents.registerEvents(state, side, installedCard, [
            {
              ...
              async: true,
              effect: effect(
                (s: State, sd: Side, e: EID, c: Card, _t: any[]) => {
                  coreFlags.clearPersistentFlag(
                    s,
                    "corp",
                    installedCard,
                    "can-rez",
...
export const mutuallyAssuredDestruction: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg(
      ...
      " and give the runner ",
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        utils.quantify(targets.length, "tag"),
    ),
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreMoving.trashCards(state, side, targets, { causeCard: card });
        coreTags.gainTags(state, "corp", eid, targets.length);
      },
    ),
  },
};
  ...
  onPlay: {
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      coreEvents.noEvent(
        ...
        side,
        "agenda-scored",
        (t: any) => t[0]?.scoredCard?.installed !== "this-turn",
      ),
    ),
    msg: "add itself to [their] score area as an Agenda worth 2 points",
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreMoving.asAgenda(state, side, card, 2);
      },
    ),
  },
};
  ...
  staticAbilities: [
    {
      ...
      value: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          corePayment.toC(
            "credit",
            4 + 2 * (targets[0]?.counters?.advancement || 0),
          ),
      ),
    },
  ],
...
export const netWatchlist: CardDef = {
  title: "Net Watchlist",
  implementation:
    "Only modifies ability costs, does not adjust non-ability uses",
  staticAbilities: [
    {
      type: "card-ability-additional-cost",
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreCard.hasSubtype(targets[0]?.card, "Icebreaker") &&
          !targets[0]?.ability?.break,
      ),
...
export const neuralEMP: CardDef = {
  ...
  onPlay: {
    req: req(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        (state as any).runner?.register?.lastTurn?.madeRun,
    ),
    msg: "do 1 net damage",
    async: true,
    effect: effect(
...
export const neurospike: CardDef = {
  ...
  onPlay: {
    msg: msg(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `${(state as any).corp?.register?.scoredAgenda?.[0] || 0} net damage`,
    ),
    onChangeGameState: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          (state as any).corp?.register?.scoredAgenda?.[0] > 0,
      ),
    },
    async: true,
    effect: effect(
...
export const nextActivationCommand: CardDef = lockdown({
  ...
  staticAbilities: [
    { type: "ice-strength", value: 2 },
    {
      ...
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          !coreCard.hasSubtype(targets[0], "Icebreaker") && targets[1]?.break,
      ),
      value: true,
    },
  ],
...
export const o2Shortage: CardDef = {
  ...
  onPlay: coreChooseOne.chooseOneHelper({ player: "runner" }, [
    coreChooseOne.costOption(
      [corePayment.toC("randomly-trash-from-hand", 1)],
      "runner",
    ),
    {
      option: "The Corp gains [Click][Click]",
      player: "corp",
      ability: gainNClicks(2),
    },
  ]),
};
          ...
          return continue_ability(
            ...
            side,
            {
              optional: {
                ...
                yesAbility: {
                  async: true,
                  cost: [corePayment.toC("credit", 5)],
                  msg: "give the Runner 2 tags",
                  effect: effect(coreTags.gainTags(state, "corp", eid, 2)),
                },
              },
            },
            card,
  ...
  events: [
    {
      ...
      msg: msg(
        "trash ",
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreToString.cardStr(state, targets[0]?.ice),
      ),
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreMoving.trash("corp", eid, targets[0]?.ice, {
            unpreventable: true,
            causeCard: card,
          });
        },
...
export const pettyCash: CardDef = {
  ...
  onPlay: {
    ...
    async: true,
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      coreEvents.noEvent(state, side, "action-resolved"),
    ),
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreGaining.gainCredits(state, side, 5);
        coreEid.effectCompleted(state, side, eid);
      },
    ),
  },
...
export const pivot: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg(
      ...
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        targets[0]?.title,
      " from R&D and add it to HQ",
    ),
    choices: req(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        [...(state.corp?.deck || [])]
          .sort((a: any, b: any) => a.title.localeCompare(b.title))
          .filter((c: Card) => coreCard.operation(c) || coreCard.agenda(c)),
    ),
    onChangeGameState: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
...
export const predictiveAlgorithm: CardDef = {
  ...
  staticAbilities: [
    { type: "steal-additional-cost", value: corePayment.toC("credit", 2) },
  ],
};
...
export const predictivePlanogram: CardDef = {
  ...
  onPlay: {
    ...
    choices: req(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        [
          "Gain 3 [Credits]",
          "Draw 3 cards",
          utils.isTagged(state) ? "Gain 3 [Credits] and draw 3 cards" : null,
        ].filter(Boolean),
    ),
    msg: msg(
      "choose ",
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        if (targets[0] === "Gain 3 [Credits]")
          return coreGaining.gainCredits(state, "corp", eid, 3);
        if (targets[0] === "Draw 3 cards")
          return coreDrawing.draw(state, "corp", eid, 3);
        if (targets[0] === "Gain 3 [Credits] and draw 3 cards")
          return (
            coreGaining.gainCredits(state, "corp", 3),
            coreDrawing.draw(state, "corp", eid, 3)
          );
...
export const priorityConstruction: CardDef = {
  ...
  onPlay: {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.inHandStar(state, c) && coreCard.corp(c) && coreCard.ice(c),
    },
...
export const productRecall: CardDef = {
  ...
  onPlay: {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.rezzed(c) && (coreCard.asset(c) || coreCard.upgrade(c)),
    },
    msg: msg(
      "trash ",
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      ...
      " and gain ",
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        coreCostFns.trashCost(state, side, targets[0]),
      " [Credits]",
    ),
    async: true,
    effect: effect(
...
export const psychographics: CardDef = {
  ...
  onPlay: {
    onChangeGameState: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          utils.isTagged(state) && corePayment.xCostValue(eid) > 0,
      ),
    },
    waitingPrompt: true,
    basePlayCost: [
      corePayment.toC("x-credits", 0, {
        maximum: req(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
    ...
    msg: msg(
      "place ",
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        utils.quantify(corePayment.xCostValue(eid), " advancement counter"),
      " on ",
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        coreToString.cardStr(state, targets[0]),
    ),
  ...
  onPlay: {
    trace: {
      ...
      successful: {
        ...
        msg: msg(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            `${(state as any).runner?.register?.lastTurn?.stoleAgenda?.[0] || 0} meat damage`,
        ),
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            coreDamage.damage(
              eid,
              "meat",
              (state as any).runner?.register?.lastTurn?.stoleAgenda?.[0] || 0,
              { card },
            );
          },
        ),
...
export const realloc: CardDef = {
  ...
  onPlay: {
    onChangeGameState: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreBoard
            .allInstalled(state, "corp")
            .filter((c: Card) => coreCard.rezzed(c) && coreCard.ice(c)).length >
          0,
      ),
    },
    waitingPrompt: true,
    ...
    msg: msg(
      ...
      " and gain ",
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        targets.reduce((sum: number, c: Card) => sum + (c.cost || 0), 0),
      " [Credits]",
    ),
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
...
export const reanimationProtocol: CardDef = {
  ...
  onPlay: {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.corp(c) && coreCard.ice(c) && coreCard.inDiscard(c),
    },
    async: true,
    waitingPrompt: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreInstalling.corpInstall(state, side, targets[0], null, {
          msgKeys: { installSource: card, displayOrigin: true },
          installState: "rezzed",
          combinedCreditDiscount: 10,
        });
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const installedCard = targets[0];
          if (
            installedCard &&
            coreCard.rezzed(installedCard) &&
            coreCard.hasAnySubtype(installedCard, ["Liability", "Illicit"])
          ) {
            return coreEid.effectCompleted(state, side, eid);
          }
...
export const restore: CardDef = {
  ...
  onPlay: {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.corp(c) && !coreCard.operation(c) && coreCard.inDiscard(c),
    },
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
...
export const restoringFace: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg(
      ...
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        targets[0]?.title,
      " to remove 2 bad publicity",
    ),
    choices: {
      card: (c: Card) =>
        coreCard.hasAnySubtype(c, ["Clone", "Executive", "Sysop"]),
    },
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreBadPublicity.loseBadPublicity(state, side, 2);
