  ...
  events: [
    {
      event: ":agenda-scored",
      prompt:
        "Choose a card from Archives or HQ to install, ignoring all costs",
      "show-discard": true,
        ...
        yield wait_for(
          ...
          [
            ...
            coreGaining.gainCredits(
              ...
              side,
              eid,
              2 * coreCard.getCounters(card, ":advancement"),
            ),
          ],
          [],
        );
...
export const toshiyukiSakai: CardDef = {
  ...
  ...advanceAmbush(0, {
    ...
    effect: req(function* (
      ...
      const [movedCard, movedTarget] = coreInstalling.swapCards(
        state,
        side,
        card,
        targets[0],
                ...
                "yes-ability": {
                  ...
                  effect: req(function* (
                    ...
                    yield wait_for(
                      ...
                      [
                        { asyncResult: "result" },
                        coreAccess.accessCard(
                          state,
                          side,
                          eid,
                          coreCard.getCard(state, movedTarget),
                        ),
                      ],
                      [],
                    );
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return coreCard.sameCard(
          targets[0],
          (card as any).special?.triesteTarget,
        );
      }),
      value: req(function* (
        state: State,
    ...
    },
    {
      ...
      req: req(function* (
        ...
        return (
          coreCard.sameCard(
            (state as any).currentIce,
            (card as any).special?.triesteTarget,
          ) &&
          coreCard.runner(breakCard) &&
          (!coreCard.identity(breakCard) || coreCard.fakeIdentity(breakCard)) &&
          (breakAbility?.break !== undefined ||
            breakAbility?.breaks !== undefined ||
            breakAbility?.heapBreakerBreak !== undefined ||
            breakAbility?.breakCost !== undefined)
        );
      }),
      value: true,
    },
...
export const trojan: CardDef = {
  ...
  "on-access": {
    ...
    effect: req(function* (
      ...
      yield wait_for(
        state,
        [{ asyncResult: "result" }, coreGaining.loseCredits(state, ":corp", 2)],
        [],
      );
      coreMoving.move(state, side, card, ":destroyed");
      const hand = (state as any).corp.hand;
      const trashTarget =
        hand.length > 0 ? hand[Math.floor(Math.random() * hand.length)] : null;
      if (trashTarget) {
        yield wait_for(
          state,
          [
            { asyncResult: "result" },
            coreMoving.trash(state, ":corp", eid, trashTarget, {
              causeCard: card,
            }),
          ],
          [],
  ...
  events: [
    {
      ...
      effect: req(function* (
        ...
        if (
          coreCard.getCounters(coreCard.getCard(state, card), ":power") <= 0
        ) {
          yield wait_for(
            state,
            [
          ...
          coreSay.systemMsg(
            state,
            ":corp",
            `uses ${(card as any).title} to do 4 meat damage`,
          );
          yield wait_for(
            state,
            [
...
export const urticaCipher: CardDef = {
  title: "Urtica Cipher",
  ...advanceAmbush(0, {
    msg: msg(
      (state: State, side: Side, eid: EID, card: Card) =>
...
export const vaporframeFabricator: CardDef = {
  ...
  "on-trash": {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.corp(c) && coreCard.inHand(c) && !coreCard.operation(c),
    },
    effect: req(function* (
      state: State,
      side: Side,
          ...
          coreEngine.resolveAbility(
            ...
            side,
            {
              ...
              choices: req(function* (
                ...
                return coreServers
                  .installableServers(state, cardToInstall)
...
export const veraIvanovnaShuyskaya: CardDef = (() => {
  const ability: any = {
    optional: {
      ...
      "yes-ability": coreDefHelpers.withRevealedHand(
        ":runner",
        { eventSide: ":corp" },
        {
  ...
  return {
    ...
    abilities: [
      coreDefHelpers.setAutoresolve(":auto-fire", "Vera Ivanovna Shuyskaya"),
    ],
  };
})();
    ...
    events: [
      {
        ...
        effect: req(function* (
          ...
          const similarActions = coreEvents.eventCount(
            state,
            side,
            ":action-resolved",
            (ts: any[]) => {
              const ctx = ts[0];
              return (
                JSON.stringify(relevantKeys(ctx)) ===
                JSON.stringify(relevantKeys(context))
              );
            },
          );
          yield wait_for(
...
export const wallToWall: CardDef = (() => {
  ...
  const ability: any = {
    ...
    effect: effect(function* (
      ...
      const assetCount = coreFinding
        .allActiveInstalled(state, ":corp")
        .filter(coreCard.asset).length;
  ...
  return {
    title: "Wall to Wall",
    "derezzed-events": [
      { ...coreDefHelpers.corpRezToast, event: ":runner-turn-ends" },
    ],
    events: [{ ...ability, event: ":corp-turn-begins" }],
    abilities: [ability],
  };
        ...
        (s: State, _: any, c: Card) => {
          if (coreCard.sameCard(c, installedCard as Card)) {
            coreToasts.toast(
              state,
              ":corp",
              "Cannot score due to Warm Reception.",
              "warning",
            );
            return false;
          }
...
export const watchdog: CardDef = (() => {
  function notTriggered(state: State): boolean {
    return coreEvents.noEvent(state, ":runner", ":rez", (ts: any[]) =>
      coreCard.ice(ts[0]?.card),
    );
  }
  return {
    title: "Watchdog",
  ...
  abilities: [
    ...
    },
    {
      ...
      effect: req(function* (
        ...
        yield wait_for(
          state,
          [{ asyncResult: "result" }, coreGaining.gainCredits(state, side, 6)],
          [],
        );
        yield wait_for(
          state,
          [
            ...
            coreEngine.resolveAbility(
              ...
              side,
              {
                ...
                req: req(function* (state: State): Generator<any, any, any> {
                  return (
                    coreFinding.allInstalledRunnerType(state, ":resource")
                      .length > 0
                  );
                }),
  ...
  abilities: [
    {
      ...
      choices: {
        card: (c: Card) =>
          coreCard.asset(c) && coreCard.inHand(c) && coreCard.corp(c),
      },
      msg: msg(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      ...
      effect: req(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreRezzing.rez(state, side, eid, hosted[hosted.length - 1], {
              costBonus: -2,
            }),
          ],
          [],
            ...
            coreEngine.resolveAbility(
              ...
              side,
              {
                optional: {
                  ...
                  prompt: msg(
                    (state: State, side: Side, eid: EID, card: Card) => {
                      const cards = (ctx?.cards || []).map((c: Card) =>
                        coreToString.cardStr(state, c, { visible: true }),
                      );
