  ...
  "static-abilities": [
    coreIce.iceStrengthBonus(
      req(function* (
        ...
        return ((state as any).corp?.servers?.[zone?.[1]]?.ices?.length ??
          0) === 1
          ? 4
          : 0;
      }),
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          !coreCard.hasSubtype(context?.icebreaker, "Fracter")
        );
      }),
      value: true,
    },
    ...
    effect: req(function* (
      ...
      if (targets[0] === "Lose 3 [Credits]") {
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            corePayment.pay(state, ":runner", eid, card, [
...
export const sherlock10: CardDef = {
  title: "Sherlock 1.0",
  subroutines: [
    traceAbility(4, addProgramToTopOfStack),
    traceAbility(4, addProgramToTopOfStack),
  ],
        ...
        "no-ability": {
          ...
          effect: effect(function* (
            ...
            yield wait_for(
              ...
              [
                { asyncResult: "result" },
                coreRuns.breachServer(state, ":runner", eid, [":rd"], {
                  noRoot: true,
                }),
              ],
              [],
...
export const sleipnir: CardDef = {
  ...
  subroutines: [
    maybeDrawSub(1),
    {
      ...
      choices: {
        card: (c: Card) =>
          coreCard.corp(c) && (coreCard.inHand(c) || coreCard.inDiscard(c)),
      },
      async: true,
      msg: {
        public: msg(function (
      ...
      },
      {
        ...
        effect: req(function* (
          ...
          const uniqueTypes = new Set(cards.map((c: Card) => (c as any).type))
            .size;
          if (
            (uniqueTypes <= 2 && cards.length === 3) ||
            (uniqueTypes === 1 && cards.length === 2)
          ) {
            coreSay.systemMsg(
              state,
              ":corp",
              `uses ${(card as any).title} to gain 3 [Credits]`,
            );
            yield wait_for(
              state,
              [
        ...
        const trashAbility = coreRevealing.withRevealedHand(
          ...
          { eventSide: ":corp" },
          {
            ...
            req: req(function* (s: State): Generator<any, any, any> {
              return ((s as any).runner?.hand?.length ?? 0) > 0;
            }),
            choices: {
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          !!coreIce.getCurrentEncounter(state) &&
          (targets as any[]).some(
            (t: any) => coreCard.runner(t.card) && coreCard.installed(t.card),
          )
        );
      }),
      effect: effect(function* (
        ...
        coreCard.updateCard(
          ...
          side,
          Object.assign({}, card, {
            special: Object.assign({}, (card as any).special, {
              sorocabanBlade: true,
            }),
          }),
        );
    ...
    effect: req(function* (
      ...
      coreEffects.registerLingeringEffect(state, side, card, {
        ...
        value: req(function* (
          ...
          return { subroutines: Array(subs).fill(endTheRun) };
        }),
      });
    }),
  },
          ...
          if ((state as any).run) {
            coreEffects.registerLingeringEffect(state, side, card, {
              ...
              req: req(function* (
                ...
                return coreCard.sameCard(tgts[0], card);
              }),
              value: 5,
            });
  ...
  subroutines: [
    {
      ...
      "change-in-game-state": {
        ...
        req: req(function* (state: State): Generator<any, any, any> {
          return (
            !!(state as any).run &&
            (state as any).run?.server?.[0] !== ":discard"
          );
        }),
      },
...
export const swarm: CardDef = (() => {
  ...
  return Object.assign({}, heroToHero(sub), {
    title: "Swarm",
    "on-rez": takeBadPub,
  });
})();
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return (
          coreCard.sameCard(card, context?.ice) &&
          coreCard.hasSubtype(context?.icebreaker, "AI")
        );
      }),
      value: true,
    },
      ...
      "change-in-game-state": {
        ...
        req: req(function* (state: State): Generator<any, any, any> {
          return coreBoard
            ...
            .some(
              (c: Card) => coreCard.program(c) && coreCard.hasSubtype(c, "AI"),
            );
        }),
      },
      choices: {
...
export const tatuBola: CardDef = {
  ...
  events: [
    {
      event: ":pass-ice",
      interactive: req(function* (state: State): Generator<any, any, any> {
      ...
      effect: req(function* (
        ...
        const iceInHand = ((state as any).corp?.hand ?? []).filter((c: Card) =>
          coreCard.ice(c),
        );
        ...
        if (iceInHand.length > 0) {
          innerAbility = {
            optional: {
              prompt: msg(function (s: State) {
                return `Gain 4 [Credits] and swap ${coreToString.cardStr(s, card)} with a piece of ice in HQ?`;
              }),
              "waiting-prompt": true,
              ...
              "yes-ability": {
                ...
                choices: req(function* (s: State): Generator<any, any, any> {
                  return ((s as any).corp?.hand ?? []).filter((c: Card) =>
                    coreCard.ice(c),
                  );
                }),
                async: true,
...
export const thoth: CardDef = {
  ...
  subroutines: [
    traceAbility(4, {
      ...
      async: true,
      msg: msg(function (state: State) {
        return `do ${utils.countTags(state)} net damage`;
      }),
      effect: effect(function* (
        state: State,
        side: Side,
    ...
    traceAbility(4, {
      ...
      async: true,
      msg: msg(function (state: State) {
        return `force the Runner to lose ${utils.countTags(state)} [Credits]`;
      }),
      effect: effect(function* (
        state: State,
        side: Side,
  ...
  subroutines: [
    ...
    trashProgramSub,
    {
      ...
      effect: req(function* (
        ...
        const resources = coreBoard
          .allInstalled(state, ":runner")
          .filter((c: Card) => coreCard.resource(c));
