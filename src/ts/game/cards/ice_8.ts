...
export const snoop: CardDef = {
  ...
  abilities: [{
    ...
    'change-in-game-state': { req: req(function*(state: State): Generator<any, any, any> { return ((state as any).runner?.hand?.length ?? 0) > 0; }) },
    ...
    effect: req(function*(state: State, side: Side, eid: EID, card: Card): Generator<any, any, any> {
      const trashAbility = coreRevealing.withRevealedHand(':runner', { eventSide: ':corp' }, {
        ...
        req: req(function*(s: State): Generator<any, any, any> { return ((s as any).runner?.hand?.length ?? 0) > 0; }),
        ...
        effect: req(function*(state: State, side: Side, eid: EID, card: Card): Generator<any, any, any> {
          if ((state as any).run) {
            coreEffects.registerLingeringEffect(state, side, card, {
              type: ':ice-strength',
              duration: ':end-of-run',
              req: req(function*(s: State, sd: Side, e: EID, c: Card, tgts: any[]): Generator<any, any, any> { return coreCard.sameCard(tgts[0], card); }),
              value: 5,
            });
...
export const tatuBola: CardDef = {
  ...
  events: [{
    event: ':pass-ice',
    interactive: req(function*(state: State): Generator<any, any, any> { return !!(state as any).run; }),
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
        ...
        innerAbility = {
          optional: {
            ...
            'yes-ability': {
              ...
              'waiting-prompt': true,
              choices: req(function*(s: State): Generator<any, any, any> { return ((s as any).corp?.hand ?? []).filter((c: Card) => coreCard.ice(c)); }),
              async: true,
              effect: req(function*(s: State, sd: Side, e: EID, c: Card, tgts: any[]): Generator<any, any, any> {
                yield wait_for(s, [{ asyncResult: 'result' },
