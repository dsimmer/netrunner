...
import {
  ...
  trashOnEmptyFn,
  triggerEventFn,
  unregisterEffectByUuidFn,
  updateBreakerStrengthFn,
  zoneNameFn,
} from "./hardware_1";
  ...
  let: {
    ...
    patchworkAbility: {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreUpdate.updateIn(card, ["special", "patchwork"], () => true);
        },
      ),
    },
    patchworkManualPrognosis: {
      ...
      req: req(function* (
        ...
        return !!(
          runnerFn(state)?.hand?.length &&
          canTriggerFn(
            state,
            side,
            eid,
            forms.let?.patchworkAbility,
            card,
            targets,
          )
        );
                ...
                sameCard(toTrash, toPlay)
                  ? {
                      ...
                      effect: effect(
                        (
                          ...
                          trashFn(state, side, eid, toTrash, {
                            causeCard: card,
                          });
                        },
                      ),
                  ...
                  : {
                      ...
                      effect: req(function* (
                        ...
                        if (eventFn(toPlay)) {
                          playInstantFn(state, ":runner", eid, toPlay, {
                            "cost-bonus": -2,
                          });
                        } else {
                          runnerInstallFn(state, ":runner", eid, toPlay, {
                            "cost-bonus": -2,
                          });
                        }
                      }),
      ...
      custom: req(function* (
        ...
        const costType =
          eid["source-type"] === ":play"
            ? "play"
            : eid["source-type"] === ":runner-install"
              ? "install"
              : "";
        ...
        continue_ability(
          ...
          side,
          {
            ...
            choices: {
              card: (c: Card) =>
                inHandFn(c) && runnerFn(c) && !sameCard(c, targetCard),
            },
            msg: (
              state: State,
              side: Side,
              ...
              return ((): string => {
                const target: any = (targets as any[])?.[0];
                return `trash ${target?.title || ""} to lower the ${costType} cost of ${targetCard?.title || ""} by 2 [Credits]`;
              })();
            },
            effect: req(function* (
              state: State,
  ...
  abilities: [
    {
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `gain ${1 + (getCounters(card, "credit") ?? 0)} [Credits]`,
      effect: req(function* (
        state: State,
        side: Side,
        eid: EID,
  ...
  prevention: [
    {
      ...
      ability: {
        ...
        req: req(function* (
          ...
          return (
            preventableFn(ctx) && (ctx.type === "meat" || ctx.type === ":meat")
          );
        }),
        effect: req(function* (
          state: State,
      ...
      optional: {
        ...
        "yes-ability": {
          ...
          effect: req(function* (
            ...
            yield wait_for(
              state,
              [{ asyncResult: "result" }, drawFn(state, ":runner", 1)],
              [],
            );
  ...
  events: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          continue_ability(state, ":runner", forms.let?.abi, card, null);
        },
      ),
    },
    {
      ...
      req: req(function* (
        ...
        const run = targetFn(state, card, targets) as { server?: string[]; position?: number } | undefined;
        return (
          (run?.server || []).join("") === "hq" &&
          (run?.position ?? 0) === 0 &&
          ((runnerFn(state) as any)?.deck?.length ?? 0) > 0
        );
      }),
      async: true,
      once: ":per-turn",
    ...
    { ...setAutoresolveFn("auto-fire", "Prognostic Q-Loop") },
    {
      ...
      "change-in-game-state": {
        req: req(function* (
          ...
          return (runnerFn(state)?.deck?.length ?? 0) > 0;
        }),
      },
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `reveal ${runnerFn(state)?.deck?.[0]?.title || ""} from the top of the stack`,
      async: true,
      effect: req(function* (
        state: State,
        side: Side,
          ...
          side,
          {
            optional: {
              req: req(function* (
                ...
                return (
                  topCard &&
                  (programFn(topCard) || hardwareFn(topCard)) &&
                  runnerCanPayAndInstallFn(
                    state,
                    side,
                    { ...eid, "source-type": ":runner-install" },
              ...
              prompt: (
                ...
                `Install ${runnerFn(state)?.deck?.[0]?.title || "the top card"}?`,
              "yes-ability": {
                async: true,
                effect: effect(
                  (
  ...
  events: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          trashFn(eid, card, { causeCard: card });
        },
      ),
    },
    {
  ...
  "on-install": {
    optional: {
      ...
      prompt: (
        ...
        card: Card,
        targets: any[],
      ) => `Install another copy of ${card.title}?`,
      "yes-ability": {
        async: true,
        effect: req(function* (
          state: State,
  ...
  let: {
    maxTrash: (
      ...
      allActiveInstalledFn(state, ":runner").filter(
        (c: Card) => c.title === "Ramujan-reliant 550 BMI",
      ).length,
  },
  prevention: [
    {
      ...
      ability: {
        ...
        cost: [toC(":trash-can")],
        msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          `prevent up to ${getCardFn(state, card) ? 1 + allActiveInstalledFn(state, ":runner").filter((c: Card) => c.title === "Ramujan-reliant 550 BMI").length : 1} damage`,
        req: req(function* (
          state: State,
          side: Side,
          eid: EID,
        ...
        effect: req(function* (
          ...
          yield wait_for(
            ...
            [
              ...
              resolveAbilityFn(
                state,
                side,
                preventUpToNDamageFn(maxTrash, [":net", ":core", ":brain"]),
                card,
                targets,
              ),
            ],
          ...
          systemMsg(
            state,
            side,
            `uses ${card.title} to trash the top ${prevented} cards of the stack`,
          );
  ...
  prevention: [
    {
      ...
      ability: {
        ...
        req: req(function* (
          ...
          return (
            preventableFn(forms.context(state, card, targets)) &&
            sameCard(
              (forms.context(state, card, targets) as any)?.sourceCard,
              (state as any).access,
            )
          );
        }),
          ...
          continue_ability(
            ...
            side,
            {
              cost: [
                ...
                toC(":x-credits", 0, {
                  maximum:
                    (forms.context(state, card, targets) as any)?.remaining ??
                    0,
                }),
              ],
              msg: (
                ...
                `prevent ${costValueFn(eid, ":x-credits")} ${damageTypeFn(state)} damage`,
              async: true,
              effect: req(function* (
                state: State,
                side: Side,
...
function costValueFn(eid: EID, type: string): number {
  return (corePayment.costValue?.(eid, type) as number | undefined) ?? 0;
}
    ...
    successfulRunReplaceBreach({
      ...
      ability: {
        ...
        msg: (
          ...
          return ((): string => {
            const target: any = (targets as any[])?.[0];
            return `add ${target.title} to the top of R&D`;
          })();
        },
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            const target: any = (targets as any[])?.[0];
            moveFn(":corp", target, ":deck", { front: true });
          },
        ),
      },
    }),
      ...
      effect: req(function* (
        ...
        if (targetCard) {
          systemMsg(
            state,
            ":runner",
            `force the Corp to reveal ${targetCard.title} from HQ`,
          );
  ...
  events: [
    {
      ...
      interactive: req(function* (
        ...
        return (
          ctx.card &&
          hardwareFn(ctx.card) &&
          (runnerFn(state)?.deck || []).some(
            (c: Card) => c.title === ctx.card.title,
          )
        );
      }),
      silent: req(function* (
      ...
      optional: {
        prompt: (
          ...
          `Search the stack for another copy of ${(forms.context(state, card, targets) as any)?.card?.title || "this card"} and add it to the grip?`,
        req: req(function* (
          state: State,
          side: Side,
          eid: EID,
        ...
        "yes-ability": {
          msg: (
            ...
            `add a copy of ${(forms.context(state, card, targets) as any)?.card?.title || "this card"} from the stack to the grip`,
          effect: effect(
            (
              state: State,
              side: Side,
              ...
              shuffleDeck(":deck");
              moveFn(
                (runnerFn(state)?.deck || []).find(
                  (c: Card) =>
                    c.title ===
                    (forms.context(state, card, targets) as any)?.card?.title,
                ),
                ":hand",
              );
            },
...
export const respirocytes: CardDef = {
  title: "Respirocytes",
  implementation:
    "Only watches trashes, playing events, and installing. Doesnt know about your hand size pre-install.",
  let: {
    ability: {
      once: ":per-turn",
      ...
      effect: req(function* (
        ...
        if (getCounters(getCardFn(state, card), "power") >= 3) {
          systemMsg(
            state,
            ":runner",
            `trashes ${card.title} as it reached 3 power counters`,
          );
          trashFn(state, side, eid, card, {
            unpreventable: true,
            causeCard: card,
          });
        } else {
    ...
    event: {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          continue_ability(forms.let?.ability, card, targets);
        },
      ),
    },
  },
  ...
  events: [
    ...
    { event: "runner-hand-changed?", ...(forms.let?.event || {}) },
    {
      ...
      req: req(function* (
        ...
        return (
          targets.some((t: any) => runnerFn(t.card) && inHandFn(t.card)) &&
    ...
    },
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          continue_ability(forms.let?.ability, card, null);
        },
      ),
    },
    {
      ...
      optional: {
        ...
        "yes-ability": {
          ...
          msg: (
            ...
            `access 1 additional card from ${zoneNameFn(targetServerFn(forms.context(state, card, targets)))}`,
          effect: effect(
            (
              state: State,
              side: Side,
              ...
              accessBonusFn(
                targetServerFn(forms.context(state, card, targets)),
                1,
              );
            },
          ),
          ...
          side,
          {
            ...
            effect: req(function* (
              ...
              derezFn(state, side, eid, target, {
                "msg-keys": { "include-cost-from-eid": paymentEid },
              });
            }),
          },
          card,
...
export const securityChip: CardDef = {
  ...
  abilities: [
    {
      label:
        "Add [Link] strength to a non-Cloud icebreaker until the end of the run",
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = (targets as any[])?.[0];
        return ((): string => {
          const target: any = (targets as any[])?.[0];
          return `add ${getLinkFn(state)} strength to ${target.title} until the end of the run`;
        })();
      },
      req: req(function* (
        state: State,
      ...
      choices: {
        card: (c: Card) =>
          hasSubtypeFn(c, "Icebreaker") &&
          !hasSubtypeFn(c, "Cloud") &&
          installedFn(c),
      },
      cost: [toC(":trash-can")],
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const target: any = (targets as any[])?.[0];
          pumpFn(target, getLinkFn(state), ":end-of-run");
        },
      ),
    },
    {
      label:
        "Add [Link] strength to any Cloud icebreakers until the end of the run",
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = (targets as any[])?.[0];
        return ((): string => {
          const target: any = (targets as any[])?.[0];
          return `add ${getLinkFn(state)} strength to ${targets.length} Cloud icebreakers until the end of the run`;
        })();
      },
      req: req(function* (
        state: State,
          ...
          trace: {
            ...
            successful: {
              ...
              effect: req(function* (
                ...
                yield wait_for(
                  state,
                  [{ asyncResult: "result" }, gainTagsFn(state, ":runner", 1)],
                  [],
                );
            ...
            unsuccessful: {
              msg: (
                ...
                card: Card,
                targets: any[],
              ) => `bypass ${cardStr(state, forms.currentIce?.(state))}`,
              effect: effect(
                (
                  state: State,
                  side: Side,
...
export const severniusStimImplant: CardDef = {
  ...
  let: {
    implantFn: (srv: string, kw: string) => ({
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `trash ${quantify(targets.length, "card")} and access ${quantify(Math.floor(targets.length / 2), "additional card")}`,
      async: true,
      effect: req(function* (
        state: State,
        side: Side,
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            trashCardsFn(state, side, targets, {
              unpreventable: true,
              causeCard: card,
            }),
          ],
        ...
        registerEventsFn(state, side, card, [
          breachAccessBonus(kw, bonus, { duration: ":end-of-run" }),
        ]);
...
export const sifr: CardDef = {
  ...
  let: {
    gatherPreSifrEffects: (
      ...
      const iceStrengthEffects = effects.filter(
        (e: any) => e.type === ":ice-strength",
      );
      return iceStrengthEffects.reduce((sum: number, e: any) => {
        const value =
          typeof e.value === "function"
            ? e.value(state, side, eid, getCardFn(state, e.card), targets)
            : e.value;
        return sum + value;
      }, 0);
    },
  ...
  events: [
    {
      ...
      optional: {
        prompt:
          "Lower your maximum hand size by 1 to reduce the strength of encountered ice to 0?",
        once: ":per-turn",
        "yes-ability": {
          msg: (
            state: State,
            ...
            `lower [their] maximum hand size by 1 and reduce the strength of ${forms.currentIce?.(state)?.title || "the encountered ice"} to 0`,
          effect: effect(
            (
              state: State,
              side: Side,
              ...
              registerLingeringEffectFn(":runner", card, {
                ...
                value: req(function* (
                  ...
                  return -(
                    strength +
                    (forms.let?.gatherPreSifrEffects?.(
                      card,
                      state,
                      side,
                      eid,
