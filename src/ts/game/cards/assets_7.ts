  ...
  abilities: [
    ...
    },
    {
      ...
      choices: req(function* (
        ...
        return corePrompts.cancellable(
          (state as any).corp.deck.filter(coreCard.agenda),
          { sorted: true },
        );
      }),
      cost: [corePayment.toC("trash-can", 1)],
    ...
    },
    {
      ...
      choices: req(function* (
        ...
        return corePrompts.cancellable(
          (state as any).corp.discard.filter(coreCard.agenda),
          { sorted: true },
        );
      }),
...
export const shatteredRemains: CardDef = {
                  ...
                  effect: req(function* (
                    ...
                    if ((targets[0] as string).startsWith("Add")) {
                      ...
                      yield wait_for(
                        ...
                        [
                          { asyncResult: "result" },
                          coreDamage.damage(state, ":corp", eid, ":net", dmg, {
                            card,
                          }),
                        ],
                        [],
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const hasMatch = targets.some(
          (t: any) => t?.card?.faction === runnerFaction,
        );
        return (
          hasMatch &&
          coreEvents.firstEvent(state, side, ":corp-trash", (ts: any[]) =>
            ts.some((t: any) => t?.card?.faction === runnerFaction),
          )
        );
      }),
      ...
      req: req(function* (
        ...
        return (
          ...
          (state as any).runner.discard.some(
            (c: Card) => (c as any).title === (t as any).title,
          )
        );
      }),
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return (
          coreCard.corp(t) &&
          coreEvents.noEvent(state, side, ":play-operation", (ts: any[]) =>
            coreCard.hasSubtype(ts[0]?.card, "Double"),
          ) &&
  ...
  events: [
    ...
    },
    {
      ...
      req: req(function* (
        ...
        return (
          coreEvents.firstEvent(state, side, ":runner-spent-click") &&
          !!(state as any).run &&
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const ctx = targets[0];
        return (
          coreCard.hasSubtype(ctx?.ice, "AP") &&
...
export const technoCo: CardDef = (() => {
  function isTechnoTarget(c: Card): boolean {
    return (
      coreCard.program(c) ||
      coreCard.hardware(c) ||
      (coreCard.resource(c) && coreCard.hasSubtype(c, "Virtual") !== undefined)
    );
  }
  return {
    title: "TechnoCo",
  ...
  abilities: [
    {
      ...
      req: req(function* (
        ...
        return (
          coreFinding.allInstalled(state, ":corp").filter(coreCard.ice)
            .length >= 2
        );
      }),
      choices: {
      ...
      effect: req(function* (
        ...
        const cardsToDerez = Math.min(
          coreFinding
            ...
            .filter((c: Card) => coreCard.rezzed(c) && !coreCard.agenda(c))
            .length,
