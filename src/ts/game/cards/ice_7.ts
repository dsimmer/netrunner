...
import {
  addProgramToTopOfStack,
  bioraidBreak,
  currentlyEncounteringCard,
  endTheRun,
  endTheRunIfTagged,
  endTheRunUnlessRunner,
...
export const mlinzi: CardDef = (() => {
      ...
      effect: req(function* (
        ...
        if (targets[0] === `Take ${netDmg} net damage`) {
          coreSay.systemMsg(
            state,
            ":corp",
            `uses ${(card as any).title} to do ${netDmg} net damage`,
          );
          yield wait_for(
            state,
            [
              { asyncResult: "result" },
              coreDamage.damage(state, ":runner", eid, ":net", netDmg, {
                card,
              }),
            ],
            [],
  ...
  return {
    title: "Mlinzi",
    subroutines: [netOrMill(1, 2), netOrMill(2, 3), netOrMill(3, 4)],
  };
})();
  ...
  "static-abilities": [
    {
      ...
      value: req(function* (
        ...
        const ices: Card[] = Object.values(corp?.servers ?? {}).flatMap(
          (s: any) => s?.ices ?? [],
        );
        return ices
          .filter(
            (ice: Card) =>
              coreCard.rezzed(ice) && !coreCard.sameCard(card, ice),
          )
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return coreCard.ice(tgts[0]?.card);
      }),
      effect: effect(function* (s: State, sd: Side): Generator<any, any, any> {
        coreIce.updateAllSubtypes(s, sd);
      }),
    },
    {
      event: ":derez",
...
export const nPot: CardDef = (() => {
    ...
    return Object.assign({}, endTheRun, {
      ...
      "change-in-game-state": {
        ...
        req: req(function* (state: State): Generator<any, any, any> {
          return coreThreat.threatLevel(x, state);
        }),
      },
    });
  }
  return {
    ...
    "runner-abilities": [
      coreIce.breakSub([corePayment.toC("credit", 3)], 1, null, {
        req: req(function* (
          state: State,
          side: Side,
          eid: EID,
...
export const negotiator: CardDef = {
  ...
  "runner-abilities": [
    coreIce.breakSub([corePayment.toC("credit", 2)], 1, "All", {
      req: req(function* (
        state: State,
        side: Side,
        eid: EID,
...
export const nerine20: CardDef = (() => {
  ...
  return {
    ...
    abilities: [
      coreEngine.setAutoresolve(":auto-fire", "Nerine 2.0 drawing cards"),
    ],
  };
})();
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          [...(state.corp?.current ?? []), ...(state.runner?.current ?? [])]
            .length > 0
        );
      }),
...
export const nextGold: CardDef = (() => {
    ...
    if (cnt > 0) {
      return (function* (): Generator<any, any, any> {
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            trashPrograms(cnt - 1, state, side, card, eid),
          ],
          [],
        );
      })();
  ...
  return {
    title: "NEXT Gold",
    "x-fn": req(function* (state: State): Generator<any, any, any> {
      return nextIceCount((state as any).corp);
    }),
    subroutines: [
      {
        ...
        msg: msg(function (state: State) {
          return `do ${nextIceCount((state as any).corp)} net damage`;
        }),
        async: true,
        effect: effect(function* (
          state: State,
          ...
          yield wait_for(
            ...
            [
              ...
              coreDamage.damage(
                ...
                eid,
                ":net",
                nextIceCount((state as any).corp),
                { card },
              ),
            ],
            [],
      ...
      },
      {
        ...
        effect: req(function* (
          ...
          const programs = coreBoard
            .allActiveInstalled(state, ":runner")
            .filter((c: Card) => coreCard.program(c)).length;
          yield wait_for(
            state,
            [
              { asyncResult: "result" },
              trashPrograms(Math.min(programs, n), state, side, card, eid),
            ],
            [],
          );
        }),
  ...
  subroutines: [
    {
      ...
      msg: msg(function (
        ...
        return `draw ${utils.quantify(targets[0], "card")}`;
      }),
      choices: {
        number: req(function* (state: State): Generator<any, any, any> {
          return nextIceCount((state as any).corp);
        }),
        default: req(function* (): Generator<any, any, any> {
          return 1;
        }),
      },
      async: true,
    ...
    },
    {
      ...
      choices: {
        ...
        max: req(function* (state: State): Generator<any, any, any> {
          return nextIceCount((state as any).corp);
        }),
      },
      effect: req(function* (
...
export const nightdancer: CardDef = (() => {
  const sub: any = {
    ...
    effect: effect(function* (
      ...
      (state as any).corp.extraClickTemp =
        ((state as any).corp.extraClickTemp ?? 0) + 1;
    }),
  };
  ...
  subroutines: [
    {
      ...
      req: req(function* (state: State): Generator<any, any, any> {
        return coreBoard
          ...
          .some((c: Card) => !coreCard.ice(c));
      }),
      choices: {
        card: (c: Card) =>
          coreCard.corp(c) && coreCard.installed(c) && !coreCard.ice(c),
      },
      effect: req(function* (
        state: State,
        side: Side,
        ...
        const canPay3 = corePayment.canPay(state, ":runner", eid, card, null, [
          ...
          effect: req(function* (
            ...
            if (tgts[0] === "Pay 3 [Credits]") {
              const result: any = yield wait_for(
                ...
                [
                  { asyncResult: "result" },
                  corePayment.pay(s, ":runner", coreEid.makeEid(s, e), ca, [
                    corePayment.toC("credit", 3),
                  ]),
                ],
                [],
...
export const peepingTom: CardDef = (() => {
  const sub = endTheRunUnlessRunner(
    "takes 1 tag",
    "take 1 tag",
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          ((state as any).runner?.hand?.length ?? 0) >= 4
        );
      }),
      ...
      custom: req(function* (
        ...
        const ability = {
          ...
          effect: req(function* (
            ...
            yield wait_for(
              s,
              [{ asyncResult: "result" }, coreCard.forfeit(s, sd, tgts[0])],
              [],
            );
  ...
  subroutines: [
    {
      ...
      msg: msg(function (state: State) {
        return `make the runner lose ${harmonicIceCount((state as any).corp)} [Credits]`;
      }),
      async: true,
      effect: req(function* (
        state: State,
        ...
        yield wait_for(
          ...
          [
            ...
            coreGaining.loseCredits(
              ...
              ":runner",
              eid,
              harmonicIceCount((state as any).corp),
            ),
          ],
          [],
        );
...
export const reverb: CardDef = {
  ...
  "rez-cost-bonus": req(function* (
    ...
    return -coreBoard
      ...
      .filter(
        (c: Card) =>
          coreCard.ice(c) && !coreCard.sameCard(card, c) && !coreCard.rezzed(c),
      ).length;
  }),
  subroutines: [endTheRun, endTheRun],
};
          ...
          effect: req(function* (
            ...
            if (tgts[0] === "Arrange cards") {
              yield wait_for(
                ...
                [
                  ...
                  coreEngine.resolveAbility(
                    s,
                    sd,
                    coreIce.reorderChoice(":corp", topCards),
                    c,
                    null,
                  ),
                ],
              ...
              coreSay.systemMsg(
                s,
                ":corp",
                `rearranges the top ${utils.quantify(topCards.length, "card")} of R&D`,
              );
              yield wait_for(
                s,
                [
      ...
      effect: req(function* (
        ...
        const trashHQAbility = {
          ...
          choices: req(function* (s: State): Generator<any, any, any> {
            return coreCard.cancellable((s as any).corp?.hand ?? [], {
              sorted: true,
            });
          }),
          async: true,
          effect: req(function* (
            ...
            yield wait_for(
              ...
              [
                { asyncResult: "result" },
                coreMoving.trash(s, ":corp", coreEid.makeEid(s, e), tgts[0], {
                  cause: ":subroutine",
                }),
              ],
              [],
  ...
  const sub: any = {
    ...
    effect: req(function* (
      ...
      yield wait_for(
        ...
        [
          { asyncResult: "result" },
          coreDamage.damage(state, side, eid, ":net", 1, {
            card,
            cause: ":subroutine",
          }),
        ],
    ...
    "on-encounter": {
      ...
      effect: effect(function* (
        ...
        coreCard.updateCard(
          state,
          side,
          Object.assign({}, card, { cardTarget: targets[0] }),
        );
      }),
    },
    events: [
      {
        ...
        req: req(function* (
          ...
          const context = targets[0];
          return (
            context?.damageType === ":net" &&
            context?.cause === ":subroutine" &&
            coreCard.sameCard(context?.card, card)
          );
        }),
        async: true,
        effect: req(function* (
          ...
          const matching = trashedCards.filter((c: Card) =>
            coreCard.isType(c, chosenType),
          );
          ...
          function resolveExtraDamage(x: number): any {
            return (function* (): Generator<any, any, any> {
              coreSay.systemMsg(
                state,
                ":corp",
                `uses ${(card as any).title} to deal 1 additional net damage${x > 1 ? ` (${x - 1} remaining)` : ""}`,
              );
              if (x <= 1) {
                yield wait_for(
                  state,
                ...
                yield wait_for(
                  state,
                  [{ asyncResult: "result" }, resolveExtraDamage(x - 1)],
                  [],
                );
              }
            })();
          ...
          yield wait_for(
            state,
            [{ asyncResult: "result" }, resolveExtraDamage(matching.length)],
            [],
          );
        }),
      },
      {
        ...
        effect: effect(function* (
          ...
          coreCard.updateCard(
            state,
            side,
            Object.assign({}, card, { cardTarget: undefined }),
          );
        }),
      },
    ],
  ...
  subroutines: [
    {
      ...
      choices: req(function* (state: State): Generator<any, any, any> {
        ...
        return coreCard.cancellable(
          forms.servers(state).filter((s: string) => s !== currentServer),
        );
      }),
      msg: msg(function (
        state: State,
      ...
      effect: req(function* (
        ...
        const movedIce = coreMoving.move(state, side, card, [
        ...
        yield wait_for(
          ...
          [
            ...
            coreMoving.trash(
              ...
              coreEid.makeEid(state, eid),
              movedIce,
              { unpreventable: true, cause: ":subroutine" },
            ),
          ],
          [],
        );
