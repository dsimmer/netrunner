...
export const tourGuide: CardDef = (() => {
  return {
    ...
    ...variableSubsIce(
      (state: State) =>
        coreBoard
          .allActiveInstalled(state as any, ":corp")
  ...
  subroutines: [
    {
      ...
      msg: msg(function (state: State) {
        return `force the Runner to lose ${
          (state as any).runner?.tag?.base > 0 || (state as any).tagged
...
export const viper: CardDef = {
  title: "Viper",
  subroutines: [traceAbility(3, runnerLosesClick), traceAbility(3, endTheRun)],
};
  ...
  "static-abilities": [
    coreIce.iceStrengthBonus(
      req(function* (state: State): Generator<any, any, any> {
        return !coreBoard
          .allActiveInstalled(state as any, ":runner")
          ...
          yield wait_for(
            ...
            [
              ...
              coreEngine.resolveAbility(
                state,
                side,
                searchForType(nextT, [...chosen, targets[0]]),
                card,
                null,
              ),
            ],
...
export const treeLine: CardDef = {
  ...
  expend: Object.assign(
    {},
    coreDefHelpers.placeAdvancementCounter(null, 3, "an ice", (c: Card) =>
      coreCard.ice(c),
    ),
    { cost: [corePayment.toC("credit", 1)] },
  ),
...
export const tributary: CardDef = {
  ...
  subroutines: [
    {
      label:
        "Draw 1 card and install a piece of ice from HQ protecting another server",
      async: true,
      effect: req(function* (
        state: State,
        side: Side,
    ...
    {
      label:
        "Give +2 strength to each piece of ice for the remainder of the run",
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        const ctx = targets?.[0];
        return (
          coreCard.sameCard(card, ctx?.ice) &&
          coreCard.hasSubtype(ctx?.icebreaker, "AI")
        );
      }),
      value: true,
    },
...
export const unsmilingTsarevna: CardDef = (() => {
  const breakableFn = req(function* (
    ...
    const someBrokenPrinted =
      subs.filter((s: any) => s.broken && s.printed).length > 0;
    if (!someBrokenPrinted) return true;
    return !coreEffects.anyEffects(
      state,
      side,
      ":unsmiling-effect",
      (v: any) => v === true,
    ...
    "on-rez": {
      optional: {
        ...
        req: req(function* (
          ...
          return (
            !!(state as any).run &&
            coreServers.cardToServer(state as any, card) ===
  ...
  subroutines: [
    {
      ...
      effect: req(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreTags.gainTags(state, side, coreEid.makeEid(state, eid), x, {
    ...
    },
    {
      ...
      effect: req(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreMoving.trash(state, side, coreEid.makeEid(state, eid), card, {
              cause: ":subroutine",
            }),
          ],
          [],
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const printedSub = (((card as any).subroutines || []) as any[]).find(
          (s: any) => s.printed,
        );
        const ctx = targets?.[0];
        return (
          coreCard.sameCard(card, ctx?.ice) &&
          (!printedSub?.broken ||
            !new Set(printedSub?.["breaker-subtypes"] || []).has("Decoder"))
        );
      }),
      msg: "give the Runner 1 tag",
      async: true,
  ...
  subroutines: [
    traceAbility(5, {
      ...
      effect: req(function* (
        ...
        yield wait_for(
          state,
          [{ asyncResult: "result" }, coreRevealing.reveal(state, side, grip)],
          [],
        );
    ...
    runnerLosesClick,
    {
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreMoving.trash(state, ":runner", eid, targets[0], {
              cause: ":subroutine",
            }),
          ],
          [],
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          coreCard.protectingHq(state as any, card)
        );
      }),
...
function spentClickToBreakSub(state: State): boolean {
  const events = coreEvents.runEvents(
    state as any,
    null,
    ":subroutines-broken",
  ) as any[];
  const breakers = events
    .map((entry: any) => entry?.[0]?.breaker)
    .filter(Boolean);
  for (const b of breakers) {
    const abs = (((b as any).side === "Runner"
      ? (b as any).abilities
      : (b as any)["runner-abilities"]) || []) as any[];
    for (const ab of abs) {
      const costs = ([] as any[]).concat((ab as any)["break-cost"] || []);
      if (
        costs.some(
          (c: any) =>
            (c as any)?.["cost/type"] === ":lose-click" ||
            (c as any)?.type === ":lose-click",
        )
      ) {
        return true;
      }
      ...
      effect: req(function* (
        ...
        if (spentClickToBreakSub(state)) {
          ...
          coreSay.systemMsg(
            state,
            side,
            "does not do core damage with Zed 2.0",
          );
          coreEid.effectCompleted(state, side, eid);
        }
      }),
