...
export const finality: CardDef = {
  ...
  events: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreEngine.registerEvents(card, [coreDefHelpers.breachAccessBonus('rd', 3, { duration: 'end-of-run' })]); }),
  }],
};
...
export const frameJob: CardDef = {
  ...
  onPlay: {
    ...
    onChangeGameState: {
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).runner?.scored?.length > 0; }),
    },
    choices: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).runner?.scored || []; }),
    msg: msg('forfeit ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg, ' and give the Corp 1 bad publicity'),
    async: true,
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
      yield wait_for(
        state,
...
export const freelanceCodingContract: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg('trash ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => (targets || []).map((c: Card) => c.title).join(', '), ' and gain ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => (targets || []).length * 2, ' [Credits]'),
    async: true,
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
...
export const gameDay: CardDef = {
  title: 'Game Day',
  onPlay: {
    msg: msg('draw ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreUtils.quantify(coreHandSize.handSize(state, 'runner') - ((state as any).runner?.hand?.length || 0), 'card')),
    onChangeGameState: {
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDrawing.draw(eid, coreHandSize.handSize(state, 'runner') - ((state as any).runner?.hand?.length || 0)); }),
  },
};
...
export const glutCipher: CardDef = {
  ...
  events: [{
    ...
    mandatory: true,
    ability: {
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).corp?.discard?.length >= 5; }),
      showDiscard: true,
      async: true,
      player: 'corp',
      waitingPrompt: true,
...
export const hacktivistMeeting: CardDef = {
  ...
  staticAbilities: [{
    ...
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return msg; }),
...
function harmonyChooseEnd(toShuffle: string[]): any {
  ...
  return {
    msg: msg('shuffle ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreUtils.quantify(toShuffle.length, 'card'), ' back into the stack: ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => toShuffle.join(', ')),
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
...
export const highStakesJob: CardDef = {
  ...
  events: [{
    ...
    msg: 'gain 12 [Credits]',
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreGaining.gainCredits('runner', eid, 12); }),
  }],
};
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreEngine.triggerEvent('searched-stack'); continue_ability(
        (() => {
          ...
          if (coreInstalling.runnerCanPayAndInstall(state, side, { ...eid, source: card }, connection)) {
            return {
              optional: {
                prompt: msg('Install ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => connection.title, '?'),
                yesAbility: {
                  async: true,
                  effect: effect(
                ...
                noAbility: {
                  effect: effect(coreMoving.move(state, side, connection, 'hand'), coreShuffling.shuffle(state, side, 'deck')),
                },
              },
            };
          }
          return { effect: effect(coreMoving.move(state, side, connection, 'hand'), coreShuffling.shuffle(state, side, 'deck')) };
        })(),
        card,
        null
      ); }),
...
export const iveHadWorse: CardDef = {
  ...
  onTrash: {
    ...
    msg: 'draw 3 cards',
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDrawing.draw('runner', eid, 3); }),
  },
};
...
export const immolationScript: CardDef = {
  ...
  events: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { continue_ability(
        {
          async: true,
          prompt: msg('Choose a rezzed copy of ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => msg, ' to trash'),
...
export const inTheGroove: CardDef = {
  ...
  events: [{
    ...
    interactive: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const ctx: any = ((targets as any[])?.[0] as any)?.context ?? (targets as any[])?.[0];
...
export const independentThinking: CardDef = {
  ...
  onPlay: {
    ...
    onChangeGameState: {
      req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (coreBoard.allInstalled(state, 'runner') || []).length > 0; }),
    },
    choices: { max: 5, card: (c: Card) => coreCard.installed(c) && coreCard.runner(c) },
    msg: msg('trash ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => (targets || []).map((c: Card) => c.title).join(', '), ' and draw ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreUtils.quantify((targets || []).length * (targets || []).some((c: Card) => !c.facedown && coreCard.hasSubtype(c, 'Directive')) ? 2 : 1, 'card')),
    async: true,
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
  ...
  onPlay: coreChooseOne.chooseOneHelper([
    { option: 'Gain 2 [Credits]', ability: gainCreditsAbility(2) },
    {
      ...
      ability: {
        ...
        async: true,
        effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreExpose.expose(eid, [msg]); }),
      },
    },
  ]),
};
...
export const informationSifting: CardDef = {
  ...
  events: [{
    ...
    ability: {
      ...
      async: true,
      waitingPrompt: true,
      prompt: msg('Choose up to ', (state: State, side: Side, eid: EID, card: Card, targets: any[]) => coreUtils.quantify((state as any).corp?.hand?.length - 1 || 0, 'card'), ' for the first pile'),
      choices: { card: (c: Card) => coreCard.inHand(c) && coreCard.corp(c), max: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { return (state as any).corp?.hand?.length - 1 || 0; }) },
      effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
        yield continue_ability(
          state,
          'runner',
...
export const interdiction: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreFlags.registerTurnFlag(
        card,
        'can-rez',
        function*(state: State, _side: Side, card: Card): Generator<any, any, any> {
