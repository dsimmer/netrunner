  ...
  subroutines: [
    {
      ...
      choices: req(function* (state: State): Generator<any, any, any> {
        return [
          (state as any).corp?.deck?.length !== 1
...
export const f2p: CardDef = {
  ...
  "runner-abilities": [
    coreIce.breakSub([corePayment.toC("credit", 2)], 1, null, {
      req: req(function* (
        state: State,
        side: Side,
        eid: EID,
  ...
  subroutines: [
    traceAbility(6, {
      ...
      effect: req(function* (
        ...
        const hasHardware = coreBoard
          .allInstalled(state, ":runner")
          .some((c: Card) => coreCard.hardware(c));
          ...
          innerAbility = {
            ...
            effect: req(function* (
              ...
              yield wait_for(
                ...
                [
                  { asyncResult: "result" },
                  coreMoving.trash(s, sd, coreEid.makeEid(s, e), tgts[0], {
                    cause: ":subroutine",
              ...
              yield wait_for(
                ...
                [
                  { asyncResult: "result" },
                  coreDamage.damage(s, sd, coreEid.makeEid(s, e), ":meat", 2, {
                    unpreventable: true,
                    suppressCheckpoint: true,
                    card: c,
                  }),
              ...
              yield wait_for(
                s,
                [{ asyncResult: "result" }, coreRuns.endRun(s, sd, e, c)],
                [],
              );
            }),
          };
...
export const flyswatter: CardDef = {
  title: "Flyswatter",
  "suppress-rez-sound": req(function* (
    state: State,
    side: Side,
    eid: EID,
    card: Card,
...
export const formicary: CardDef = {
  ...
  "derezzed-events": [
    {
      ...
      interactive: req(function* (
        ...
        return (
          coreEngine.getAutoresolve(state, side, card, ":auto-fire")(
            state,
            side,
            eid,
            card,
      ...
      optional: {
        ...
        req: req(function* (
          ...
          return (
            coreRezzing.canRez(state, side, card) &&
            corePayment.canPay(
              state,
              side,
              eid,
              card,
              null,
              coreRezzing.getRezCost(state, side, card),
            )
          );
        }),
        "yes-ability": {
          ...
          effect: req(function* (
            ...
            const result: any = yield wait_for(
              state,
              [{ asyncResult: "result" }, coreRezzing.rez(state, side, card)],
              [],
            );
  ...
  abilities: [
    coreEngine.setAutoresolve(
      ":auto-fire",
      "Formicary rezzing and moving itself on approach",
    ),
  ],
};
  ...
  const revealAndShuffle: any = {
    ...
    choices: {
      card: (c: Card) =>
        ...
        coreCard.agenda(c),
      max: req(function* (): Generator<any, any, any> {
        return 3;
      }),
    },
    async: true,
      ...
      yield wait_for(
        ...
        [
          ...
          coreRevealing.revealLoud(
            ...
            side,
            card,
            { andThen: ", and shuffle [them] into R&D" },
            targets,
          ),
        ],
        [],
  ...
  function gfLoseCredits(state: State, side: Side, eid: EID, n: number): any {
    if (n > 0) {
      return (function* (): Generator<any, any, any> {
        ...
        yield wait_for(
          state,
          [{ asyncResult: "result" }, gfLoseCredits(state, side, eid, n - 1)],
          [],
        );
      })();
    }
  ...
  return {
    ...
    "on-break-subs": {
      req: req(function* (
        ...
        return (context?.brokenSubs ?? context?.["broken-subs"] ?? []).some(
          (s: any) => s.printed,
        );
      }),
      msg: msg(function (
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          state,
          [{ asyncResult: "result" }, gfLoseCredits(state, side, eid, n)],
          [],
        );
      }),
    },
...
export const hafrun: CardDef = (() => {
  function preventSubBreakBy(t: Card): any {
    return {
      ...
      req: req(function* (
        ...
        return (
          coreCard.sameCard(breakCard, t) &&
          (breakAbility?.break != null ||
            breakAbility?.breaks != null ||
            breakAbility?.heapBreakerBreak != null ||
            breakAbility?.breakCost != null)
        );
      }),
    };
  }
  return {
    ...
    "on-rez": {
      optional: {
        prompt:
          "Trash a card from HQ to prevent subroutines from being broken by a Runner card abilities for the remainder of the run?",
        req: req(function* (
          state: State,
          side: Side,
          eid: EID,
...
export const hakarl10: CardDef = {
  ...
  "on-rez": {
    req: req(function* (
      ...
      return (
        ...
        coreBoard
          ...
          .filter(
            (c: Card) => !coreCard.sameCard(card, c) && coreCard.rezzed(c),
          ).length > 0
      );
    }),
    prompt:
      "Derez another card to prevent the runner from using printed abilities on bioroid ice this turn?",
    choices: {
      "not-self": true,
      req: req(function* (
        state: State,
    ...
    effect: req(function* (
      ...
      yield wait_for(
        state,
        [{ asyncResult: "result" }, coreRezzing.derez(state, side, targets[0])],
        [],
      );
      coreSay.systemMsg(
        state,
        side,
        "prevents the runner from using printed abilities on bioroid ice for the rest of the turn",
      );
      coreEffects.registerLingeringEffect(state, side, card, {
        type: ":prevent-paid-ability",
        ...
        req: req(function* (
          ...
          return (
            coreCard.ice(tgts[0]) &&
            sd === ":runner" &&
            coreCard.hasSubtype(tgts[0], "Bioroid")
          );
        }),
        value: true,
      });
  ...
  subroutines: [
    {
      ...
      choices: req(function* (state: State): Generator<any, any, any> {
        return coreCard.cancellable((state as any).runner?.discard ?? [], {
          sorted: true,
        });
      }),
      msg: msg(function (
...
export const hammer: CardDef = (() => {
  const breakableFn = req(function* (
    ...
    const killerBroken =
      (card as any).subroutines?.filter(
        (s: any) =>
          s.printed &&
          s.broken &&
          !(s.breakerSubtypes ?? s["breaker-subtypes"] ?? []).includes(
            "Killer",
          ),
      ) ?? [];
    return killerBroken.length === 0 ||
      coreCard.hasSubtype(targets[0], "Killer")
      ? ":unrestricted"
      : false;
  });
  return {
    ...
    "static-abilities": [
      {
        ...
        req: req(function* (
          ...
          return (
            ...
            !coreCard.hasSubtype(context?.breaker, "Killer")
          );
        }),
        value: true,
      },
    ...
    subroutines: [
      Object.assign({}, coreDefHelpers.giveTags(1), { breakable: breakableFn }),
      {
        ...
        "change-in-game-state": {
          ...
          req: req(function* (state: State): Generator<any, any, any> {
            return coreBoard
              .allInstalled(state, ":runner")
              .some((c: Card) => coreCard.hardware(c) || coreCard.resource(c));
          }),
        },
        choices: {
          req: req(function* (
            ...
            return (
              coreCard.installed(targets[0]) &&
              (coreCard.hardware(targets[0]) || coreCard.resource(targets[0]))
            );
          }),
        },
        async: true,
      ...
      {
        label:
          "Choose a program to trash that is not a decoder, fracter or killer",
        "change-in-game-state": {
          silent: true,
          req: req(function* (state: State): Generator<any, any, any> {
            return coreBoard
...
export const descent: CardDef = (() => {
  const shuffleAb: any = {
    label:
      "Draw 1 card and shuffle up to 2 agendas in HQ and/or Archives into R&D",
    msg: "draw 1 card",
    async: true,
      ...
      if (delta > 0) {
        const discardAbility = {
          ...
          effect: req(function* (
            ...
            const discarded = tgts.map((t: Card) =>
              coreMoving.move(s, sd, t, ":discard"),
            );
            const ev =
              sd === ":runner"
