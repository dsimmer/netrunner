  ...
  events: [
    {
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `gain ${(coreCard.getCounters(coreCard.getCard(state, card), ":advancement") || 0) >= 5 ? "3" : "2"} [Credits]`,
      async: true,
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      optional: {
        ...
        "yes-ability": {
          ...
          msg: (
            ...
            return ((): string => {
              const target: any = (targets as any[])?.[0];
              return `reveal ${target.title} from R&D`;
            })();
          },
          effect: effect(function* (
            ...
            continue_ability(
              ...
              coreChooseOne.chooseOneHelper([
                {
                  option: `Install ${targetCard.title}`,
                  ability: {
                    async: true,
                    effect: effect(
                      coreInstalling.corpInstall(state, side, eid, targetCard, {
                        "ignore-install-cost": true,
                        msgArgs: { displayOrigin: true, installSource: card },
                      }),
                    ),
                  },
                },
                {
                  option: `Add ${targetCard.title} to HQ`,
                  ability: {
                    msg: (
                      state: State,
                      side: Side,
                      ...
                      card: Card,
                      targets: any[],
                    ) => `add ${targetCard.title} to HQ`,
                    effect: effect(
                      coreMoving.move(state, side, targetCard, "hand"),
                    ),
                  },
                },
              ]),
  ...
  abilities: [
    {
      ...
      req: req(function* (
        ...
        return coreFlags.isScored(state, ":runner", card);
      }),
      msg: "shuffle itself into R&D",
      label: "Shuffle this agenda into R&D",
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreMoving.move(":corp", card, "deck", null);
    ...
    trace: {
      ...
      successful: {
        ...
        effect: effect(function* (
          ...
          const resources = allInstalled.filter(
            (c: Card) =>
              coreCard.hasSubtype(c, "Job") ||
          ...
          yield wait_for(
            ...
            [
              { asyncResult: "result" },
              coreMoving.trashCards(state, side, eid, resources, {
                causeCard: card,
              }),
            ],
            [],
  ...
  events: [
    {
      ...
      effect: effect(function* (
        ...
        coreSay.systemMsg(
          state,
          side,
          `uses ${card.title} to force the Runner to trash ${c.title} from the grip at random`,
        );
        yield wait_for(
          state,
          [
            { asyncResult: "result" },
            coreMoving.trash(state, side, eid, c, { causeCard: card }),
          ],
          [],
        );
      }),
    },
    {
      ...
      req: req(function* (
        ...
        return (
          (ctx.previousZone || []).includes(":discard") &&
...
export const philoticEntanglement: CardDef = {
  ...
  "on-score": {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      `do ${(state as any).runner?.scored?.length} net damage`,
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreDamage.damage(
...
export const postedBounty: CardDef = {
  ...
  "on-score": {
    optional: {
      prompt:
        "Forfeit this agenda to give the Runner 1 tag and take 1 bad publicity?",
      "yes-ability": {
        ...
        effect: effect(function* (
          ...
          yield wait_for(
            ...
            [
              { asyncResult: "result" },
              coreBadPublicity.gainBadPublicity(
                state,
                ":corp",
...
export const priorityRequisition: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = (targets as any[])?.[0];
        coreRezzing.rez(eid, target, { "ignore-cost": ":all-costs" });
      },
    ),
  },
};
...
export const profiteering: CardDef = {
  ...
  "on-score": {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      return ((): string => {
        const target: any = (targets as any[])?.[0];
        return `take ${target} bad publicity and gain ${5 * parseInt(target, 10)} [Credits]`;
      })();
    },
    async: true,
    effect: effect(function* (
...
export const projectAres: CardDef = {
  ...
  "on-score": {
    ...
    prompt: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      `Choose ${utils.quantify(((forms.context(state, card, targets) || {}).advancement || 0) - 4, "installed card")} to trash`,
    choices: {
      max: req(function* (
        state: State,
        side: Side,
        ...
        const adv =
          (forms.context(state, card, targets) || {}).advancement || 0;
        return Math.min(
          adv - 4,
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      `force the Runner to trash ${utils.quantify(((forms.context(state, card, targets) || {}).advancement || 0) - 4, "installed card")} and take 1 bad publicity`,
    async: true,
    effect: effect(function* (
      state: State,
      side: Side,
      ...
      coreSay.systemMsg(
        state,
        side,
        `trashes ${utils.enumerateCards(targets)}`,
      );
      yield wait_for(
        state,
        [
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return corePayment.canPay(state, side, eid, card, null, [
          corePayment.toC("agenda", 1),
        ]);
      }),
      interactive: req(function* (
        state: State,
      ...
      "change-in-game-state": {
        ...
        req: req(function* (
          ...
          return corpDiscard.some(
            (c: Card) => !c.seen || !coreCard.operation(c),
          );
        }),
      },
      choices: {
      ...
      effect: effect(function* (
        ...
        coreEffects.registerLingeringEffect(card, {
          ...
          req: req(function* (
            ...
            return coreCard.sameCard(targets[0] || {}, t);
          }),
...
export const projectVacheron: CardDef = {
  ...
  "move-zone": req(function* (
    ...
    if (
      coreCard.inScored(card) &&
      (card as any)["scored-side"] === ":runner" &&
      (card as any)["previous-zone"]?.[0] !== ":discard"
    ) {
      coreSay.systemMsg(
        state,
        side,
        `uses ${card.title} to place 4 agenda counters on itself`,
      );
      yield wait_for(
        state,
        [
  ...
  events: [
    {
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        return ((): string => {
          const target: any = (targets as any[])?.[0];
          return `remove 1 agenda counter from ${target.title}`;
        })();
      },
      async: true,
      effect: effect(function* (
        ...
        if ((coreCard.getCounters(card, ":agenda") || 0) > 0) {
          yield wait_for(
            ...
            [
              { asyncResult: "result" },
              coreProps.addCounter(state, side, card, ":agenda", -1, null),
            ],
            [],
          );
          ...
          if ((coreCard.getCounters(cardObj, ":agenda") || 0) === 0) {
            ...
            coreSay.systemMsg(
              state,
              ":runner",
              `gains ${utils.quantify(points, "agenda point")} from ${cardObj?.title}`,
            );
          }
  ...
  abilities: [
    {
      req: req(function* (
        ...
        return (
          ci &&
          coreCard.rezzed(ci) &&
          coreCard.hasSubtype(ci, "Bioroid") &&
          forms.run(state)?.phase === ":approach-ice"
        );
      }),
      cost: [corePayment.toC("agenda", 1)],
      "keep-menu-open": ":while-agenda-tokens-left",
      ...
      effect: effect(function* (
        ...
        coreEffects.registerLingeringEffect(card, {
          ...
          req: req(function* (
            ...
            return coreCard.sameCard(targets[0] || {}, cardTarget);
          }),
          value: {
            subroutines: [
              {
    ...
    abilities: [
      {
        ...
        effect: effect(function* (
          ...
          continue_ability(
            state,
            ":runner",
            chooseCard((forms.run(state) || {}).server),
            card,
            null,
          );
        }),
...
function chooseCard(runServer: any): any {
  return {
    ...
    choices: {
      card: (c: Card) =>
        (coreCard.getZone(c) || [])[0] === (runServer || [])[0],
    },
    effect: effect(function* (
      state: State,
      side: Side,
  ...
  return {
    ...
    choices: {
      ...
      card: (c: Card) => {
        return (
          coreCard.corp(c) &&
          coreCard.inHand(c) &&
          (!coreCard.ice(toSwap) || coreCard.ice(c))
        );
      },
    },
    msg: {
      public: (
        ...
        card: Card,
        targets: any[],
      ) => `swap ${coreToString.cardStr(state, toSwap)} with a card from HQ`,
      corp: (
        state: State,
        side: Side,
        eid: EID,
    ...
    effect: effect(function* (
      ...
      yield wait_for(
        ...
        [
          { asyncResult: "result" },
          coreInstalling.swapCardsAsync(state, side, toSwap, target),
        ],
        [],
      );
      continue_ability(
  ...
  events: [
    {
      ...
      choices: {
        req: req(function* (
          ...
          return coreCard.canBeAdvanced(state, card);
        }),
      },
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const target: any = (targets as any[])?.[0];
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const target: any = (targets as any[])?.[0];
          coreProps.addProp(":corp", eid, target, ":advance-counter", 1, {
            placed: true,
          });
        },
      ),
...
export const proprionegation: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreProps.addCounter(eid, card, ":agenda", 1);
      },
    ),
  },
  abilities: [
      ...
      effect: effect(function* (
        ...
        if ((state as any)["forced-encounter"]) {
          ...
          if (coreRuns.getCurrentEncounter(state)) {
            coreEngine.triggerEvent(state, ":end-of-encounter", {
              ice: coreIce.getCurrentIce(state),
            });
          }
          yield wait_for(
            state,
...
export const quantumPredictiveModel: CardDef = {
  ...
  "on-access": {
    ...
    choices: ["OK"],
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      "add itself to [their] score area and gain 1 agenda point",
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreMoving.move(":corp", card, ":scored", { force: true });
        coreAgendas.updateAllAgendaPoints();
        coreWinning.checkWinByAgenda();
      },
    ),
  },
};
  ...
  "move-zone": req(function* (
    ...
    if (coreCard.inScored(card) && (card as any)["scored-side"] === ":corp") {
      coreSay.systemMsg(
        state,
        side,
        `uses ${card.title} to make all assets gain Advertisement`,
      );
    }
    coreEid.effectCompleted(state, side, eid);
  }),
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return coreCard.asset(targets[0] || {});
      }),
      value: "Advertisement",
    },
  ],
    ...
    effect: effect(function* (
      ...
      if (from.length > 0) {
        ...
        coreSay.systemMsg(
          state,
          side,
          "does not add any cards from HQ to bottom of R&D",
        );
        coreEid.effectCompleted(state, side, eid);
      }
    }),
    ...
    effect: effect(function* (
      ...
      if (target !== "Done") {
        const remainingFiltered = remaining.filter(
          (c: Card) => !coreCard.sameCard(c, target),
        );
        if (remainingFiltered.length > 0) {
          continue_ability(
            state,
            side,
            corpChoice(remainingFiltered, [target, ...chosen], original),
            card,
            null,
          );
        }
      } else {
        const finalChosen = chosen.filter((c: Card | string) => c !== "Done") as Card[];
...
function corpFinal(chosen: Card[], original: Card[]): any {
  return {
    ...
    async: true,
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      `add ${utils.quantify(chosen.length, "card")} from HQ to the bottom of R&D and draw ${utils.quantify(chosen.length, "card")}${chosen.length <= ((state as any).runner?.hand?.length || 0) ? `. The Runner randomly adds ${utils.quantify(chosen.length, "card")} from [runner-pronoun] Grip to the bottom of the Stack` : ""}`,
    effect: effect(function* (
      state: State,
      side: Side,
      eid: EID,
      ...
      if (target === "Done") {
        ...
        yield wait_for(
          state,
          [{ asyncResult: "result" }, coreDrawing.draw(state, ":corp", n)],
          [],
        );
        if (n <= ((state as any).runner?.hand?.length || 0)) {
          ...
          for (const r of shuffled.slice(0, n)) {
            yield wait_for(
              ...
              [
                { asyncResult: "result" },
                coreMoving.move(state, ":runner", r, "deck"),
              ],
              [],
            );
          }
...
export const regenesis: CardDef = {
  ...
  "on-score": {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.agenda(c) && coreCard.inDiscard(c) && !coreCard.faceup(c),
    },
    "show-discard": true,
    async: true,
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const target: any = (targets as any[])?.[0];
      return `reveal ${(target || []).title} and add it to [their] score area`;
    },
    effect: effect(function* (
      state: State,
      side: Side,
      ...
      yield wait_for(
        state,
        [{ asyncResult: "result" }, coreRevealing.reveal(state, side, target)],
        [],
      );
...
export const remasteredEdition: CardDef = {
  ...
  "on-score": agendaCounters(1),
  abilities: [
    Object.assign(coreDefHelpers.placeAdvancementCounter(null, 1), {
      cost: [corePayment.toC("agenda", 1)],
    }),
  ],
};
  ...
  "move-zone": req(function* (
    ...
    if (coreCard.inScored(card) && (card as any)["scored-side"] === ":corp") {
      coreSay.systemMsg(
        state,
        side,
        `uses ${card.title} to increase [their] maximum hand size by 2`,
      );
    }
    coreEid.effectCompleted(state, side, eid);
  }),
              ...
              side,
              {
                ...
                choices: req(function* (
                  ...
                  return corePrompts.cancellable(
                    (corp?.deck || []).filter((c: Card) => coreCard.ice(c)),
                    { sorted: true },
                  );
                }),
...
export const researchGrant: CardDef = {
  ...
  "on-score": {
    ...
    silent: req(function* (
      ...
      return (
        allInstalled.filter((c: Card) => c.title === card.title).length === 0
      );
    }),
    async: true,
    effect: effect(function* (
      ...
      continue_ability(
        ...
        side,
        {
          ...
          req: req(function* (
            ...
            return coreBoard
              .allInstalled(state, ":corp")
              .some((c: Card) => c.title === card.title);
          }),
          effect: effect(
            coreAgendas.score(eid, coreCard.getCard(state, target), {
              "no-req": true,
            }),
          ),
      ...
      req: req(function* (
        ...
        return (
          ...
          coreEvents.firstEvent(
            ...
            side,
            "advance",
            (t: any[]) => t[0] && coreCard.sameCard(card, t[0].card),
          )
        );
      }),
      msg: "gain 3 [Credits]",
