  ...
  prevention: [
    {
      ...
      ability: {
        ...
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            corePrevention.damageBoost(state, side, eid, 1);
          },
        ),
      },
    },
...
export const digitalRightsManagement: CardDef = {
  ...
  onPlay: {
    req: req(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        state.turn > 1 &&
        !(state as any).runner?.register?.lastTurn?.successfulRun?.includes(
          "hq",
        ),
    ),
...
export const diversifiedPortfolio: CardDef = {
  ...
  onPlay: {
    msg: msg(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const remotes = coreServers
          .getRemoteNames(state)
          .filter(
            (name: string) =>
              (state as any).corp?.servers?.[name]?.content?.length > 0,
          );
...
export const divertPower: CardDef = {
  ...
  onPlay: {
    ...
    choices: {
      ...
      max: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreBoard.allInstalled(state, "corp").filter(coreCard.rezzed).length,
      ),
    },
    onChangeGameState: {
      req: req(
...
export const eavesdrop: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg(
      "give ",
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        coreToString.cardStr(state, targets[0], { visible: false }),
      " additional text",
    ),
    onChangeGameState: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreBoard.allInstalled(state, "corp").some(coreCard.ice),
      ),
    },
    async: true,
    effect: effect(
  ...
  events: [
    {
      ...
      trace: {
        ...
        req: req(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            utils.sameCard(targets[0]?.currentIce, card),
        ),
...
export const economicWarfare: CardDef = {
  ...
  onPlay: {
    ...
    onChangeGameState: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          (state as any).runner?.credit >= 4,
      ),
    },
    msg: "make the runner lose 4 [Credits]",
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreGaining.lose("runner", eid, 4);
      },
    ),
  },
};
...
export const electionDay: CardDef = {
  ...
  onPlay: {
    req: req(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        (state as any).corp?.hand?.filter((c: Card) => !utils.sameCard(c, card))
          .length > 0,
    ),
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        coreDrawing.draw(state, side, eid, 5);
      },
    ),
  },
};
...
export const endOfTheLine: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreDamage.damage(eid, "meat", 4, { card });
      },
    ),
  },
};
  ...
  onPlay: {
    trace: {
      ...
      successful: {
        async: true,
        prompt:
          "Choose an installed card not matching the faction of the Runner's identity",
        choices: {
          req: req(
            (
              state: State,
              ...
              return (
                ...
                coreCard.runner(t) &&
                (t.faction !== runnerFaction || runnerFaction === "Neutral")
              );
            },
          ),
        },
...
export const enhancedLoginProtocol: CardDef = {
  ...
  onPlay: {
    msg: "add an additional cost of [Click] to make the first run not through a card ability each turn",
  },
  staticAbilities: [
    {
...
export const exchangeOfInformation: CardDef = {
  ...
  onPlay: {
    ...
    choices: {
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          coreFlags.inRunnerScored(state, side, targets[0]),
      ),
    },
    async: true,
    effect: effect(
              ...
              choices: {
                req: req(
                  (
                    ...
                    card: Card,
                    targets: any[],
                  ) => coreFlags.inCorpScored(state, side, targets[0]),
                ),
              },
              msg: msg(
                "swap ",
...
export const fastBreak: CardDef = {
  ...
  onPlay: {
    ...
    msg: msg(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `gain ${(state as any).runner?.scored?.length || 0} [Credits]`,
    ),
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
...
export const financialCollapse: CardDef = {
  ...
  onPlay: {
    optional: {
        ...
        req: req(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            coreBoard
              ...
              .filter(coreCard.resource).length > 0,
        ),
      },
      player: "runner",
      waitingPrompt: true,
      ...
      yesAbility: {
        displaySide: "runner",
        cost: [corePayment.toC("resource", 1)],
        msg: ":cost",
      },
      noAbility: {
        player: "corp",
        ...
        msg: msg(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            `make the Runner lose ${coreBoard.allActiveInstalled(state, "runner").filter(coreCard.resource).length} [Credits]`,
        ),
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            coreGaining.lose(
              ...
              coreBoard
                .allActiveInstalled(state, "runner")
                .filter(coreCard.resource).length,
            );
          },
        ),
      },
...
export const freelancer: CardDef = {
  title: "Freelancer",
  onPlay: trashType("resource", coreCard.resource, true, 2, null, {
    req: req((state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      utils.isTagged(state),
    ),
  }),
  ...
  return {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          if (n < 2)
            return continue_ability(state, side, fhelper(n + 1), card, null);
          return coreEid.effectCompleted(state, side, eid);
        };
      },
    ),
...
export const fullyOperational: CardDef = {
  ...
  onPlay: {
    msg: msg(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `make ${(fullServersCount(state) || 0) + 1} gain/draw decisions`,
    ),
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
...
export const gameChanger: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreGaining.gainClicks((state as any).runner?.scored?.length || 0);
      },
    ),
  },
};
...
export const genotyping: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreMoving.mill(state, "corp", "corp", 2);
        coreShuffling.shuffleIntoRdEffect(state, side, eid, card, 4);
      },
    ),
  },
};
...
export const hanseiReview: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreGaining.gainCredits(state, "corp", 10);
        coreEid.effectCompleted(state, side, eid);
      },
    ),
  },
  ...
  onPlay: {
    trace: {
      ...
      successful: {
        ...
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            coreMoving.move("runner", targets[0], "hand", true);
          },
        ),
      },
    },
...
export const hellionAlphaTest: CardDef = {
  ...
  onPlay: {
    trace: {
      ...
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          (state as any).runner?.register?.lastTurn?.installedResource,
      ),
      successful: {
        ...
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            coreMoving.move("runner", targets[0], "deck", { front: true });
            coreSay.systemMsg(
              `adds ${targets[0]?.title} to the top of the Stack`,
            );
          },
        ),
      },
      unsuccessful: {
        ...
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            coreBadPublicity.gainBadPublicity("corp", 1);
          },
        ),
      },
    },
...
export const hellionBetaTest: CardDef = {
  ...
  onPlay: {
    trace: {
      ...
      req: req(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          (state as any).runner?.register?.lastTurn?.trashedAccessedCard,
      ),
      label: "Trash 2 installed non-program cards or take 1 bad publicity",
      successful: trashType(
        "non-program",
        (c: Card) => coreCard.facedown(c) || !coreCard.program(c),
        true,
        2,
        true,
      ),
...
export const highProfileTarget: CardDef = {
  ...
  onPlay: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreDamage.damage(eid, "meat", utils.countTags(state) * 2, { card });
      },
    ),
  },
};
  ...
  events: [
    {
      ...
      choices: {
        card: (c: Card) => coreCard.runner(c) && coreCard.inHandStar(state, c),
      },
      async: true,
      msg: msg(
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreMoving.trash("runner", eid, targets[0], {
            unpreventable: true,
            causeCard: card,
...
export const hyoubuPrecogManifold: CardDef = lockdown({
  ...
  onPlay: {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreUpdate.update(state, side, { ...card, cardTarget: targets[0] });
      },
    ),
  },
  events: [
    {
      ...
      psi: {
        req: req(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            coreServers.zoneToName((state as any).run?.server) ===
            card.cardTarget,
        ),
        notEqual: {
          msg: "end the run",
  ...
  onPlay: {
    trace: {
      ...
      successful: {
        ...
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            coreMoving.trashCards(state, side, eid, targets, {
              causeCard: card,
            });
          },
        ),
...
export const ipEnforcement: CardDef = (() => {
  const validAgenda = (c: Card, x: number): boolean => {
    ...
    const inScored = Array.isArray(zone)
      ? zone[0] === "scored"
      : zone === "scored";
    return (
      coreCard.agenda(c) &&
          ...
          effect: effect((s: State, sd: Side, e: EID, c: Card, t: any[]) => {
            ...
            coreInstalling.corpInstall(s, sd, e, target, null, {
              msgKeys: {
                installSource: c,
                known: true,
                includeCostFromEid: e,
                setZone: "the Runner score area",
                displayOrigin: true,
              },
    ...
    onPlay: {
      ...
      choices: {
        number: req(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            Math.min(
              coreTags.countTags(state),
              coreCosts.totalAvailableCredits(
                state,
                side,
                { ...eid, sourceType: "play" },
                card,
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          resolveFixedCostAbi(state, side, eid, card, targets[0]),
      ),
      cancel: {
        async: true,
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
            resolveFixedCostAbi(state, side, eid, card, 0),
        ),
      },
    },
  };
      ...
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          ...
          for (const c of corp.discard || []) {
            coreUpdate.update(state, side, { ...c, seen: false });
          }
