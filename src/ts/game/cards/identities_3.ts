  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return coreEvents.firstEvent(state, side, "runner-trash", (ctx: any) =>
          ctx.some((t: any) => t.context?.accessed),
        );
      }),
      async: true,
      ...
      effect: effect(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreDrawing.draw(state, "runner", 1, { suppressCheckpoint: true }),
          ],
          [],
        );
      ...
      effect: effect(function* (
        ...
        coreEffects.registerLingeringEffect(card, {
          ...
          req: req(function* (
            ...
            return coreCard.sameCard(ice, targets[0]);
          }),
          value: "Code Gate",
        });
      }),
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return coreEvents.firstEvent(
          ...
          side,
          "successful-run",
          (ctx: any) => (ctx[0]?.["subroutines-fired"] || 0) > 0,
        );
      }),
      interactive: true,
      ...
      effect: effect(function* (
        ...
        if (((state as any).corp?.hand || []).length === 0)
  ...
  abilities: [
    {
      ...
      msg: msg(
        "install a card in a remote server and place 1 advancement counter on it",
      ),
      effect: effect(function* (
        state: State,
        side: Side,
        ...
        continue_ability(
          ...
          side,
          {
            ...
            effect: effect(function* (
              ...
              coreFlags.registerPersistentFlag!(
                state2,
                side2,
                card2,
                "can-rez",
                (st: any, _: any, c: any) => (c.cid === tgtcid ? false : true),
              );
                ...
                (st: any, _: any, c: any) => {
                  ...
                  if (req != null && adv != null && req > adv) {
                    coreToasts.toast(
                      st,
                      "corp",
                      "Cannot score due to Saraswati Mnemonics: Endless Exploration.",
                      "warning",
                    );
                    return false;
                  }
              ...
              coreInstalling.corpInstall(state2, side2, eid2, chosen, t2[0], {
                counters: { "advance-counter": 1 },
                msgKeys: { "install-source": card2, "display-origin": true },
              });
            }),
          },
          card,
  ...
  staticAbilities: [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          coreCard.hasSubtype(targets[0], "Connection") &&
          side === "corp"
        );
      }),
  ...
  abilities: [
    {
      req: req(function* (
        ...
        return run && ((state as any).corp?.discard || []).length > 0;
      }),
...
export const card_SkorpiosDefenseSystems_PersuasivePower: CardDef = {
  ...
  implementation:
    'Switch between Manual, "Smart", and Automatic resolution by using the ability on the card',
  events: [
    {
      ...
      effect: effect(function* (
        ...
        coreUpdate.update!(state, side, {
          ...card,
          special: { ...(card as any).special, "resolution-mode": "Smart" },
        });
        coreToasts.toast(
          state,
          "corp",
          "Set Skorpios resolution to Smart mode",
        );
        coreUpdate.update!(state, side, {
    ...
    },
    {
      ...
      req: req(function* (
        ...
        const validCards = (
          coreEvents.turnEvents(state, "runner", "damage") || []
        ).filter((e: any) =>
          e[0]?.card
            ? coreCard.runner(coreCard.getCard(state, e[0].card))
            : false,
        );
        ...
        if (resType === "Smart") {
          ...
          if (
            playArea.some(
              (c: Card) => c.title && relevantCards.includes(c.title),
            )
          )
            return true;
          const bufferDrive = (
            coreBoard.allActiveInstalled(state, "runner") || []
          ).some((c: Card) => c.title === "Buffer Drive");
          if (bufferDrive) {
            const relevantTrashed = [
              "I've Had Worse",
              "Strike Fund",
              "Steelskin Scarring",
              "Crowdfunding",
            ...
            if (
              contextCards.some(
                (c: any) => c.title && relevantTrashed.includes(c.title),
              )
            )
              return true;
            if (contextCards.some((c: any) => c.type === "program"))
              return true;
          }
          return validCards.some((c) => coreCard.program(c as Card));
        }
        return false;
      }),
      ...
      choices: (
        ...
        card: Card,
        targets: any[],
      ) => corePrompts.cancellable((targets as any)[0]?.cards || []),
  ...
  abilities: [
    {
      ...
      effect: effect(function* (
        ...
        const helper = coreChooseOne.chooseOneHelper(
          { optional: true, label: "Set resolution mode" },
          ["Manual", "Smart", "Automatic"].map((x: string) => ({
            option: x,
            ability: {
              effect: effect(function* (
                st: any,
                ...
                coreUpdate.update!(st, s, {
                  ...c,
                  special: { ...(c as any).special, "resolution-mode": x },
                });
                coreToasts.toast(
                  st,
                  "corp",
                  `Set Skorpios resolution to ${x} mode`,
                );
                coreUpdate.update!(st, s, {
                  ...coreCard.getCard(st, c),
                  "card-target": x,
                });
              }),
            },
    ...
    },
    {
      ...
      once: "per-turn",
      choices: (state: any) =>
        corePrompts.cancellable(
          ((state as any).runner?.discard || []) as Card[],
        ),
      msg: msg("remove ", (c: Card) => c.title || "", " from the game"),
      effect: effect(function* (
        state: State,
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          contextCard &&
          coreCard.hasSubtype(contextCard, "Advertisement") &&
      ...
      effect: effect(function* (
        ...
        coreGaining.loseCredits("runner", eid, 1);
      }),
  ...
  events: [
    {
      ...
      optional: {
        req: req(function* (
          ...
          ).filter((c: Card) => coreCard.agenda(c) && coreCard.faceup(c));
          const selectableIce = (
            coreBoard.allInstalled(state, "corp") || []
          ).filter(
            (c: Card) =>
              ...
              (coreCard.getCounters(c, "advancement") || 0) === 0,
          );
        ...
        yesAbility: {
          ...
          effect: effect(function* (
            ...
            const agendaPoints = installed.reduce(
              (sum: any, c: any) => sum + (c.agendapoints || 0),
              0,
            );
            continue_ability(
              state,
      ...
      optional: {
        req: req(function* (
          ...
          return (
            context.targetServer === "hq" &&
            coreEvents.firstSuccessfulRunOnServer(state, "hq") &&
            ((state as any).runner?.discard || []).length >= 2 &&
        ...
        yesAbility: {
          ...
          choices: {
            max: 2,
            all: true,
            card: (c: Card) => coreCard.inDiscard(c) && coreCard.runner(c),
          },
          effect: effect(function* (
            state: State,
            side: Side,
  ...
  events: [
    ...
    },
    {
      ...
      effect: effect(function* (
        ...
        if (((state as any).corp?.discard || []).length === 0) {
...
export const card_SunnyLebeau_SecuritySpecialist: CardDef = {
  ...
  staticAbilities: [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          coreCard.sameCard(
            (targets as any)[0],
            ((state as any).runner as any)?.["basic-action-card"],
          ) &&
    ...
    },
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          coreCard.sameCard(
            (targets as any)[0],
            ((state as any).corp as any)?.["basic-action-card"],
          ) &&
          (card as any)["ability"]?.label ===
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return coreEvents.firstEvent(
          ...
          side,
          "runner-lose-tag",
          (ctx: any) => (ctx[0]?.context?.amount || 0) > 0,
        );
      }),
...
export const card_SyntheticSystems_TheWorldReimagined: CardDef = {
    ...
    },
    {
      ...
      optional: {
        req: req(function* (
          ...
          return (
            hasMostFaction(state, "corp", "Jinteki") &&
            (coreBoard.allInstalled(state, "corp") || []).filter((c: Card) =>
              coreCard.ice(c),
            ).length >= 2
          );
        }),
  ...
  flags: {
    "corp-phase-12": req(function* (
      ...
      if (gc?.disabled || coreEffects.isDisabled(state, side, card))
        return false;
      return (
        hasMostFaction(state, "corp", "Jinteki") &&
        (coreBoard.allInstalled(state, "corp") || []).filter((c: Card) =>
        ...
        yesAbility: {
          ...
          choices: {
            req: req(function* (
              ...
              return coreCard.installed(targets[0]) && coreCard.ice(targets[0]);
            }),
            max: 2,
            all: true,
          },
...
export const card_TenninInstitute_TheSecretsWithin: CardDef = {
  ...
  events: [
    {
      msg: msg(
        "place 1 advancement token on ",
        (c: Card) => coreToString.cardStr(state, c) || "",
      ),
      label:
        "Place 1 advancement token on a card if the Runner did not make a successful run last turn",
      ...
      req: req(function* (
        ...
        return coreEvents.notLastTurn(state, "runner", "successful-run");
      }),
      waitingPrompt: true,
      once: "per-turn",
      async: true,
      ...
      effect: effect(function* (
        ...
        coreProps.addProp(eid, targets[0], "advance-counter", 1, {
          placed: true,
        });
      }),
    },
...
export const card_TheCatalyst_ConventionBreaker: CardDef = {
  ...
  events: [
    {
      ...
      effect: effect(function* (
        ...
        const filteredContext = {
          cid: context.card?.cid,
          idx: context["ability-idx"],
        };
        if (
          currentQueue &&
          currentQueue.length > 0 &&
          JSON.stringify(currentQueue[0]) === JSON.stringify(filteredContext)
        ) {
          const newQueue = [...currentQueue, filteredContext];
          coreUpdate.update!(state, side, {
            ...card,
            special: { ...(card as any).special, "previous-actions": newQueue },
          });
          ...
          coreUpdate.update!(state, side, {
            ...
            special: {
              ...(card as any).special,
              "previous-actions": [filteredContext],
            },
          });
          return coreEid.effectCompleted(state, side, eid);
        }
    ...
    },
    {
      ...
      effect: effect(function* (
        ...
        coreUpdate.update!(state, side, {
          ...card,
          special: { ...(card as any).special, "previous-actions": null },
        });
      }),
    },
  ],
      ...
      optional: {
        ...
        req: req(function* (
          ...
          return (
            ...
            coreEvents.firstEvent(state, "runner", "rez", (ctx: any) => {
              const c = ctx.card ? coreCard.getCard(state, ctx.card) : null;
        ...
        yesAbility: {
          effect: effect(function* (
            ...
            const contextCard = context.card
              ...
              : null;
            const foundCard = [
              ...((state as any).corp?.deck || []),
              ...((state as any).corp?.playArea || []),
            ].find((c: Card) => c.title === contextCard?.title);
            if (foundCard) {
              coreMoving.move(state, side, foundCard, "hand");
              coreSay.systemMsg(
                state,
                side,
                `uses ${card.title} to add a copy of ${foundCard.title} to HQ, and shuffle R&D`,
              );
...
export const card_TheProfessor_KeeperOfKnowledge: CardDef = {
...
export const card_TheSyndicate_ProfitOverPrinciple: CardDef = {
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const source = context.source
          ? coreCard.getCard(state, context.source)
          : null;
        return (
          (source && coreCard.agenda(source)) ||
          (source &&
            coreCard.operation(source) &&
            coreEvents.firstEvent(
              state,
              side,
              "corp-credit-gain",
              (ctx: any) => {
                const s = ctx[0]?.source
                  ? coreCard.getCard(state, ctx[0].source)
                  : null;
        ...
        continue_ability(
          ...
          side,
          {
            ...
            msg: msg(function* (t: string): Generator<any, any, any> {
              if (t === "Pay [Click] and 2 [Credits]")
                return "force the runner to " + t.toLowerCase();
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          contextCard &&
          coreCard.ice(contextCard) &&
          (coreCard.hasSubtype(contextCard, "AP") ||
            coreCard.hasSubtype(contextCard, "Destroyer"))
        );
      }),
      msg: msg(
        "give ",
        (c: Card) => coreToString.cardStr(state, c) || "",
        ' +1 strength and "End the run unless the Runner trashes 1 of their installed cards" after its other subroutines',
      ),
      async: true,
      effect: effect(function* (
        state: State,
        ...
        const thunderboltSub: any = {
          ...
          choices: (() => {
            ...
            if (
              corePayment.canPay(state, "runner", eid, card, null, [
                corePayment.toC("trash-installed", 1),
              ])
            ) {
              opts.push(
                corePayment.costToString(
                  corePayment.toC("trash-installed", 1),
                ) || "trash-installed",
              );
            }
          ...
          msg: msg(function* (t: string): Generator<any, any, any> {
            return t === "End the run"
              ? "end the run"
              : "force the runner to " + t.toLowerCase();
          }),
          effect: effect(function* (
            state: State,
            side: Side,
            ...
            if (targets[0] === "End the run") {
              ...
              if (asyncResult) {
                coreSay.systemMsg(
                  state,
                  "runner",
                  `${(asyncResult as any).msg} due to ${card.title} subroutine`,
                );
              }
              return coreEid.effectCompleted(state, side, eid);
            }
        ...
        coreEffects.registerLingeringEffect(card, {
          ...
          req: req(function* (
            ...
            return (
              coreCard.rezzed(targets[0]) && coreCard.sameCard(t, targets[0])
            );
          }),
  ...
  abilities: [
    {
      ...
      label:
        "Install 1 card from your grip, paying 2{c} less. When you install that card, suffer 1 meat damage.",
      effect: effect(function* (
        state: State,
        side: Side,
        eid: EID,
        ...
        const evs = coreEngine.registerEvents(state, side, card, [
          {
            ...
            effect: effect(function* (
              ...
              coreDamage.damage(state, side, eid, "meat", 1);
            }),
          },
        ]);
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreInstalling.runnerInstall(state, side, targets[0], {
              "cost-bonus": -2,
              msgKeys: { "include-cost-from-eid": eid, "install-source": card },
            }),
          ],
          [],
        );
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          side === "runner" && ((state as any).corp?.badPublicity || 0) === 0
        );
      }),
      effect: effect(function* (
        state: State,
  ...
  abilities: [
    {
      ...
      msg: msg(function* (tagged: boolean): Generator<any, any, any> {
        return tagged ? "draw 1 card and remove 1 tag" : "draw 1 card";
      }),
      async: true,
      changeInGameState: {
        req: req(function* (
      ...
      req: req(function* (
        ...
        return (
          ...
          (coreEid.sourceType(eid) === "advance" ||
            coreEid.isBasicAdvanceAction(eid))
        );
      }),
      type: "recurring",
    },
...
export const card_WeylandConsortium_BuilderOfNations: CardDef = {
  ...
  implementation:
    "[Erratum] The first time an encounter with a piece of ice with at least 1 advancement counter ends each turn, do 1 meat damage.",
  events: [
    {
        ...
        return coreEvents.firstEvent(
          ...
          (ctx: any) => {
            ...
            return (
              ice &&
              coreCard.rezzed(ice) &&
              (coreCard.getCounters(ice, "advancement") || 0) > 0
            );
          },
        );
      }),
      ...
      effect: effect(function* (
        ...
        coreDamage.damage(eid, "meat", 1, { card });
      }),
    },
  ],
};
  ...
  interactions: {
    "pay-credits": {
      req: req(function* (
        ...
        return (
          coreEid.sourceType(eid) === "runner-trash-corp-cards" &&
          coreCard.corp(targets[0])
        );
      }),
      type: "recurring",
  ...
  events: [
    ...
    },
    {
      ...
      req: req(function* (
        ...
        return (
          hasMostFaction(state, "runner", "Anarch") &&
          coreCard.corp((targets as any)[0]?.card || {})
        );
      }),
      effect: effect(function* (
        state: State,
        ...
        if (lastCard) {
          coreShuffling.shuffleCardsIntoDeck!(state, "runner", card, [
            lastCard,
          ]);
        }
      }),
  ...
  events: [
    {
      ...
      optional: {
        ...
        once: "per-turn",
        yesAbility: {
          msg: msg("gain ", (n: number) => n.toString(), " [Credits]"),
          once: "per-turn",
          async: true,
          effect: effect(function* (
            state: State,
