              ...
              coreEvents.registerEvents(s, sd, c, [
                Object.assign({}, coreDefHelpers.doNetDamage(3), {
                  ...
                  req: req(function* (
                    ...
                    const ctx = tgts2[0];
                    return (
                      coreCard.sameCard(ctx?.ice, targetIce) &&
                      (ctx?.ice?.subroutines ?? []).some(
                        (sub: any) => !sub.broken,
                      )
                    );
                  }),
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          (card as any).rezzed === ":this-turn" &&
          coreCard.sameCard(context?.ice, card)
        );
      }),
      async: true,
      effect: req(function* (
        ...
        const promptAbility = {
          ...
          effect: req(function* (
              ...
              innerAbility = {
                ...
                effect: effect(function* (
                  ...
                  yield wait_for(
                    ...
                    [
                      { asyncResult: "result" },
                      coreTags.gainTags(s2, ":runner", e2, 2, {
                        unpreventable: true,
                      }),
                    ],
                    [],
              ...
              innerAbility = {
                ...
                effect: req(function* (
                  ...
                  yield wait_for(
                    ...
                    [
                      { asyncResult: "result" },
                      corePayment.pay(s2, ":runner", e2, c2, [
                        corePayment.toC("net", 3),
                      ]),
                    ],
                    [],
            ...
            const resolvePlayer =
              target === "Corp trashes 1 Runner card" ? ":corp" : ":runner";
            yield wait_for(
              s,
              [
                { asyncResult: "result" },
    ...
    },
    {
      ...
      "change-in-game-state": {
        ...
        req: req(function* (
          ...
          return (
            installed.some((c: Card) => coreCard.program(c)) ||
            (wonderSub(card, 3) &&
              installed.some((c: Card) => coreCard.resource(c)))
          );
        }),
      },
      effect: req(function* (
  ...
  subroutines: [
    {
      ...
      msg: msg(function (state: State) {
        return `do ${utils.availableMu(state)} net damage`;
      }),
      "change-in-game-state": {
        silent: true,
        req: req(function* (state: State): Generator<any, any, any> {
          return utils.availableMu(state) > 0;
        }),
      },
      async: true,
      effect: effect(function* (
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          coreServers.cardToServer(state, card) ===
            coreServers.cardToServer(state, targets[0])
        );
      }),
      effect: effect(function* (
        state: State,
    ...
    },
    {
      ...
      req: req(function* (
        ...
        const context = targets[0];
        return (
          !coreCard.sameCard(card, context?.card) &&
          coreServers.cardToServer(state, card) ===
            coreServers.cardToServer(state, context?.card)
        );
      }),
      effect: effect(function* (
        state: State,
    ...
    return {
      ...
      effect: req(function* (
        ...
        const remaining = cards.filter(
          (c: Card) => !coreCard.sameCard(c, targets[0]),
        );
        const reorder = coreIce.reorderChoice(
          ":runner",
          ":runner",
          remaining,
          [],
        ...
        effect: req(function* (
          ...
          if (c > 1) {
            ...
            yield wait_for(
              ...
              [
                { asyncResult: "result" },
                coreMoving.trash(state, side, eid, from[0], {
                  unpreventable: true,
                  cause: ":subroutine",
                }),
              ],
  ...
  "on-encounter": {
    ...
    effect: req(function* (
      ...
      const ability = {
        ...
        msg: msg(function () {
          return `add ${utils.quantify(n, "card")} from the grip to the top of the stack`;
        }),
        effect: effect(function* (
          s: State,
          sd: Side,
          ...
          for (const t of tgts)
            coreMoving.move(s, ":runner", t, ":deck", { front: true });
        }),
      };
      yield wait_for(
        state,
  ...
  subroutines: [
    ...
    Object.assign({}, coreDefHelpers.doNetDamage(2), {
      ...
      "change-in-game-state": {
        silent: true,
        req: req(function* (state: State): Generator<any, any, any> {
          return utils.countTags(state) >= 2;
        }),
      },
    }),
  ],
...
export const drafter: CardDef = {
  ...
  subroutines: [
    installFromArchivesSub(),
    installFromHqOrArchivesSub({ ignoreAllCost: true }),
  ],
};
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const context = targets[0];
        return (
          coreCard.hasSubtype(context?.card, "Harmonic") &&
          coreCard.ice(context?.card)
        );
      }),
      async: true,
      effect: effect(function* (
  ...
  "static-abilities": [
    {
      ...
      value: req(function* (
        ...
        return {
          subroutines: Array(coreCard.getCounters(card, ":power")).fill(
            endTheRun,
          ),
        };
      }),
          ...
          const result: any = yield wait_for(
            ...
            [
              ...
              corePayment.pay(
                ...
                coreEid.makeEid(
                  state,
                  Object.assign({}, eid, { sourceType: ":subroutine" }),
                ),
                card,
          ...
          yield wait_for(
            ...
            [
              ...
              coreEngine.resolveAbility(
                state,
                side,
                breakFn(unbrokenSubs.slice(1), total + 1),
                card,
                null,
              ),
            ],
    ...
    "runner-abilities": [
      {
        req: req(function* (
          ...
          const unbroken = ((card as any).subroutines ?? []).filter(
            (s: any) => !s.broken && s.resolve !== false,
          );
          return (
            unbroken.length <=
            corePayment.totalAvailableCredits(state, ":runner", eid, card)
          );
        }),
        async: true,
        ...
        effect: req(function* (
          ...
          const resolved = unbroken.reduce(
            (c: Card, s: any) => coreIce.resolveSubroutine(c, s),
            card,
          );
  ...
  subroutines: [
      ...
      "change-in-game-state": {
        ...
        req: req(function* (state: State): Generator<any, any, any> {
          return coreBoard
            ...
            .some((c: Card) => coreCard.hasSubtype(c, "Console"));
        }),
      },
      choices: {
        card: (c: Card) =>
          coreCard.hasSubtype(c, "Console") && coreCard.installed(c),
      },
      msg: msg(function (
        state: State,
        side: Side,
    ...
    },
    {
      ...
      "change-in-game-state": {
        ...
        req: req(function* (state: State): Generator<any, any, any> {
          return coreBoard
            ...
            .some(
              (c: Card) =>
                coreCard.hasSubtype(c, "Virtual") && coreCard.resource(c),
            );
        }),
      },
      async: true,
      effect: req(function* (
        ...
        const cards = coreBoard
          .allActiveInstalled(state, ":runner")
          .filter((c: Card) => coreCard.hasSubtype(c, "Virtual"));
        yield wait_for(
          state,
          [
            { asyncResult: "result" },
            coreMoving.trashCards(state, side, eid, cards, {
              cause: ":subroutine",
            }),
          ],
          [],
        ...
        coreEvents.registerEvents(state, side, card, [
          {
            ...
            req: req(function* (
              ...
              return (
                revealedCards.every((rc: Card) => coreCard.inHand(rc)) &&
            ...
            effect: req(function* (
              ...
              const trashAbility = coreRevealing.withRevealedHand(
                ":runner",
                { skipReveal: true },
                {
                  ...
                  effect: req(function* (
                    ...
                    yield wait_for(
                      ...
                      [
                        { asyncResult: "result" },
                        coreMoving.trash(s2, sd2, e2, tgts2[0], {
                          cause: ":subroutine",
                        }),
                      ],
                      [],
  ...
  subroutines: [
    {
      ...
      async: true,
      msg: msg(function (state: State, side: Side, eid: EID, card: Card) {
        return `trash ${(card as any).title}`;
      }),
      effect: effect(function* (
        state: State,
        side: Side,
  ...
  const sub: any = {
    ...
    effect: req(function* (
      ...
      if (targets[0] === "Gain 1 [Credits]") {
        ...
        yield wait_for(
          state,
          [{ asyncResult: "result" }, coreDrawing.draw(state, ":corp", eid, 1)],
          [],
        );
      }
    }),
