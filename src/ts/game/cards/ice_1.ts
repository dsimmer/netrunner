...
export function bioraidBreak(cost: number, qty: number, args: any = {}): any {
  return coreIce.breakSub(
    [corePayment.toC("lose-click", cost)],
    qty,
    null,
    Object.assign({}, args, {
      req: req(function* (
        ...
        return (
          ...
          currentlyEncounteringCard(card, state)
        );
      }),
    }),
  );
...
export const endTheRun: any = {
  ...
  effect: effect(function* (
    ...
    yield wait_for(
      state,
      [{ asyncResult: "result" }, coreRuns.endRun(state, ":corp", eid, card)],
      [],
    );
  }),
};
...
export function endTheRunUnlessRunnerPays(
  cost: any,
  reason: string = "subroutine",
): any {
  return {
    player: ":runner",
    async: true,
    ...
    effect: req(function* (
      ...
      if (target === "End the run") {
        ...
        const result: any = yield wait_for(
          ...
          [
            { asyncResult: "result" },
            corePayment.pay(state, ":corp", coreEid.makeEid(state, eid), card, [
              cost,
            ]),
          ],
          [],
...
export function endTheRunUnlessRunner(
  label: string,
  prompt: string,
  ability: any,
): any {
...
export function traceAbility(base: number, ability: any, unAbility?: any): any {
  if (unAbility) {
    ...
    return {
      label: `Trace ${base} - ${label}`,
      trace: { base, label, successful: ability, unsuccessful: unAbility },
    };
  }
  return {
    label: `Trace ${base} - ${ability.label}`,
    trace: { base, label: ability.label, successful: ability },
  };
}
      ...
      if (canPay) {
        ...
        ability = {
          ...
          effect: req(function* (
            ...
            yield wait_for(
              s,
              [{ asyncResult: "result" }, coreTags.gainTags(s, sd, e, 1)],
              [],
            );
          }),
        };
...
export function corpsGainsAndRunnerLosesCredits(
  gain: number,
  loss: number,
): any {
  return {
...
export const runnerTrashInstalledSub: any = Object.assign(
  {},
  trashInstalledSub,
  {
    player: ":runner",
...
export function installFromArchivesSub(args: any = {}): any {
  return {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.corpInstallableType(c) && coreCard.inDiscard(c),
    },
    async: true,
    effect: effect(function* (
      state: State,
    ...
    coreFlags.registerRunFlag(
      ...
      (_state: State, _side: Side, _card: Card) => {
        coreToasts.toast(
          state,
          ":runner",
          "Cannot steal due to subroutine.",
          "warning",
        );
        return false;
      },
      ...
      (_state: State, _side: Side, c: Card) => {
        if (coreCard.corp(c)) {
          coreToasts.toast(
            state,
            ":runner",
            "Cannot trash due to subroutine.",
            "warning",
          );
          return false;
        }
...
const revealGrail: any = {
  prompt:
    "Reveal up to 2 pieces of Grail ice from HQ (first ice chosen will be first sub)",
...
export function trashTypeOrEndTheRun(
  typeName: string,
  typeFn: (c: Card) => boolean,
  sub: any,
): any {
  return {
...
export function variableSubsIce(
  subsCount: (
    state: State,
    side: Side,
    eid: EID,
...
export function subtypeIceCount(corp: any, subtype: string): number {
  ...
  return Object.values(servers)
    .flatMap((s: any) => s?.ices || [])
    .filter(
      (ice: Card) => coreCard.rezzed(ice) && coreCard.hasSubtype(ice, subtype),
    ).length;
}
...
function constellationIce(ability: any): any {
  ...
  return {
    subroutines: [
      Object.assign({}, base, {
        trace: Object.assign({}, base.trace, {
          kicker: ability,
          "kicker-min": 5,
        }),
      }),
    ...
    "static-abilities": [
      {
        ...
        value: req(function* (
          ...
          return {
            position: ":front",
            subroutines: Array(coreCard.getCounters(card, ":advancement")).fill(
              sub,
            ),
          };
        }),
...
export function resolveAnotherSubroutine(
  ...
  const predFn = (card: Card, target: Card) =>
    coreCard.ice(target) &&
    coreCard.rezzed(target) &&
    ((target as any).subroutines?.length ?? 0) >= 1 &&
    (allowSameCard || !coreCard.sameCard(card, target)) &&
    pred(target);
  return {
    async: true,
    label,
    ...
    effect: effect(function* (
      ...
      const chosenIce: Card = yield wait_for(
        state,
        [
          { asyncResult: "result" },
          coreEngine.resolveAbility(
...
export const takeBadPub: any = {
  ...
  effect: effect(function* (
    ...
    coreSay.systemMsg(
      state,
      side,
      `takes 1 bad publicity from ${(card as any).title}`,
    );
    yield wait_for(
      state,
      [
...
export const ablativeBarrier: CardDef = {
  ...
  "on-rez": {
    ...
    prompt:
      "Choose a non-agenda card to install from Archives or HQ in another server",
    "waiting-prompt": true,
    "show-discard": true,
    choices: {
      card: (c: Card) =>
