...
export const sdsDroneDeployment: CardDef = {
  ...
  "steal-cost-bonus": req(function* (
    ...
    return [corePayment.toC("program", 1)];
  }),
  "on-score": {
    req: req(function* (
      state: State,
  ...
  "move-zone": req(function* (
    ...
    if (coreCard.inScored(card) && (card as any)["scored-side"] === ":corp") {
      coreSay.systemMsg(
        state,
        side,
        `uses ${card.title} to decrease the Runner's maximum hand size by 1`,
      );
    }
  ...
  abilities: [
    {
      ...
      choices: {
        card: (c: Card) =>
          coreCard.ice(c) && !coreCard.rezzed(c) && coreCard.installed(c),
      },
      async: true,
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
  ...
  abilities: [
    {
      ...
      req: req(function* (
        ...
        return allInstalled.some(
          (c: Card) => coreCard.hasSubtype(c, "Bioroid") && !coreCard.rezzed(c),
        );
      }),
      ...
      choices: {
        card: (c: Card) =>
          coreCard.hasSubtype(c, "Bioroid") && !coreCard.rezzed(c),
      },
      async: true,
      effect: effect(function* (
        state: State,
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreRezzing.rez(state, side, target, {
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          coreCard.rezzed(ice) &&
          (coreCard.hasSubtype(ice, "Code Gate") ||
            coreCard.hasSubtype(ice, "Sentry")) &&
          coreEvents.firstEvent(state, side, "pass-ice", (t: any[]) => {
            const first = t[0];
            const firstIce = first?.ice || {};
            return (
              coreCard.rezzed(firstIce) &&
              (coreCard.hasSubtype(firstIce, "Code Gate") ||
                coreCard.hasSubtype(firstIce, "Sentry"))
            );
          })
        );
      }),
        ...
        continue_ability(
          ...
          side,
          {
            prompt: (
              ...
              card: Card,
              targets: any[],
            ) => `Make the runner encounter ${encIce?.title} again?`,
            choices: req(function* (
              ...
              if (
                ...
                opts.push("Pay 1 [Credit]");
              if (
                corePayment.canPay(state, ":corp", eid, card, null, [
                  corePayment.toC("trash-from-hand", 1),
                ])
              )
                opts.push("Trash 1 card from HQ");
              opts.push("Done");
                ...
                continue_ability(
                  ...
                  side,
                  {
                    ...
                    msg: (
                      ...
                      `make the runner encounter ${coreToString.cardStr(state, encIce)} again`,
                    async: true,
                    effect: effect(
                      coreRuns.forceIceEncounter(state, side, eid, encIce),
                    ),
                  },
                  card,
                  null,
...
export const slashAndBurnAgriculture: CardDef = {
  ...
  expend: Object.assign(coreDefHelpers.placeAdvancementCounter(true, 2), {
      ...
      optional: {
        ...
        "yes-ability": {
          ...
          msg: (
            ...
            `gain ${Math.min(3, coreCard.getCounters(card, ":credit") || 0)} [Credits]`,
          effect: effect(function* (
            state: State,
            side: Side,
            eid: EID,
            ...
            if ((coreCard.getCounters(card, ":credit") || 0) > 0) {
              yield wait_for(
                ...
                [
                  { asyncResult: "result" },
                  coreEngine.takeCredits(state, side, eid, card, ":credit", 3),
                ],
                [],
              );
            } else {
...
export const standoff: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(function* (
      ...
      corePrompts.showWaitPrompt(
        String(
          (side === ":corp" ? "Runner" : "Corp") +
            " to trash a card for Standoff",
        ),
      );
...
function stand(side: string): any {
  return {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.installed(c) && coreTags.sameSide(side, (c as any).side),
    },
    cancel: {
      "display-side": side,
      ...
      effect: effect(function* (
        ...
        if (side === ":runner") {
          yield wait_for(
            state,
            [{ asyncResult: "result" }, coreDrawing.draw(state, ":corp", 1)],
            [],
          );
          ...
          coreSay.systemMsg(
            state,
            ":corp",
            `uses ${card.title} to draw 1 card and gain 5 [Credits]`,
          );
    ...
    effect: effect(function* (
      ...
      coreSay.systemMsg(
        state,
        side,
        `trashes ${coreToString.cardStr(state, target)} for ${card.title}`,
      );
      corePrompts.clearWaitPrompt(state, coreTags.otherSide(side));
      corePrompts.showWaitPrompt(
        state,
        side,
        `${coreTags.sideStr(coreTags.otherSide(side))} to trash a card for ${card.title}`,
      );
      continue_ability(
        state,
        coreTags.otherSide(side),
        stand(coreTags.otherSide(side)),
        card,
        null,
      );
    }),
  ...
  events: [
    {
      ...
      effect: effect(function* (
        ...
        const target: any = (targets as any[])?.[0];
        const rezzedTargets = (coreBoard.allInstalledCorp(state) || []).filter(
          (c: Card) =>
            coreCard.ice(c) &&
            coreCard.rezzed(c) &&
            (coreCard.getZone(c) || [])[1] !==
              (forms.context(state, card, targets) || {}).server,
        );
        if (rezzedTargets.length > 0) {
          continue_ability(
            ...
            {
              prompt:
                "Choose a piece of ice protecting another server to derez",
              "waiting-prompt": true,
              choices: {
                req: req(function* (
                  state: State,
              ...
              effect: effect(function* (
                ...
                yield wait_for(
                  ...
                  [
                    { asyncResult: "result" },
                    coreRezzing.derez(state, side, target, {
                      msgKeys: { "and-then": " and gain 1 [Credits]" },
                    }),
                  ],
                  [],
                );
    ...
    },
    {
      ...
      req: req(function* (
        ...
        return (
          forms.run(state) &&
          coreEvents.firstRunEvent(state, side, "derez", (ctx: any[]) =>
            ctx.some((c: any) => coreCard.ice(c.card)),
          )
        );
      }),
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return (
          ...
          coreEvents.runEventCount(state, side, "derez", (ctx: any[]) =>
            ctx.some((c: any) => coreCard.ice(c.card)),
          ) >= 1
        );
      }),
...
export const sting: CardDef = {
  ...
  "on-score": {
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      `deal ${1 + countOppStings(state, ":corp")} net damage`,
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreDamage.damage(eid, ":net", 1 + countOppStings(state, ":corp"), {
          card: card,
        });
      },
    ),
  ...
  stolen: {
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
      `deal ${1 + countOppStings(state, ":runner")} net damage`,
    async: true,
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreDamage.damage(eid, ":net", 1 + countOppStings(state, ":runner"), {
          card: card,
        });
      },
    ),
  ...
  "derezzed-events": [
    {
      ...
      optional: {
        prompt:
          "Reveal this agenda to gain 2 [Credits] and place 1 advancement counter on an installed card?",
        req: req(function* (
          state: State,
          side: Side,
          eid: EID,
          ...
          const target: any = (targets as any[])?.[0];
          return (
            (card as any).previousZone?.[0] !== ":hand" &&
        ...
        "yes-ability": {
          msg: (
            ...
            card: Card,
            targets: any[],
          ) => `reveal itself from ${(card as any).previousZone?.[0]}`,
          async: true,
          effect: effect(function* (
            state: State,
            side: Side,
...
function scoreAbi(credGain: number): any {
  return {
    ...
    effect: effect(function* (
      ...
      const placeAbi = Object.assign(
        coreDefHelpers.placeAdvancementCounter(null, 1),
        {
          req: req(function* (
            state: State,
            side: Side,
...
export const successfulFieldTest: CardDef = {
  ...
  "on-score": {
    ...
    effect: effect(function* (
      ...
      const handOps = (corp?.hand || []).filter(
        (c: Card) => !coreCard.operation(c),
      );
...
function sft(n: number, maxOps: number): any {
  return {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.corp(c) && !coreCard.operation(c) && coreCard.inHand(c),
    },
    effect: effect(function* (
      state: State,
      side: Side,
  ...
  prevention: [
    {
      ...
      ability: {
        ...
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            coreDamage.damageBoost(state, side, eid, 1);
          },
        ),
      },
    },
        ...
        effect: effect(
          (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
            coreFlags.registerRunFlag(
              card,
              ":can-steal",
              function (_s: State, _sd: Side, c: Card) {
                return !coreCard.sameCard(c, card);
              },
            );
        ...
        continue_ability(
          ...
          side,
          {
            ...
            choices: req(function* (
              ...
              return coreBoard.installableServers(state, chosenIce);
            }),
            async: true,
            effect: effect(function* (
              state: State,
              ...
              const zone = [
                ...coreBoard.serverToZone(state, chosenServer),
                ":ices",
              ];
              const numIce = (
                (state as any).corp?.servers?.[zone[1]]?.ices || []
              ).length;
              continue_ability(
                state,
                side,
                {
                  ...
                  choices: Array.from({ length: numIce + 1 }, (_, i) =>
                    String(i),
                  ).reverse(),
                  async: true,
                  effect: effect(function* (
  ...
  events: [
    {
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        const adv =
          coreCard.getCounters(coreCard.getCard(state, card), ":advancement") ||
          0;
        const n = adv >= 4 ? 2 : 1;
...
function adv4(s: State, c: Card): number {
  return (coreCard.getCounters(coreCard.getCard(s, c), ":advancement") || 0) >=
    4
    ? 2
    : 1;
}
...
export const unorthodoxPredictions: CardDef = {
  ...
  "on-score": {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      ...
      return ((): string => {
        const target: any = (targets as any[])?.[0];
        return `prevent subroutines on ${target} ice from being broken until next turn`;
      })();
    },
  },
};
  ...
  "static-abilities": [
    {
      ...
      req: req(function* (
        ...
        return (
          (coreCard.getCounters(targets[0] || {}, ":advancement") || 0) > 0
        );
      }),
      value: req(function* (
        state: State,
        ...
        return corePayment.toC(
          "credit",
          2 * (coreCard.getCounters(targets[0] || {}, ":advancement") || 0),
        );
      }),
    },
  ],
        ...
        coreEngine.registerEvents(card, [
          {
            ...
            msg: (
              ...
              card: Card,
              targets: any[],
            ) => `do ${(state as any).runner?.hand?.length} net damage`,
            async: true,
            effect: effect(
              coreDamage.damage(
                eid,
...
export const vulnerabilityAudit: CardDef = {
  ...
  flags: {
    "can-score": req(function* (
      ...
      const result = (card as any).installed !== "this-turn";
      if (!result) {
        coreToasts.toast(
          state,
          ":corp",
          "Cannot score Vulnerability Audit the turn it was installed.",
          "warning",
        );
      }
      return result;
      ...
      effect: effect(function* (
        ...
        if ((forms.tagged(state) || 0) > 0) {
          yield wait_for(
            ...
            [
              { asyncResult: "result" },
              coreTags.loseTags(state, side, ":all", {
