  ...
  events: [
    {
      ...
      optional: {
        "waiting-prompt": true,
        prompt:
          "Trash Acacia to gain 1 [Credits] for each purged virus counter?",
        "yes-ability": {
          async: true,
          effect: req(function* (
            state: State,
            ...
            yield wait_for(
              ...
              [
                { asyncResult: "result" },
                coreMoving.trash(state, side, eid, { causeCard: card }),
              ],
              [],
            );
            systemMsg(
              state,
              side,
              `trashes Acacia and gains ${counters} [Credit]`,
            );
...
export const adjustedMatrix: CardDef = {
  ...
  "on-install": {
    req: req(function* (
      ...
      return allActiveInstalled.some((c: Card) =>
  ...
  abilities: [
    {
      ...breakSubFn(toC(":lose-click", 1), 1, "All", {
        req: req(function* (
          state: State,
          side?: Side,
          eid?: EID,
  ...
  prevention: [
    ...
    },
    {
      ...
      ability: {
        ...
        msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
          `prevent the encounter ability on ${(forms.context(state, card, targets) as any)?.ice?.title || "the encountered ice"}`,
        effect: req(function* (
          state: State,
          side: Side,
          eid: EID,
            ...
            optional: {
              ...
              "yes-ability": {
                ...
                msg: (
                  ...
                  `bypass ${cardStr(state, (forms.context(state, card, targets) as any)?.ice)}`,
                effect: req(function* (
                  state: State,
                  side: Side,
                  eid: EID,
  ...
  events: [
    {
      ...
      optional: {
        ...
        req: req(function* (
          ...
          const ctx = forms.context(state, card, targets) || {};
          return (
            (ctx.side === ":runner" || ctx.side === "runner") &&
            ctx.amount > 0 &&
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const hasEvent = eventTargets.some(
          (t: any) => t.card && eventFn(t.card),
        );
        if (!hasEvent) return false;
        return firstEventFn(state, null, "trash", (t: any[]) =>
          t.some((x: any) => x.card && eventFn(x.card)),
        );
      }),
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return (
          (ctx.server === ":archives" || ctx.server === "archives") &&
          (run?.maxAccess ?? 0) !== 0 &&
            ...
            optional: {
              ...
              "yes-ability": {
                ...
                msg: (
                  ...
                  return ((): string => {
                    const target: any = (targets as any[])?.[0];
                    return `remove ${target?.title || "the target"} from the game`;
                  })();
                },
                effect: effect(
                  (
                    ...
                    const target: any = (targets as any[])?.[0];
                    coreMoving.move(":corp", target, ":rfg");
                  },
                ),
              },
            },
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        if (!prevZone.includes("hand") && !prevZone.includes(":hand"))
          return false;
        return firstEventFn(state, ":runner", "runner-install", (t: any[]) => {
          const first = t[0];
    ...
    },
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreMoving.trash(eid, card, { causeCard: card });
        },
      ),
    },
  ],
...
export const basilarSynthgland: CardDef = {
  ...
  "on-install": {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        coreDamage.damage(eid, ":brain", 2, { card: card });
      },
    ),
  },
                ...
                side,
                {
                  ...
                  effect: effect(
                    (
                      ...
                      rezFn(state, ":corp", eid, cardsList[0]);
                    },
                  ),
                },
                card,
              ...
              side,
              {
                ...
                effect: req(function* (
                  ...
                  if (chosen) {
                    ...
                    const additionalCosts = rezAdditionalCostBonusFn(
                      s,
                      sd,
                      chosen,
                    );
                    ...
                    const remaining = cardsList.filter(
                      (x) => !sameCard(x, chosen),
                    );
  ...
  events: [
    {
      ...
      optional: {
        ...
        req: req(function* (
          ...
          const hasNoCredits = !costs.some(
            (c: any) =>
              (c["cost/type"] === ":credit" || c["cost/type"] === "credit") &&
              c["cost/amount"] > 0,
          );
        ...
        "yes-ability": {
          msg: (
            ...
            card: Card,
            targets: any[],
          ) => `host ${runnerFn(state)?.deck?.[0]?.title || "the top card"}`,
          effect: req(function* (
            state: State,
            side: Side,
            eid: EID,
            ...
            const timesHosted = Math.min(
              eventCountFn(state, null, ":bling-hosted"),
              10,
            );
...
export const boomerang: CardDef = {
  ...
  "on-install": {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
      const target: any = (targets as any[])?.[0];
      return `target ${cardStr(state, target)}`;
    },
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        coreUpdate.update(state, side, {
          ...card,
          special: { ...card.special, "boomerang-target": target },
        });
      },
    ),
  },
                ...
                optional: {
                  req: req(function* (
                    ...
                    return !!(
                      ...
                      (runnerFn(state)?.discard || []).some(
                        (c: Card) => c.title === cardObj2.title,
                      )
                    );
                  }),
                  once: ":per-run",
...
export const boxE: CardDef = {
  title: "Box-E",
  "static-abilities": [muPlusFn(2), runnerHandSizePlusFn(2)],
};
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        const hasTrash = targets.some(
          (t: any) =>
            runnerFn(t.card) && (inHandFn(t.card) || inDeckFn(t.card)),
        );
        if (!hasTrash) return false;
        return firstEventFn(state, null, "trash", (t: any[]) =>
          t.some(
            (x: any) =>
              runnerFn(x.card) && (inHandFn(x.card) || inDeckFn(x.card)),
          ),
        );
      }),
      interactive: req(function* (
      ...
      effect: req(function* (
        ...
        systemMsg(
          state,
          side,
          `uses ${card.title} to add ${target} to the bottom of the stack`,
        );
  ...
  abilities: [
    {
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        return ((): string => {
          const target: any = (targets as any[])?.[0];
          return `add ${target?.title || ""} to the top of the stack`;
        })();
      },
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          const target: any = (targets as any[])?.[0];
          moveFn(target, ":deck", { front: true });
        },
      ),
    },
  ],
