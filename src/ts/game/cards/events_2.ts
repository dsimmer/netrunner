...
export const carpeDiem: CardDef = {
  ...
  onPlay: {
    ...
    effect: req(function* (
      ...
      yield wait_for(
        ...
        [
          ...
          coreEngine.resolveAbility(
            state,
            side,
            coreMark.identifyMarkAbility as any,
            card,
            null as any,
          ),
        ],
      ...
      (state as any).runner[card.title as string] =
        coreServers.centralToName(markedServer);
      coreSay.systemMsg(state, side, `uses ${card.title} to gain 4 [Credits]`);
      yield wait_for(
        state,
        [
          { asyncResult: "result" },
          coreGaining.gainCredits(state, "runner", 4),
        ],
        [],
      );
              ...
              effect: effect(
                (
                  ...
                  coreSay.systemMsg(
                    state,
                    side,
                    `declines to use ${card.title} to make a run`,
                  );
                },
              ),
            },
            yesAbility: {
              ...
              effect: effect(
                (
                  ...
                  coreRuns.makeRun(state, side, eid, markedServer, card);
                },
              ),
            },
          },
  ...
  return {
    ...
    effect: effect(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
        ...
        const targetCard =
          remaining.find((c: Card) => c.title === target) ?? target;
        continue_ability(
          (() => {
            const newChosen = [targetCard, ...chosen];
            if (newChosen.length < n) {
              return cbiChoice(
                remaining.filter((c: Card) => c.title !== targetCard.title),
                newChosen,
                n,
                original,
              );
      ...
      if (target === "Done") {
        for (const c of chosen.reverse()) {
          yield wait_for(
            ...
            [
              { asyncResult: "result" },
              coreMoving.move(state, "corp", c, "deck", { front: true }),
            ],
            [],
          );
        }
        coreSay.systemMsg(
          state,
          side,
          `The top cards of R&D are ${chosen.map((c: Card) => c.title).join(", ")}`,
          { logSide: "corp" },
        );
        ...
        yield continue_ability(
          state,
          side,
          cbiChoice(from, [], from.length, from),
          card,
          null,
        );
      }
              ...
              (
                ...
                continue_ability(
                  ...
                  "corp",
                  {
                    ...
                    choices: {
                      card: (c: Card) =>
                        coreCard.runner(c) && coreCard.installed(c),
                    },
                    waitingPrompt: true,
                    displaySide: "corp",
                    effect: effect(
                      (
                        ...
                        coreMoving.trash(state, "corp", eid, targets?.[0]);
                      },
                    ),
                  },
                  card,
  ...
  events: [
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          ...
          coreUpdate.updateIn(
            card as any,
            ["special", "accessed"],
            (s: any) => [...(s || []), ctx.accessedCard.title],
          );
        },
      ),
    },
    {
      ...
      req: req(function* (
        ...
        return (
          ((coreCard.getCard(state, card) as any)?.special?.accessed?.length ||
            0) > 0
        );
      }),
      async: true,
      ...
      effect: req(function* (
        ...
        const accessedTitles =
          (coreCard.getCard(state, card) as any)?.special?.accessed || [];
        const intersection = (rezzedTitles as string[]).filter((t: string) =>
          (accessedTitles as string[]).includes(t),
        );
...
export const chrysopoeianSkimming: CardDef = {
  ...
  onPlay: {
    ...
    choices: req(function* (
      ...
      const agendas =
        (state as any).corp?.hand?.filter((c: Card) => coreCard.agenda(c)) ||
        [];
    ...
    effect: req(function* (
      ...
      if (target === "Done") {
        coreSay.systemMsg(
          state,
          "corp",
          "declines to reveal an agenda from HQ",
        );
  ...
  events: [
    {
      ...
      ability: {
        ...
        choices: req(function* (
          ...
          const rdIce =
            -3 * ((state as any).corp?.servers?.rd?.ices?.length || 0);
          return (
        ...
        effect: req(function* (
          ...
          yield wait_for(
            state,
            [{ asyncResult: "result" }, coreTags.gainTags(state, side, eid, 1)],
            [],
          );
        }),
      },
    ...
    choices: {
      req: req(function* (
        ...
        return corePrompts.cancellable(
          (state as any).runner?.[where]?.filter((c: Card) =>
            coreCard.program(c),
          ) || [],
        );
      }),
      ...
      if (target) {
        coreEffects.registerLingeringEffect(state, side, card, {
          ...
          req: req(function* (
            ...
            return utils.sameCard(target, target);
          }),
        ...
        coreEngine.registerEvents(state, side, card, [
          {
            ...
            onChangeGameState: {
              req: req(function* (
                ...
                return coreCard.installed(coreCard.getCard(state, target));
              }),
              silent: true,
            },
            msg: msg(
              ...
              (
                ...
                card: Card,
                targets: any[],
              ) => coreToString.cardStr(state, target),
    ...
    effect: req(function* (
      ...
      if (topCard) {
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreMoving.move(state, "runner", topCard, "hand"),
          ],
          [],
        );
      }
      yield continue_ability(
        ...
        side,
        runAnyServerAbility(),
        (coreCard.getCard(state, card) ?? card) as Card,
        null,
      );
    }),
  },
    ...
    choices: {
      req: req(function* (
        ...
        return (
          ...
          coreCard.runner(t) &&
          coreVirus.getVirusCounters(state, t) === 0
        );
      }),
    },
    onChangeGameState: {
      req: req(function* (
        ...
        return (coreBoard.allActiveInstalled(state, "runner") || []).some(
          (c: Card) => coreVirus.getVirusCounters(state, c) === 0,
        );
      }),
    },
    async: true,
  ...
  events: [
    {
      ...
      req: req(function* (
        ...
        return coreEvents.firstEvent(state, side, "runner-install");
      }),
      msg: "force the Corp to lose 1 [Credit]",
      async: true,
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreGaining.lose(state, "corp", "credit", 1);
        },
      ),
    },
  ],
...
export const corporateScandal: CardDef = {
  ...
  onPlay: {
    ...
    effect: req(function* (
      ...
      coreUpdate.updateIn(
        state,
        ["corp", "bad-publicity", "additional"],
        (n: number | undefined) => (n || 0) + 1,
      );
      coreSay.systemMsg(
        state,
        side,
        "give the Corp 1 additional bad publicity",
      );
    }),
  },
      ...
      effect: req(function* (
        ...
        if (
          ...
          yield continue_ability(
            ...
            "corp",
            {
              optional: {
                waitingPrompt: true,
                prompt: msg(
                  `Spend ${cost} [Credits] to prevent the trash of ${title}?`,
                ),
                player: "corp",
                yesAbility: {
                  async: true,
                  effect: effect(
                    (
                      ...
                      coreSay.systemMsg(
                        state,
                        "corp",
                        `spends ${cost} [Credits] to prevent ${title} from being trashed at no cost`,
                      );
                      coreGaining.lose(state, "corp", "credit", cost);
                    },
                  ),
                },
                noAbility: {
          ...
          coreSay.systemMsg(
            state,
            side,
            `uses ${card.title} to trash ${title} at no cost`,
          );
          yield wait_for(
            state,
            [
              { asyncResult: "result" },
              coreMoving.trash(state, side, eid, { ...c, seen: true }, null),
            ],
            [],
          );
        }
...
export const creditKiting: CardDef = {
  ...
  onPlay: {
    req: req(function* (
      ...
      const reg = (state as any).runner?.register;
      return reg?.successfulRun?.some((s: string) =>
        ["hq", "rd", "archives"].includes(s),
      );
    }),
    ...
    effect: req(function* (
      ...
      yield wait_for(
        state,
        [{ asyncResult: "result" }, coreTags.gainTags(state, "runner", eid, 1)],
        [],
      );
    }),
  },
...
export const cyberThreat: CardDef = {
  ...
  onPlay: {
    ...
    effect: req(function* (
      ...
      const canRez = corpInstalled.some(
        (c: Card) =>
          coreCard.installed(c) &&
          !coreCard.rezzed(c) &&
          coreCard.ice(c) &&
          corePayment.canPay(state, side, eid, card, null, [
            corePayment.toC("credit", coreCostFns.rezCost(state, side, c) || 0),
          ]),
      );
      yield continue_ability(
        state,
                  ...
                  choices: {
                    card: (c: Card) =>
                      ...
                      corePayment.canPay(state, side, eid, card, null, [
                        corePayment.toC(
                          "credit",
                          coreCostFns.rezCost(state, side, c) || 0,
                        ),
                      ]),
                  },
                  effect: effect(
                    (
                      ...
                      coreRezzing.rez(state, "corp", eid, targets?.[0]);
                    },
                  ),
                  cancel: {
                    async: true,
                          ...
                          function (state: State, _side: Side, card: Card) {
                            if (coreCard.ice(card)) {
                              coreToasts.toast(
                                state,
                                "corp",
                                "Cannot rez ice on this run due to Cyber Threat",
                              );
                              return false;
                            }
                            return true;
  ...
  events: [
    {
      ...
      effect: effect(
        (state: State, side: Side, eid: EID, card: Card, targets: any[]) => {
          coreEngine.registerEvents(state, side, card, [
            coreDefHelpers.breachAccessBonus(
              "rd",
              Math.max(0, Math.min(4, coreMemory.availableMu(state))),
              { duration: "end-of-run" },
            ),
          ]);
        },
...
export const deepDive: CardDef = {
  ...
  onPlay: {
    ...
    effect: req(function* (
      ...
      coreSetAside.setAside(
        state,
        "corp",
        eid,
        (state as any).corp?.deck?.slice(0, 8) || [],
      );
      const top8 = (coreSetAside.getSetAside(state, "corp", eid) || []).sort(
        (a: Card, b: Card) => (a.title || "").localeCompare(b.title || ""),
      );
      coreSay.systemMsg(
        state,
        side,
        `uses ${card.title} to set aside ${top8.map((c: Card) => c.title).join(", ")} from the top of R&D`,
      );
      yield wait_for(
        state,
        [
          ...
          coreEngine.resolveAbility(
            ...
            {
              async: true,
              prompt: `The set aside cards are: ${top8.map((c: Card) => c.title).join(", ")}`,
              choices: ["OK"],
            },
            card,
            null,
                ...
                optional: {
                  ...
                  req: req(function* (
                    ...
                    return corePayment.canPay(
                      state,
                      "runner",
                      { ...eid, source: card, sourceType: "ability" },
                      card,
                      null,
                    ...
                    effect: effect(
                      (
                        ...
                        coreSay.systemMsg(
                          state,
                          side,
                          `declines to use ${card.title} to access another card`,
                        );
                      },
                    ),
                  },
