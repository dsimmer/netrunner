...
export const lemuriaCodecracker: CardDef = {
  ...
  abilities: [{
    ...
    label: 'Expose a card',
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; exposeFn(eid, [target], { card: card }); }),
  }],
};
...
export const lldsProcessor: CardDef = {
  ...
  events: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { pumpFn((forms.context(state, card, targets) as any)?.card, 1, ':end-of-turn'); }),
  }],
};
  ...
  events: [{
    ...
    optional: {
      ...
      'yes-ability': {
        ...
        effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; triggerEventFn(':searched-stack'); shuffleDeck(state, side, ':deck'); moveFn(target, ':hand'); }),
      },
    },
  }],
};
  ...
  abilities: [
    {
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `host ${enumerateCards(targets, ':sorted')}`,
      effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
...
export const maglectricRapid: CardDef = {
  ...
  events: [{
    ...
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; derezFn(state, side, eid, target); }),
  }],
};
...
export const maui: CardDef = {
  ...
  'static-abilities': [muPlusFn(2)],
  recurring: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
    const corp = corpFn(state);
    return (corp?.servers?.hq?.ices || []).length;
  }),
  interactions: {
...
export const maya: CardDef = {
  ...
  events: [{
    ...
    optional: {
      ...
      once: ':per-turn',
      prompt: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `Move ${ctxFn?.()?.['accessed-card']?.title || 'the card'} to the bottom of R&D?`,
      'yes-ability': {
        msg: 'move the card just accessed to the bottom of R&D',
        async: true,
        effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { moveFn(ctxFn?.()?.['accessed-card'], ':deck'); gainTagsFn(':runner', eid, 1); }),
      },
    },
  }],
};
...
export const methuselah: CardDef = {
  ...
  events: [{
    ...
    async: true,
    'waiting-prompt': true,
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `trash ${target.title} and place 2 [Credits] on itself`; })(); },
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> { const target: any = (targets as any[])?.[0];
...
export const mirror: CardDef = {
  ...
  events: [{
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { continue_ability({
      ...
      msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `replace 1 spent [Recurring Credits] on ${target.title}`; })(); },
      async: true,
...
export const monolith: CardDef = {
  ...
  'on-install': {
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { continue_ability(mHelper(1), card, null); }),
  },
  prevention: [{
    prevents: 'damage',
    type: 'ability',
  ...
  events: [
    {
      ...
      optional: {
        ...
        'yes-ability': {
          ...
          effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { registerEventsFn(card, [breachAccessBonus(':hq', 1, { duration: ':end-of-run' })]); }),
        },
      },
    },
    {
      ...
      optional: {
        ...
        'yes-ability': {
          ...
          effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { registerEventsFn(card, [breachAccessBonus(':rd', 1, { duration: ':end-of-run' })]); }),
        },
      },
    },
  ],
...
export const netReadyEyes: CardDef = {
  ...
  'on-install': {
    ...
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { coreDamage.damage(eid, ':meat', 2, { unboostable: true, card: card }); }),
  },
  events: [{
    event: 'run',
    req: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; return ((): string => { const target: any = (targets as any[])?.[0]; return `give ${target.title} +1 strength`; })(); },
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { const target: any = (targets as any[])?.[0]; pumpFn(target, 1, ':end-of-run'); }),
  }],
};
...
export const netChip: CardDef = {
  title: 'NetChip',
  let: {
    netChipCount: (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
  ...
  'enforce-conditions': {
    ...
    silent: true,
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `trash ${cardStr(state, getHostedFn(state, card).find((c: Card) => programFn(c)))} for violating hosting restrictions`,
    async: true,
    effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
  ...
  'static-abilities': [{
    ...
    'max-mu': req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
...
export const obelus: CardDef = {
  ...
  events: [{
    ...
    msg: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `draw ${quantify(totalCardsAccessedFn(forms.context(state, card, targets)) ?? 0, 'card')}`,
    async: true,
    effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { drawFn(eid, totalCardsAccessedFn(forms.context(state, card, targets)) ?? 0); }),
  }],
};
      ...
      'yes-ability': {
        ...
        effect: req(function*(state: State, side: Side, eid: EID, card: Card, targets: any[]): Generator<any, any, any> {
          ...
          continue_ability(state, ':runner', {
            optional: {
              prompt: (state: State, side: Side, eid: EID, card: Card, targets: any[]) => `Add ${(runnerFn(state)?.deck?.[0])?.title || 'the top card'} to bottom of the stack?`,
              'yes-ability': {
                msg: 'add the top card of the stack to the bottom',
                effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { moveFn(':runner', (runnerFn(state)?.deck?.[0]), ':deck'); }),
              },
              'no-ability': { effect: effect((state: State, side: Side, eid: EID, card: Card, targets: any[]) => { systemMsg('does not add the top card of the the stack to the bottom'); }) },
            },
          }, card, null);
        }),
      },
