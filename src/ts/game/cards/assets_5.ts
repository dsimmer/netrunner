...
import {
  advanceAmbush,
  credsOnRoundStart,
  takeNCreditsStartOfTurn,
} from "./assets_1";

// Luana Campos
export const luanaCampos: CardDef = {
  title: "Luana Campos",
  uninstall: req(function* (
    state: State,
    side: Side,
    eid: EID,
    card: Card,
    ...
    if (
      coreCard.rezzed(oldCard) &&
      coreCard.getCounters(oldCard, ":bad-publicity") > 0
    ) {
      yield wait_for(
        state,
        [
          ...
          coreEngine.resolveAbility(
            ...
            side,
            {
              msg: msg(
                (s: State) =>
                  `take ${coreCard.getCounters(oldCard, ":bad-publicity")} bad publicity`,
              ),
              async: true,
              effect: req(function* (
                s: State,
                ...
                yield wait_for(
                  ...
                  [
                    ...
                    coreBadPublicity.gainBadPublicity(
                      ...
                      sd,
                      e,
                      coreCard.getCounters(oldCard, ":bad-publicity"),
                    ),
                  ],
                  [],
                );
  ...
  events: [
    {
      ...
      "change-in-game-state": {
        req: req(function* (state: State): Generator<any, any, any> {
          return coreFlags.countBadPub(state) > 0;
        }),
        silent: true,
      },
      optional: {
        ...
        prompt:
          "Host a bad publicity counter to gain 3 [Credits] and draw a card?",
        "yes-ability": {
  ...
  "static-abilities": [
    {
      ...
      value: req(function* (
        ...
        return [corePayment.toC("credit", 3)];
      }),
    },
  ],
  events: [
    ...
    "static-abilities": [
      {
        ...
        req: req(function* (
          ...
          const maliaTarget = (coreCard.getCard(state, card) || (card as any))
            ?.special?.maliaTarget;
          return (
            coreCard.sameCard(targets[0], maliaTarget) ||
...
export const marilynCampaign: CardDef = (() => {
  const ability: any = {
    ...
    effect: req(function* (
      ...
      if (
        !(coreCard.getCounters(coreCard.getCard(state, card), ":credit") > 0)
      ) {
        yield wait_for(
          state,
          [
    ...
    prevention: [
      {
        ...
        ability: {
          ...
          req: req(function* (
            ...
            return ((state as any).prevent?.trash?.remaining || []).some(
              (c: any) => coreCard.sameCard(c.card, card),
            );
          }),
          effect: effect(function* (
            state: State,
  ...
  abilities: [
    ...
    },
    {
      ...
      cost: [
        ...
        corePayment.toC("power", 3),
        corePayment.toC("trash-can", 1),
      ],
      msg: "gain 4 [Click]",
      effect: effect(function* (
        state: State,
...
export const moonPool: CardDef = (() => {
  const moonPoolPlaceAdvancements = (x: number): any => ({
    ...
    prompt: msg(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
        `Choose an installed card to place advancement counters on (${x} remaining)`,
    ),
  ...
  const moonPoolRevealAbility: any = {
    ...
    choices: {
      card: (c: Card) =>
        coreCard.corp(c) && coreCard.inDiscard(c) && !coreCard.faceup(c),
      max: 2,
    },
    msg: msg(
      (state: State, side: Side, eid: EID, card: Card, targets: any[]) =>
    ...
    effect: req(function* (
      ...
      yield wait_for(
        state,
        [{ asyncResult: "result" }, coreRevealing.reveal(state, side, targets)],
        [],
      );
      ...
      const agendaCount = targets.filter((c: Card) =>
        coreCard.agenda(c),
      ).length;
      if (agendaCount > 0) {
        yield wait_for(
          ...
          [
            ...
            coreEngine.resolveAbility(
              state,
              side,
              moonPoolPlaceAdvancements(agendaCount),
              card,
              null,
            ),
          ],
  ...
  const moonPoolDiscardAbility: any = {
    ...
    effect: req(function* (
      ...
      yield wait_for(
        ...
        [
          { asyncResult: "result" },
          coreMoving.trashCards(state, ":corp", eid, targets, {
            causeCard: card,
          }),
        ],
        [],
  ...
  return {
    ...
    abilities: [
      {
        label:
          "Trash up to 2 cards from HQ. Shuffle up to 2 cards from Archives into R&D",
  ...
  abilities: [
    {
      ...
      choices: req(function* (
        ...
        return corePrompts.cancellable(
          (state as any).corp.deck.filter(
            (c: Card) =>
              coreCard.hasSubtype(c, "Alliance") &&
              (coreCard.operation(c)
                ? (state as any).corp.credit >= (c as any).cost
                : true),
          ),
          { sorted: true },
        );
      ...
      cancel: {
        ...(coreShuffling as any).shuffleMyDeck,
        action: true,
        cost: [corePayment.toC("click", 1)],
      },
      msg: msg(
  ...
  abilities: [
    {
      ...
      effect: req(function* (
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreProps.addProp(state, side, card, ":advance-counter", -1, {
              placed: true,
            }),
          ],
          [],
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreProps.addProp(state, side, eid, target, ":advance-counter", 1, {
              placed: true,
            }),
          ],
          [],
  ...
  abilities: [
    {
      ...
      prompt: msg((state: State, side: Side, eid: EID, card: Card) => {
        const rezzedCount = coreFinding
          .allInstalled(state, ":corp")
          .filter(
            (c: Card) =>
              (c as any).title === (card as any).title && coreCard.rezzed(c),
          ).length;
...
export const neurostasis: CardDef = {
  title: "Neurostasis",
  ...advanceAmbush(3, {
    req: req(function* (
      state: State,
...
export const ngoFront: CardDef = (() => {
  function builder(cost: number, cred: number): any {
    return {
      cost: [
        corePayment.toC("advancement", cost),
        corePayment.toC("trash-can", 1),
      ],
      async: true,
      effect: effect(function* (
    ...
    effect: req(function* (
      ...
      if (coreCard.getCounters(coreCard.getCard(state, card), ":credit") > 0) {
        ...
        yield wait_for(
          ...
          [
            { asyncResult: "result" },
            coreMoving.trash(state, ":corp", card, {
              unpreventable: true,
              causeCard: card,
            }),
          ],
    ...
    },
    {
      ...
      "change-in-game-state": {
        ...
        req: req(function* (
          ...
          return (
            coreFlags.tagged(state) || coreBadPublicity.countBadPub(state) > 0
          );
        }),
      },
      ...
      effect: req(function* (
        ...
        yield wait_for(
          state,
          [{ asyncResult: "result" }, coreTags.loseTags(state, side, eid, 1)],
          [],
        );
      }),
    },
