...
import { fetchData, FetchOptions } from "./nrdb.js";
		...
		switch (arg) {
			...
			case "--local": {
				...
				if (!path) {
					...
					errors.push(`Could not find local data file at ${path}/edn/raw_data.edn`);
				} else {
					options.local = path;
				}
				break;
			...
			case "--repo": {
				const repo = args[++i];
				if (!repo) { errors.push("--repo requires a REPO argument"); }
				else { options.repo = repo; }
				break;
			}
			case "-b":
			case "--branch": {
				const branch = args[++i];
				if (!branch) { errors.push("--branch requires a BRANCH argument"); }
				else { options.branch = branch; }
				break;
			}
			case "-d":
			case "--db":
