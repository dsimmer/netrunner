...
import { createIndexes } from "./index.js";
...
function parseEdn(str: string): EdnValue {
	...
	function parseValue(): EdnValue {
		...
		if (c === ",") { pos++; return parseValue(); } // commas are whitespace in EDN
		return parseAtom();
	}
...
async function replaceCollection(db: Db, col: string, data: EdnValue[]): Promise<void> {
	const coll = db.collection(col);
	await coll.deleteMany({});
	if (data.length > 0) {
		const docs = data.map((item) => ednToDoc(item)) as Record<string, unknown>[];
...
function cardImageFile(code: string): string {
	return join("resources", "public", "img", "cards", "en", "default", "stock", `${code}.png`);
}
...
function generatePreviousCardStubs(cards: CardStub[]): CardStub[] {
	...
	for (const card of cards) {
		const prev = card["previous-versions"] as Array<{ code: string }> | undefined;
		if (prev) {
			for (const v of prev) {
...
function deriveFaceNameToIndex(card: MultiCard): Record<string, number> {
	...
	if (Object.keys(namedFaces).length > 0 && Object.keys(result).length === 0) {
		console.log(`Warning: card ${card.code} has named-faces but no face index could be derived`);
	}
...
function faceDownloadPlan(card: MultiCard): FacePlan[] {
	...
	const plans: FacePlan[] = [
		{ url: `${JNET_IMAGE_URL_V2}${code}.jpg`, file: cardImageFile(`${code}-front`) },
	];
...
export async function downloadCardImages(cards: CardStub[]): Promise<void> {
	...
	const missingCards = allCards.filter((c) => !existsSync(cardImageFile(c.code)));
	const total = allCards.length;
	const missing = missingCards.length;

	if (missing > 0) {
		console.log(`Have art for ${total - missing}/${total} cards. Downloading ${missing} missing images...`);
...
export async function downloadFaceImages(cards: CardStub[]): Promise<void> {
	...
	if (missingCount > 0) {
		console.log(`Have ${total - missingCount}/${total} face images. Downloading ${missingCount} missing...`);
