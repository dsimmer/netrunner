...
function updateDeckWithCards(
  ...
  const cards: DeckLine[] = rawCards.map((line) => {
    const title =
      typeof line.card === "string"
        ? line.card
        : ((line.card as CardData)?.title ?? "");
    return {
      qty: (line.qty as number) ?? 1,
      card: allCards.get(title) ?? { title },
    };
  });

  const rawId = deck.identity as Record<string, unknown> | string | undefined;
  const idTitle =
    typeof rawId === "string" ? rawId : (rawId?.title as string | undefined);
  const identity = idTitle
    ? (allCards.get(idTitle) ?? { title: idTitle })
    : undefined;
...
export async function updateAllDecks(): Promise<void> {
  ...
  try {
    ...
    for await (const rawDeck of cursor) {
      ...
      const identity = rawDeck.identity as
        | Record<string, unknown>
        | string
        | undefined;
      const identityEmpty =
        !identity ||
        (typeof identity === "object" && Object.keys(identity).length === 0);
      if (identityEmpty) {
        console.log(`Skipping deck ${deckId}: nil/empty identity`);
        continue;
      }

      const deck = updateDeckWithCards(
        rawDeck as unknown as Record<string, unknown>,
        allCards,
      );
      const status: DeckStatus = calculateDeckStatus(deck);
      await db
        .collection("decks")
        .updateOne({ _id: toObjectId(deckId) }, { $set: { status } });
    }
    console.log();
  ...
  return Promise.all(
    urls.map(async (url) => {
      ...
      if (!deck || !(deck.cards as DeckLine[])?.length) {
        throw new Error(
          "A deck contains no cards. Did you forget to run `fetch`?",
        );
      }
...
function createSampleDeck(
  ...
  const { deck, status } =
    sampleDecks[Math.floor(Math.random() * sampleDecks.length)];
...
function samplesForUser(
  ...
  if (userIndex === 1)
    return Math.ceil(minTotalSamples - (users - 2) * commonCase);
  return Math.ceil(commonCase);
}

function totalSamples(users: number, samples: number): number {
  return (
    samplesForUser(1, users, samples) +
    (users - 2) * samplesForUser(2, users, samples)
  );
}
...
export async function createSampleData(
  ...
  try {
    ...
    console.log(
      ...
      users,
      "users,",
      totalSamples(users, decks),
      "decks,",
      totalSamples(users, gameLogs),
      "game-logs and",
      totalSamples(users, messages),
      "messages.",
    );
    ...
    for (let b = 0; b < Math.ceil(users / BATCH_SIZE); b++) {
      ...
      for (
        let k = b * BATCH_SIZE;
        k < Math.min((b + 1) * BATCH_SIZE, users);
        k++
      ) {
      ...
      if (userBatch.length)
        await db.collection("users").insertMany(userBatch, { ordered: false });
      if (deckBatch.length)
        await db.collection("decks").insertMany(deckBatch, { ordered: false });
      if (gameLogBatch.length)
        await db
          .collection("game-logs")
          .insertMany(gameLogBatch, { ordered: false });
      if (messageBatch.length)
        await db
          .collection("messages")
          .insertMany(messageBatch, { ordered: false });
    }

    console.log("Successfully created sample data.");
    console.log(
      `You can now login with e.g. username "${usernamePrefix}1", password "password".`,
    );
  } catch (e) {
