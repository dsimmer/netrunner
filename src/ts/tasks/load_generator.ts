...
import { connect, disconnect, TaskSystem } from "./setup.js";
	...
	return new Promise((resolve) => {
		...
		const req = lib.request(
			...
			(res: http.IncomingMessage) => {
				...
				res.on("end", () => {
					const bodyStr = options.asText ? Buffer.concat(chunks).toString("utf-8") : undefined;
					resolve({
...
async function addTestUsers(system: TaskSystem, maxUsers: number): Promise<void> {
	const playingUsers = ["TestCorp", "TestRunner"];
	const watchingUsers = Array.from({ length: maxUsers }, (_, n) => `TestUser${n}`);
	...
	const existing = await system.db
		...
		.toArray();
	const existingSet = new Set((existing as unknown as { username: string }[]).map((u) => u.username));
...
async function login(username: string, password: string): Promise<Record<string, string>> {
	...
	if (postRes.error || postRes.status === 401) {
		safePrintln("Login failed for", username, ":", postRes.error?.message || postRes.status);
		return {};
	}

	const postCookie = ((postRes.headers["set-cookie"] as string) || "") as string;
	...
	if (getRes.error || getRes.status === 401) {
		safePrintln("Login failed for", username, ":", getRes.error?.message || getRes.status);
		return {};
	}
...
async function createGame(system: TaskSystem, maxUsers: number): Promise<void> {
	...
	const corpDecks = (await system.db.collection("decks").find({ username: "TestCorp" }).toArray()) as {
		_id: unknown;
		identity?: { side?: string };
	}[];
	const runnerDecks = (await system.db.collection("decks").find({ username: "TestRunner" }).toArray()) as {
		_id: unknown;
		identity?: { side?: string };
	}[];
	...
	const corpWs = new WebSocket(corpWsUrl, { headers: corpLogin, maxPayload: maxPayloadBytes });
	...
	const runnerWs = new WebSocket(runnerWsUrl, { headers: runnerLogin, maxPayload: maxPayloadBytes });

	runnerWs.on("open", () => safePrintln("Runner Connected"));
	runnerWs.on("error", (err: Error) => safePrintln("runner error", err.message));
	...
	const games = (await system.db.collection("games").find({}).toArray()) as { _id: unknown }[];
	...
	for (let n = 0; n < maxUsers; n++) {
		...
		const socket = new WebSocket(spectatorUrl, { headers: creds, maxPayload: maxPayloadBytes });
...
export { connect, disconnect, createGame, addTestUsers, login, parseArgs, usage, exit, command };
