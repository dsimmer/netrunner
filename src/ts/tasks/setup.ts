...
import { SetAllCards, SetCycles, SetMWL, SetSets } from "../jinteki/cards.js";
...
export async function connect(): Promise<TaskSystem> {
	...
	const allCards = cards.reduce<Record<string, Record<string, unknown>>>((acc, card) => {
...
export function disconnect(system: TaskSystem): void {
	if (system) {
		...
		system.client.close().catch((err) => console.error("Error closing MongoDB connection:", err));
	}
}
