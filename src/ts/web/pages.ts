...
function ogDefault(key: keyof OgData, defaultValue: string, og?: OgData): string {
...
export function resetPasswordPage(
  ...
  if (!token) {
    return response(404, { message: 'Sorry, but that reset token is invalid or has expired.' });
  }
  ...
  db.findOne(
    ...
    (err: Error | null, doc: Document | null) => {
      if (err || !doc) {
        const notFound = response(404, { message: 'Sorry, but that reset token is invalid or has expired.' });
